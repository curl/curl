 ...
 * functions.
 *
 * These callback may return `FETCH_WRITEFUNC_PAUSE` to indicate that the
                             ...
                             size_t *pmax_write, size_t *pmin_write)
{
  ...
  case CW_OUT_BODY:
    ...
    *pmax_write = FETCH_MAX_WRITE_SIZE;
...
bool Curl_cw_out_is_paused(struct Curl_easy *data)
{
  ...
  FETCH_TRC_WRITE(data, "cw-out is%spaused", ctx->paused ? "" : " not");
  return ctx->paused;
}
