                               ...
                               struct Curl_easy *data)
{
  ...
  switch(new_state) {
  case H1_TUNNEL_INIT:
    CURL_LOG_CF(data, cf, "new tunnel state 'init'");
    tunnel_reinit(ts, cf->conn, data);
    break;

  case H1_TUNNEL_CONNECT:
    CURL_LOG_CF(data, cf, "new tunnel state 'connect'");
  ...
  case H1_TUNNEL_RECEIVE:
    CURL_LOG_CF(data, cf, "new tunnel state 'receive'");
    ts->tunnel_state = H1_TUNNEL_RECEIVE;
    break;

  case H1_TUNNEL_RESPONSE:
    CURL_LOG_CF(data, cf, "new tunnel state 'response'");
    ts->tunnel_state = H1_TUNNEL_RESPONSE;
    break;

  case H1_TUNNEL_ESTABLISHED:
    CURL_LOG_CF(data, cf, "new tunnel state 'established'");
  ...
  case H1_TUNNEL_FAILED:
    if(new_state == H1_TUNNEL_FAILED)
      CURL_LOG_CF(data, cf, "new tunnel state 'failed'");
                               ...
                               const char *header)
{
  ...
  if((checkprefix("WWW-Authenticate:", header) &&
    ...
    CURL_LOG_CF(data, cf, "CONNECT: fwd auth header '%s'", header);
    ...
    if(('\r' == linep[0]) ||
      ...
      if((407 == k->httpcode) && !data->state.authproblem) {
        ...
        if(ts->cl) {
          ...
          CURL_LOG_CF(data, cf, "CONNECT: no content-length or chunked");
          ts->keepon = KEEPON_DONE;
        }
      }
      else {
                           ...
                           struct h1_tunnel_state *ts)
{
  ...
  do {
    ...
    case H1_TUNNEL_INIT:
      ...
      CURL_LOG_CF(data, cf, "CONNECT start");
    ...
    case H1_TUNNEL_CONNECT:
      ...
      CURL_LOG_CF(data, cf, "CONNECT send");
    ...
    case H1_TUNNEL_RECEIVE:
      /* read what is there */
      CURL_LOG_CF(data, cf, "CONNECT receive");
    ...
    case H1_TUNNEL_RESPONSE:
      CURL_LOG_CF(data, cf, "CONNECT response");
      if(data->req.newurl) {
        ...
        if(ts->close_connection || conn->bits.close) {
           ...
           */
          CURL_LOG_CF(data, cf, "CONNECT need to close+open");
