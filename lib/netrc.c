...
static FETCHcode file2memory(const char *filename, struct dynbuf *filebuf)
{
  ...
  Fetch_dyn_init(&linebuf, MAX_NETRC_LINE);

  if (file)
  {
    while (Fetch_get_line(&linebuf, file))
    {
      const char *line = Fetch_dyn_ptr(&linebuf);
      /* skip comments on load */
      while (ISBLANK(*line))
        line++;
      if (*line == '#')
        continue;
      result = Fetch_dyn_add(filebuf, line);
      if (result)
        goto done;
    }
  }
done:
  Fetch_dyn_free(&linebuf);
  if (file)
    fclose(file);
  return result;
}
                      ...
                      const char *netrcfile)
{
  ...
  Fetch_dyn_init(&token, MAX_NETRC_TOKEN);
  ...
  netrcbuffer = Fetch_dyn_ptr(filebuf);
  ...
  while (!done)
  {
    ...
    while (tok && !done)
    {
      ...
      if (!quoted)
      {
        ...
        if (!len || Fetch_dyn_addn(&token, tok, len))
        {
      ...
      case HOSTVALID:
        ...
        {
          if (specific_login)
            our_login = !Fetch_timestrcmp(login, tok);
          else
          {
            our_login = TRUE;
            free(login);
 ...
 * in.
 */
int Fetch_parsenetrc(struct store_netrc *store, const char *host,
...
{
  Fetch_dyn_init(&s->filebuf, MAX_NETRC_FILE);
  s->loaded = FALSE;
}
void Fetch_netrc_cleanup(struct store_netrc *s)
{
  Fetch_dyn_free(&s->filebuf);
  s->loaded = FALSE;
}
#endif
