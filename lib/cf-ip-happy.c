...
evaluate:
  ...
  if(!ongoing) {
    ...
    do_more = more_possible &&
              (curlx_timediff_ms(data->state.now, bs->last_attempt_started) >=
               bs->attempt_delay_ms);
    if(do_more)
  ...
  if(do_more) {
    ...
    if(addr) {  /* try another address */
      ...
      *panchor = a;
      bs->last_attempt_started = data->state.now;
  ...
  if(!result) {
    ...
    if(next_expire_ms <= 0) {
      failf(data, "Connection timeout after %" FMT_OFF_T " ms",
            curlx_timediff_ms(data->state.now, data->progress.t_startsingle));
