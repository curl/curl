    ...
    if(data->req.newurl || retry) {
      ...
      if(!result) {
        if(data->info.retry_after && (follow == FOLLOW_REDIR)) {
          ...
          if(data->info.retry_after >
             (curl_off_t)TIME_T_MAX - data->state.retry_time.tv_sec)
            data->state.retry_time.tv_sec = TIME_T_MAX;
          else
            data->state.retry_time.tv_sec += (time_t)data->info.retry_after;

          multistate(data, MSTATE_RETRYAFTER);

          if(data->info.retry_after < (TIMEDIFF_T_MAX / 1000))
            delay_ms = (timediff_t)data->info.retry_after * 1000;
          else
            delay_ms = TIMEDIFF_T_MAX;
...
static CURLMcode multistate_retryafter(struct Curl_easy *data,
                                       CURLcode *resultp)
{
  const struct curltime *now = Curl_pgrs_now(data);
  timediff_t delay_ms;
  *resultp = CURLE_OK;
