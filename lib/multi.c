...
CURLMcode curl_multi_add_handle(CURLM *m, CURL *d)
{
  ...
  rc = Curl_update_timer(multi, &data->state.now);
  if(rc) {
                                     ...
                                     struct easy_pollset *ps)
{
  ...
  if(ps->n) {
    bool send_blocked =
      (Curl_rlimit_avail(&data->progress.dl.rlimit, &data->state.now) <= 0);
    bool recv_blocked =
      (Curl_rlimit_avail(&data->progress.ul.rlimit, &data->state.now) <= 0);
  ...
  if(timeout_ms < 0) {
    ...
    if(data->mstate == MSTATE_RESOLVING)
      failf(data, "Resolving timed out after %" FMT_TIMEDIFF_T
            " milliseconds", curlx_timediff_ms(data->state.now, since));
      ...
      if(k->size != -1) {
        failf(data, "Operation timed out after %" FMT_TIMEDIFF_T
              ...
              curlx_timediff_ms(data->state.now, since),
              k->bytecount, k->size);
      }
      else {
        failf(data, "Operation timed out after %" FMT_TIMEDIFF_T
              " milliseconds with %" FMT_OFF_T " bytes received",
              curlx_timediff_ms(data->state.now, since), k->bytecount);
      }
    }
...
static CURLcode mspeed_check(struct Curl_easy *data)
{
  ...
  send_wait_ms =
    Curl_rlimit_wait_ms(&data->progress.ul.rlimit, &data->state.now);
  recv_wait_ms =
    Curl_rlimit_wait_ms(&data->progress.dl.rlimit, &data->state.now);
                               ...
                               int *running_handles)
{
  ...
  if(Curl_uint32_bset_first(&multi->process, &mid)) {
    ...
    do {
      ...
      *pnow = data->state.now; /* in case transfer updated */
      if(result)
        returncode = result;
    } while(Curl_uint32_bset_next(&multi->process, mid, &mid));
  }
  ...
  if(Curl_uint32_bset_first(&multi->dirty, &mid)) {
    do {
      ...
      if(data) {
        ...
        data->state.now = mrc->now;
        result = multi_runsingle(multi, data);
        mrc->now = data->state.now; /* in case transfer updated */
                    ...
                    timediff_t milli, expire_id id)
{
  ...
  multi_addtimeout(data, &set, id, &data->state.now);
