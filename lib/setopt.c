...
{
  /* Release the previous storage at `blobp' and replace by a dynamic storage
     copy of `st_blobs'.
     if flags matches CURL_BLOB_COPY, the data are copied after the
     memory blob.
     if flags not matches CURL_BLOB_COPY, only the pointer is copied.
     memory blob.
  ...
  if(st_blobs) {
    ...
    if(st_blobs->len > CURL_MAX_INPUT_LENGTH)
      return CURLE_BAD_FUNCTION_ARGUMENT;
    nblob = (struct curl_blob *)malloc(sizeof(struct curl_blob) +
             ((st_blobs->flags & CURL_BLOB_COPY) ? st_blobs->len : 0));
    if(!nblob)
        return CURLE_OUT_OF_MEMORY;
    *nblob = *st_blobs;
    if(st_blobs->flags & CURL_BLOB_COPY) {
      /* put the data after the blob struct in memory */
      nblob->data = (char *)nblob + sizeof(struct curl_blob);
      memcpy(nblob->data, st_blobs->data, st_blobs->len);
    }
