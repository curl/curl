#ifndef HEADER_FETCH_MIME_H
#define HEADER_FETCH_MIME_H
/***************************************************************************
 *                                  _   _ ____  _
 *  Project                     ___| | | |  _ \| |
 *                             / __| | | | |_) | |
...
enum mimestate {
  ...
  MIMESTATE_FETCHHEADERS,        /* In fetch-generated headers. */
...
struct mime_encoder {
  ...
  size_t         (*encodefunc)(char *buffer, size_t size, bool ateof,
                               fetch_mimepart *part);  /* Encoded read. */
  fetch_off_t     (*sizefunc)(fetch_mimepart *part);  /* Encoded size. */
};
...
struct mime_state {
  ...
  fetch_off_t offset;          /* State-dependent offset. */
};
...
/* A mime multipart. */
struct fetch_mime {
  fetch_mimepart *parent;           /* Parent part. */
  fetch_mimepart *firstpart;        /* First part. */
  fetch_mimepart *lastpart;         /* Last part. */
  char boundary[MIME_BOUNDARY_LEN + 1]; /* The part boundary. */
  struct mime_state state;         /* Current readback state. */
};

/* A mime part. */
struct fetch_mimepart {
  fetch_mime *parent;               /* Parent mime structure. */
  fetch_mimepart *nextpart;         /* Forward linked list. */
  enum mimekind kind;              /* The part kind. */
  unsigned int flags;              /* Flags. */
  char *data;                      /* Memory data or filename. */
  fetch_read_callback readfunc;     /* Read function. */
  fetch_seek_callback seekfunc;     /* Seek function. */
  fetch_free_callback freefunc;     /* Argument free function. */
  void *arg;                       /* Argument to callback functions. */
  FILE *fp;                        /* File pointer. */
  struct fetch_slist *fetchheaders;  /* Part headers. */
  struct fetch_slist *userheaders;  /* Part headers. */
  char *mimetype;                  /* Part mime type. */
  char *filename;                  /* Remote filename. */
  char *name;                      /* Data name. */
  fetch_off_t datasize;             /* Expected data size. */
  struct mime_state state;         /* Current readback state. */
