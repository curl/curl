...
#ifdef FETCH_DISABLE_SOCKETPAIR
#error "MSH3 cannot be build with FETCH_DISABLE_SOCKETPAIR set"
#endif
                                  ...
                                  FETCHcode *err)
{
  ...
  if (stream->reset)
  {
    ...
    *err = FETCHE_PARTIAL_FILE;
    FETCH_TRC_CF(data, cf, "cf_recv, was reset -> %d", *err);
    goto out;
  }
  else if (stream->error3)
  {
    ...
    *err = FETCHE_HTTP3;
    FETCH_TRC_CF(data, cf, "cf_recv, closed uncleanly -> %d", *err);
    goto out;
  }
  else
  {
    FETCH_TRC_CF(data, cf, "cf_recv, closed ok -> %d", *err);
  }
  *err = FETCHE_OK;
  nread = 0;
                            ...
                            char *buf, size_t len, FETCHcode *err)
{
  ...
  FETCH_TRC_CF(data, cf, "cf_recv(len=%zu), stream=%d", len, !!stream);
  if (!stream)
  {
    *err = FETCHE_RECV_ERROR;
    return -1;
  ...
  *err = FETCHE_OK;
  ...
  {
    FETCH_TRC_CF(data, cf, "req: nothing here, call again");
    *err = FETCHE_AGAIN;
  }
                            ...
                            FETCHcode *err)
{
  ...
  DEBUGASSERT(stream);
  FETCH_TRC_CF(data, cf, "req: send %zu bytes", len);
  ...
  if (!stream->req)
  {
    ...
    *err = Curl_http_req_to_h2(&h2_headers, h1.req, data);
    ...
    FETCH_TRC_CF(data, cf, "req: send %zu headers", nheader);
  ...
  {
    /* request is open */
    FETCH_TRC_CF(data, cf, "req: send %zu body bytes", len);
    if (len > 0xFFFFFFFF)
    {
      len = 0xFFFFFFFF;
    }
    ...
    /* TODO - msh3/msquic will hold onto this memory until the send complete
       event. How do we make sure fetch does not free it until then? */
    *err = FETCHE_OK;
    nwritten = len;
  }
                                   ...
                                   struct easy_pollset *ps)
{
  ...
  CF_DATA_SAVE(save, cf, data);
  if (stream && ctx->sock[SP_LOCAL] != FETCH_SOCKET_BAD)
  {
    if (stream->recv_error)
    {
                                    ...
                                    int event, int arg1, void *arg2)
{
  ...
  case CF_CTRL_DATA_DONE_SEND:
    FETCH_TRC_CF(data, cf, "req: send done");
    if (stream)
    {
                                  ...
                                  struct Curl_easy *data)
{
  ...
  memcpy(&addr, &ctx->addr.fetch_sa_addr, ctx->addr.addrlen);
  ...
  if (verify && (conn_config->CAfile || conn_config->CApath))
  {
    ...
    FETCH_TRC_CF(data, cf, "non-standard CA not supported, "
  ...
  FETCH_TRC_CF(data, cf, "connecting to %s:%d (verify=%d)",
...
static void cf_msh3_close(struct Curl_cfilter *cf, struct Curl_easy *data)
{
  ...
  if (ctx)
  {
    FETCH_TRC_CF(data, cf, "destroying");
    if (ctx->qconn)
    {
    ...
    if (ctx->active)
    {
      ...
      {
        FETCH_TRC_CF(data, cf, "cf_msh3_close(%d) active",
                     (int)ctx->sock[SP_LOCAL]);
        cf->conn->sock[cf->sockindex] = FETCH_SOCKET_BAD;
      }
      else
      {
        FETCH_TRC_CF(data, cf, "cf_socket_close(%d) no longer at "
    ...
    if (ctx->sock[SP_REMOTE] != FETCH_SOCKET_BAD)
    {
                                  ...
                                  bool *input_pending)
{
  ...
  (void)data;
  *input_pending = FALSE;
  return ctx && ctx->sock[SP_LOCAL] != FETCH_SOCKET_BAD && ctx->qconn &&
         ctx->connected;
}
