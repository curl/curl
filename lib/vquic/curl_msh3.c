...
static void h3_data_done(struct Curl_cfilter *cf, struct Curl_easy *data)
{
  ...
  (void)cf;
  if(stream) {
    CURL_LOG_CF(data, cf, "easy handle is done");
                                          ...
                                          void *IfContext)
{
  ...
  CURL_LOG_CF(data, cf, "[MSH3] connected");
                                          ...
                                          void *IfContext)
{
  ...
  (void)Connection;
  CURL_LOG_CF(data, cf, "[MSH3] shutdown complete");
  ctx->connected = false;
  ctx->handshake_complete = true;
}
                                  ...
                                  CURLcode *err)
{
  ...
  if(stream->reset) {
    ...
    *err = CURLE_PARTIAL_FILE;
    CURL_LOG_CF(data, cf, "cf_recv, was reset -> %d", *err);
    goto out;
  }
    ...
    *err = CURLE_HTTP3;
    CURL_LOG_CF(data, cf, "cf_recv, closed uncleanly -> %d", *err);
    goto out;
  }
  else {
    CURL_LOG_CF(data, cf, "cf_recv, closed ok -> %d", *err);
  }
  *err = CURLE_OK;
  nread = 0;
                            ...
                            char *buf, size_t len, CURLcode *err)
{
  ...
  CURL_LOG_CF(data, cf, "req: recv with %zu byte buffer", len);
  ...
  *err = CURLE_OK;
  ...
  if(!Curl_bufq_is_empty(&stream->recvbuf)) {
    ...
    CURL_LOG_CF(data, cf, "read recvbuf(len=%zu) -> %zd, %d",
                len, nread, *err);
    if(nread < 0)
      goto out;
    if(stream->closed)
    ...
    CURL_LOG_CF(data, cf, "req: nothing here, call again");
    *err = CURLE_AGAIN;
  }
                            ...
                            const void *buf, size_t len, CURLcode *err)
{
  ...
  DEBUGASSERT(stream);
  CURL_LOG_CF(data, cf, "req: send %zu bytes", len);
  ...
  if(!stream->req) {
    ...
    *err = Curl_http_req_to_h2(&h2_headers, h1.req, data);
    ...
    CURL_LOG_CF(data, cf, "req: send %zu headers", nheader);
    ...
    *err = CURLE_OK;
    ...
    /* request is open */
    CURL_LOG_CF(data, cf, "req: send %zu body bytes", len);
    if(len > 0xFFFFFFFF) {
      len = 0xFFFFFFFF;
    }
                                    ...
                                    curl_socket_t *socks)
{
  ...
  CURL_LOG_CF(data, cf, "select_sock -> %d", bitmap);
  CF_DATA_RESTORE(cf, save);
  return bitmap;
}
                                 ...
                                 const struct Curl_easy *data)
{
  ...
  if(stream && stream->req) {
    msh3_lock_acquire(&stream->recv_lock);
    CURL_LOG_CF((struct Curl_easy *)data, cf, "data pending = %zu",
                                   ...
                                   int event, int arg1, void *arg2)
{
  ...
  case CF_CTRL_DATA_DONE_SEND:
    CURL_LOG_CF(data, cf, "req: send done");
    if(stream) {
      stream->upload_done = TRUE;
      if(stream->req) {
        char buf[1];
  ...
  case CF_CTRL_CONN_INFO_UPDATE:
    CURL_LOG_CF(data, cf, "req: update info");
                                 ...
                                 struct Curl_easy *data)
{
  ...
  if(verify && (cf->conn->ssl_config.CAfile || cf->conn->ssl_config.CApath)) {
    ...
    CURL_LOG_CF(data, cf, "non-standard CA not supported, "
  ...
  CURL_LOG_CF(data, cf, "connecting to %s:%d (verify=%d)",
  ...
  *done = FALSE;
  ...
  if(ctx->handshake_complete) {
    ...
    if(ctx->handshake_succeeded) {
      CURL_LOG_CF(data, cf, "handshake succeeded");
...
static void cf_msh3_close(struct Curl_cfilter *cf, struct Curl_easy *data)
{
  ...
  if(ctx) {
    CURL_LOG_CF(data, cf, "destroying");
    if(ctx->qconn) {
    ...
    if(ctx->active) {
      ...
      if(ctx->sock[SP_LOCAL] == cf->conn->sock[cf->sockindex]) {
        CURL_LOG_CF(data, cf, "cf_msh3_close(%d) active",
                    (int)ctx->sock[SP_LOCAL]);
        cf->conn->sock[cf->sockindex] = CURL_SOCKET_BAD;
      }
      else {
        CURL_LOG_CF(data, cf, "cf_socket_close(%d) no longer at "
