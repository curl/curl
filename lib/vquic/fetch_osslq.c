...
static FETCHcode cf_progress_ingress(struct Fetch_cfilter *cf,
...
static FETCHcode make_bio_addr(BIO_ADDR **pbio_addr,
                               const struct Fetch_sockaddr_ex *addr)
{
  BIO_ADDR *ba;
  ...
  case AF_INET:
  {
    struct sockaddr_in *const sin =
        (struct sockaddr_in *const)(void *)&addr->fetch_sa_addr;
  ...
  case AF_INET6:
  {
    struct sockaddr_in6 *const sin =
        (struct sockaddr_in6 *const)(void *)&addr->fetch_sa_addr;
...
static FETCHcode cf_osslq_stream_open(struct cf_osslq_stream *s,
                                      SSL *conn,
                                      uint64_t flags,
...
{
  DEBUGASSERT(!s->ssl);
  Fetch_bufq_initp(&s->recvbuf, bufcp, 1, BUFQ_OPT_NONE);
  s->ssl = SSL_new_stream(conn, flags);
  if (!s->ssl)
  {
    return FETCHE_FAILED_INIT;
  }
  s->id = (fetch_int64_t)SSL_get_stream_id(s->ssl);
...
struct cf_osslq_ctx
{
  ...
  struct Fetch_hash streams;       /* hash `data->mid` to `h3_stream_ctx` */
  size_t max_stream_window;       /* max flow window for one stream */
  uint64_t max_idle_ms;           /* max idle time for QUIC connection */
  SSL_POLL_ITEM *poll_items;      /* Array for polling on writable state */
  struct Fetch_easy **fetch_items; /* Array of easy objs */
  size_t item_count;              /* count of elements in poll/fetch_items */
  BIT(initialized);
...
static FETCHcode cf_osslq_shutdown(struct Fetch_cfilter *cf,
                                   struct Fetch_easy *data, bool *done)
{
  ...
  *done = FALSE;
  ...
  if (rc == 0)
  { /* ongoing */
    FETCH_TRC_CF(data, cf, "shutdown ongoing");
    ctx->need_recv = TRUE;
    goto out;
  }
  else if (rc == 1)
  { /* done */
    FETCH_TRC_CF(data, cf, "shutdown finished");
    *done = TRUE;
    goto out;
  }
  else
  {
    ...
    case SSL_ERROR_NONE:
    case SSL_ERROR_ZERO_RETURN:
      FETCH_TRC_CF(data, cf, "shutdown not received, but closed");
      *done = TRUE;
      goto out;
    case SSL_ERROR_WANT_READ:
      /* SSL has send its notify and now wants to read the reply
       * from the server. We are not really interested in that. */
      FETCH_TRC_CF(data, cf, "shutdown sent, want receive");
      ctx->need_recv = TRUE;
      goto out;
    case SSL_ERROR_WANT_WRITE:
      FETCH_TRC_CF(data, cf, "shutdown send blocked");
      ctx->need_send = TRUE;
      goto out;
    default:
      /* We give up on this. */
      sslerr = ERR_get_error();
      FETCH_TRC_CF(data, cf, "shutdown, ignore recv error: '%s', errno %d",
                   (sslerr ? osslq_strerror(sslerr, err_buffer, sizeof(err_buffer)) : osslq_SSL_ERROR_to_str(err)),
                   SOCKERRNO);
      *done = TRUE;
      result = FETCHE_OK;
      goto out;
...
static void cf_osslq_close(struct Fetch_cfilter *cf, struct Fetch_easy *data)
{
  struct cf_osslq_ctx *ctx = cf->ctx;
  struct cf_call_data save;
  ...
  {
    FETCH_TRC_CF(data, cf, "cf_osslq_close()");
    if (!cf->shutdown && !ctx->protocol_shutdown)
    {
...
static void cf_osslq_destroy(struct Fetch_cfilter *cf, struct Fetch_easy *data)
{
  struct cf_osslq_ctx *ctx = cf->ctx;
  struct cf_call_data save;
  ...
  if (ctx)
  {
    FETCH_TRC_CF(data, cf, "cf_osslq_destroy()");
    if (ctx->tls.ossl.ssl)
      cf_osslq_ctx_close(ctx);
...
static FETCHcode cf_osslq_h3conn_add_stream(struct cf_osslq_h3conn *h3,
                                            SSL *stream_ssl,
                                            struct Fetch_cfilter *cf,
                                            struct Fetch_easy *data)
{
  struct cf_osslq_ctx *ctx = cf->ctx;
  fetch_int64_t stream_id = (fetch_int64_t)SSL_get_stream_id(stream_ssl);

  if (h3->remote_ctrl_n >= ARRAYSIZE(h3->remote_ctrl))
  {
    /* rejected, we are full */
    FETCH_TRC_CF(data, cf, "[%" FMT_PRId64 "] rejecting remote stream",
                 stream_id);
    SSL_free(stream_ssl);
    return FETCHE_FAILED_INIT;
  }
  ...
  case SSL_STREAM_TYPE_READ:
  {
    ...
    Fetch_bufq_initp(&nstream->recvbuf, &ctx->stream_bufcp, 1, BUFQ_OPT_NONE);
    FETCH_TRC_CF(data, cf, "[%" FMT_PRId64 "] accepted remote uni stream",
                 stream_id);
    break;
  }
  default:
    FETCH_TRC_CF(data, cf, "[%" FMT_PRId64 "] reject remote non-uni-read"
                           " stream",
                 stream_id);
    SSL_free(stream_ssl);
    return FETCHE_FAILED_INIT;
...
static FETCHcode cf_osslq_ssl_err(struct Fetch_cfilter *cf,
                                  struct Fetch_easy *data,
                                  int detail, FETCHcode def_result)
{
  struct cf_osslq_ctx *ctx = cf->ctx;
  FETCHcode result = def_result;
  sslerr_t errdetail;
...
static FETCHcode cf_osslq_verify_peer(struct Fetch_cfilter *cf,
                                      struct Fetch_easy *data)
{
  struct cf_osslq_ctx *ctx = cf->ctx;
...
struct h3_stream_ctx
{
  ...
  fetch_off_t upload_left;      /* number of request bytes left to upload */
  fetch_off_t download_recvd;   /* number of response DATA bytes received */
...
#define H3_STREAM_CTX(ctx, data) ((struct h3_stream_ctx *)(data ? Fetch_hash_offt_get(&(ctx)->streams, (data)->mid) : NULL))
...
static void h3_data_done(struct Fetch_cfilter *cf, struct Fetch_easy *data)
{
  ...
  if (stream)
  {
    FETCH_TRC_CF(data, cf, "[%" FMT_PRId64 "] easy handle is done",
                 stream->s.id);
...
static struct cf_osslq_stream *cf_osslq_get_qstream(struct Fetch_cfilter *cf,
                                                    struct Fetch_easy *data,
                                                    int64_t stream_id)
{
  struct cf_osslq_ctx *ctx = cf->ctx;
...
{
  struct Fetch_cfilter *cf = user_data;
  struct cf_osslq_ctx *ctx = cf->ctx;
  struct Fetch_easy *data = stream_user_data;
  struct h3_stream_ctx *stream = H3_STREAM_CTX(ctx, data);
  (void)conn;
  (void)stream_id;
  ...
  if (stream->error3 != NGHTTP3_H3_NO_ERROR)
  {
    ...
    FETCH_TRC_CF(data, cf, "[%" FMT_PRId64 "] RESET: error %" FMT_PRIu64,
                 stream->s.id, stream->error3);
  }
  else
  {
    FETCH_TRC_CF(data, cf, "[%" FMT_PRId64 "] CLOSED", stream->s.id);
  }
  h3_drain_stream(cf, data);
  return 0;
}
                           ...
                           void *user_data, void *stream_user_data)
{
  ...
  if (result)
  {
    FETCH_TRC_CF(data, cf, "[%" FMT_PRId64 "] DATA len=%zu, ERROR %d",
                 stream->s.id, buflen, result);
    return NGHTTP3_ERR_CALLBACK_FAILURE;
  }
  stream->download_recvd += (fetch_off_t)buflen;
  FETCH_TRC_CF(data, cf, "[%" FMT_PRId64 "] DATA len=%zu, total=%zd",
                                  ...
                                  void *stream_user_data)
{
  ...
  (void)stream_id;
  if (stream)
    FETCH_TRC_CF(data, cf, "[%" FMT_PRId64 "] deferred consume %zu bytes",
                             ...
                             void *user_data, void *stream_user_data)
{
  ...
  if (token == NGHTTP3_QPACK_TOKEN__STATUS)
  {
    ...
    ncopy = msnprintf(line, sizeof(line), "HTTP/3 %03d \r\n",
                      stream->status_code);
    FETCH_TRC_CF(data, cf, "[%" FMT_PRId64 "] status: %s", stream_id, line);
  ...
  else
  {
    ...
    FETCH_TRC_CF(data, cf, "[%" FMT_PRId64 "] header: %.*s: %.*s",
                             ...
                             int fin, void *user_data, void *stream_user_data)
{
  ...
  FETCH_TRC_CF(data, cf, "[%" FMT_PRId64 "] end_headers, status=%d",
                              ...
                              void *stream_user_data)
{
  ...
  FETCH_TRC_CF(data, cf, "[%" FMT_PRId64 "] stop_sending", stream_id);
                              ...
                              void *stream_user_data)
{
  ...
  if (stream && stream->s.ssl)
  {
    ...
    FETCH_TRC_CF(data, cf, "[%" FMT_PRId64 "] reset -> %d", stream_id, rv);
    if (!rv)
    {
      return NGHTTP3_ERR_CALLBACK_FAILURE;
    }
                    ...
                    void *stream_user_data)
{
  ...
  if (stream->sendbuf_len_in_flight < Fetch_bufq_len(&stream->sendbuf))
  {
    nvecs = 0;
    while (nvecs < veccnt &&
           Fetch_bufq_peek_at(&stream->sendbuf,
  ...
  else if (!nwritten)
  {
    ...
    FETCH_TRC_CF(data, cf, "[%" FMT_PRId64 "] read req body -> AGAIN",
                 stream->s.id);
    return NGHTTP3_ERR_WOULDBLOCK;
  }

  FETCH_TRC_CF(data, cf, "[%" FMT_PRId64 "] read req body -> "
                         "%d vecs%s with %zu (buffered=%zu, left=%" FMT_OFF_T ")",
               stream->s.id, (int)nvecs,
               *pflags == NGHTTP3_DATA_FLAG_EOF ? " EOF" : "",
               nwritten, Fetch_bufq_len(&stream->sendbuf),
               stream->upload_left);
                                   ...
                                   void *stream_user_data)
{
  ...
  Fetch_bufq_skip(&stream->sendbuf, skiplen);
...
static FETCHcode cf_osslq_h3conn_init(struct cf_osslq_ctx *ctx, SSL *conn,
                                      void *user_data)
{
  ...
  if (result)
  {
    result = FETCHE_QUIC_CONNECT_ERROR;
    goto out;
  }
...
static FETCHcode cf_osslq_ctx_start(struct Fetch_cfilter *cf,
                                    struct Fetch_easy *data)
{
  struct cf_osslq_ctx *ctx = cf->ctx;
  FETCHcode result;
  int rv;
  const struct Fetch_sockaddr_ex *peer_addr = NULL;
  BIO *bio = NULL;
  BIO_ADDR *baddr = NULL;
...
#define H3_ALPN "\x2h3"
  ...
  result = FETCHE_QUIC_CONNECT_ERROR;
  Fetch_cf_socket_peek(cf->next, data, &ctx->q.sockfd, &peer_addr, NULL);
  if (!peer_addr)
    goto out;
  ...
  if (!SSL_set_feature_request_uint(ctx->tls.ossl.ssl,
                                    ...
                                    FETCH_QUIC_MAX_IDLE_MS))
  {
    FETCH_TRC_CF(data, cf, "error setting idle timeout, ");
    result = FETCHE_FAILED_INIT;
    goto out;
  }
#endif
...
out:
  ...
  if (baddr)
    BIO_ADDR_free(baddr);
  FETCH_TRC_CF(data, cf, "QUIC tls init -> %d", result);
  return result;
}
                            ...
                            FETCHcode *err)
{
  ...
  *err = FETCHE_OK;
  ...
  if (rv <= 0)
  {
    ...
    {
      FETCH_TRC_CF(x->data, x->cf, "[%" FMT_PRId64 "] h3_quic_recv -> EOS",
                   x->s->id);
      x->s->recvd_eos = TRUE;
      return 0;
    }
             ...
             SSL_STREAM_STATE_RESET_REMOTE)
    {
      ...
      FETCH_TRC_CF(x->data, x->cf, "[%" FMT_PRId64 "] h3_quic_recv -> RESET, "
                                   "rv=%d, app_err=%" FMT_PRIu64,
                   x->s->id, rv, (fetch_uint64_t)app_error_code);
    ...
    {
      *err = cf_osslq_ssl_err(x->cf, x->data, detail, FETCHE_RECV_ERROR);
      return -1;
    }
  }
...
static FETCHcode cf_osslq_stream_recv(struct cf_osslq_stream *s,
                                      struct Fetch_cfilter *cf,
                                      struct Fetch_easy *data)
{
  struct cf_osslq_ctx *ctx = cf->ctx;
  ...
  {
    if (Fetch_bufq_is_empty(&s->recvbuf) && !s->recvd_eos)
    {
      while (!eagain && !s->recvd_eos && !Fetch_bufq_is_full(&s->recvbuf))
      {
        nread = Fetch_bufq_sipn(&s->recvbuf, 0, h3_quic_recv, &x, &result);
        if (nread < 0)
        {
    ...
    if (!Fetch_bufq_is_empty(&s->recvbuf))
    {
      const unsigned char *buf;
      size_t blen;

      while (Fetch_bufq_peek(&s->recvbuf, &buf, &blen))
      {
        nread = nghttp3_conn_read_stream(ctx->h3.conn, s->id,
                                         buf, blen, 0);
        FETCH_TRC_CF(data, cf, "[%" FMT_PRId64 "] forward %zu bytes "
                               "to nghttp3 -> %zd",
                     s->id, blen, nread);
        if (nread < 0)
        {
        ...
        Fetch_bufq_skip(&s->recvbuf, blen);
        total_recv_len += blen;
      }
    }

    /* When we forwarded everything, handle RESET/EOS */
    if (Fetch_bufq_is_empty(&s->recvbuf) && !s->closed)
    {
      int rv;
      result = FETCHE_OK;
      if (s->reset)
      ...
      else if (s->recvd_eos)
      {
        rv = nghttp3_conn_close_stream(ctx->h3.conn, s->id,
                                       NGHTTP3_H3_NO_ERROR);
        s->closed = TRUE;
        FETCH_TRC_CF(data, cf, "[%" FMT_PRId64 "] close nghttp3 stream -> %d",
                     s->id, rv);
        if (rv < 0 && rv != NGHTTP3_ERR_STREAM_NOT_FOUND)
        {
          failf(data, "nghttp3_conn_close_stream returned error: %s",
...
out:
  if (result)
    FETCH_TRC_CF(data, cf, "[%" FMT_PRId64 "] cf_osslq_stream_recv -> %d",
                 s->id, result);
  return result;
}
                                     ...
                                     struct Fetch_easy *data)
{
  ...
  if (!SSL_handle_events(ctx->tls.ossl.ssl))
  {
    ...
    result = cf_osslq_ssl_err(cf, data, detail, FETCHE_RECV_ERROR);
  }
  ...
  if (ctx->h3.conn)
  {
    ...
    {
      struct Fetch_easy *sdata = Fetch_node_elem(e);
      if (sdata->conn == data->conn && FETCH_WANT_RECV(sdata))
      {
        stream = H3_STREAM_CTX(ctx, sdata);
        if (stream && !stream->closed &&
            !Fetch_bufq_is_full(&stream->recvbuf))
        {
...
out:
  FETCH_TRC_CF(data, cf, "progress_ingress -> %d", result);
  return result;
}

/* Iterate over all streams and check if blocked can be unblocked */
static FETCHcode cf_osslq_check_and_unblock(struct Fetch_cfilter *cf,
                                            struct Fetch_easy *data)
{
  struct cf_osslq_ctx *ctx = cf->ctx;
  struct h3_stream_ctx *stream;
  ...
  if (ctx->h3.conn)
  {
    ...
    if (ctx->item_count < Fetch_llist_count(&data->multi->process))
    {
      ctx->item_count = 0;
      tmpptr = realloc(ctx->poll_items,
                       Fetch_llist_count(&data->multi->process) *
      ...
      tmpptr = realloc(ctx->fetch_items,
                       Fetch_llist_count(&data->multi->process) *
      ...
      ctx->fetch_items = tmpptr;

      ctx->item_count = Fetch_llist_count(&data->multi->process);
    }
    ...
    for (e = Fetch_llist_head(&data->multi->process); e; e = Fetch_node_next(e))
    {
      ...
      if (sdata->conn == data->conn)
      {
        ...
        if (stream && stream->s.ssl && stream->s.send_blocked)
        {
          ...
          ctx->fetch_items[poll_count] = sdata;
          poll_count++;
        }
      }
    }
         ...
         idx_count++)
    {
      ...
      {
        stream = H3_STREAM_CTX(ctx, ctx->fetch_items[idx_count]);
        nghttp3_conn_unblock_stream(ctx->h3.conn, stream->s.id);
        stream->s.send_blocked = FALSE;
        h3_drain_stream(cf, ctx->fetch_items[idx_count]);
        FETCH_TRC_CF(ctx->fetch_items[idx_count], cf, "unblocked");
        result_count--;
      }
    }
  }
...
static FETCHcode h3_send_streams(struct Fetch_cfilter *cf,
                                 struct Fetch_easy *data)
{
  struct cf_osslq_ctx *ctx = cf->ctx;
  FETCHcode result = FETCHE_OK;
  ...
  for (;;)
  {
    ...
    if (!s)
    {
      failf(data, "nghttp3_conn_writev_stream gave unknown stream %" FMT_PRId64, (fetch_int64_t)stream_id);
      result = FETCHE_SEND_ERROR;
      goto out;
    }
    ...
    for (i = 0; (i < n) && !blocked; ++i)
    {
      ...
      if (ok)
      {
        ...
        FETCH_TRC_CF(data, cf, "[%" FMT_PRId64 "] send %zu bytes to QUIC ok",
                     s->id, vec[i].len);
        acked_len += vec[i].len;
      }
      else
      {
        ...
        case SSL_ERROR_WANT_READ:
          ...
          FETCH_TRC_CF(data, cf, "[%" FMT_PRId64 "] send %zu bytes to "
                                 "QUIC blocked",
                       s->id, vec[i].len);
          written = 0;
        ...
        default:
          ...
          result = cf_osslq_ssl_err(cf, data, detail, FETCHE_HTTP3);
          goto out;
        }
      }
    }
    ...
    if (acked_len > 0 || (eos && !s->send_blocked))
    {
      ...
      FETCH_TRC_CF(data, cf, "[%" FMT_PRId64 "] forwarded %zu/%zu h3 bytes "
                             "to QUIC, eos=%d",
    ...
    if (eos && !s->send_blocked && !eos_written)
    {
      ...
      FETCH_TRC_CF(data, cf, "[%" FMT_PRId64 "] closing QUIC stream", s->id);
      SSL_stream_conclude(s->ssl, 0);
    }
  }

out:
  FETCH_TRC_CF(data, cf, "h3_send_streams -> %d", result);
  return result;
}

static FETCHcode cf_progress_egress(struct Fetch_cfilter *cf,
                                    struct Fetch_easy *data)
{
  struct cf_osslq_ctx *ctx = cf->ctx;
  FETCHcode result = FETCHE_OK;
  ...
  {
    int detail = SSL_get_error(ctx->tls.ossl.ssl, 0);
    result = cf_osslq_ssl_err(cf, data, detail, FETCHE_SEND_ERROR);
  }

  result = cf_osslq_check_and_unblock(cf, data);

out:
  FETCH_TRC_CF(data, cf, "progress_egress -> %d", result);
  return result;
}

static FETCHcode check_and_set_expiry(struct Fetch_cfilter *cf,
                                      struct Fetch_easy *data)
{
  struct cf_osslq_ctx *ctx = cf->ctx;
  FETCHcode result = FETCHE_OK;
      ...
      !is_infinite)
  {
    ...
    if (!is_infinite)
    {
      Fetch_expire(data, timeoutms, EXPIRE_QUIC);
      FETCH_TRC_CF(data, cf, "QUIC expiry in %ldms", (long)timeoutms);
    }
  }
out:
  return result;
...
static FETCHcode cf_osslq_connect(struct Fetch_cfilter *cf,
                                  struct Fetch_easy *data,
                                  bool blocking, bool *done)
{
  ...
  *done = FALSE;
  ...
  if (!ctx->got_first_byte)
  {
    ...
    if (readable > 0 && (readable & FETCH_CSELECT_IN))
    {
  ...
  if (err == 1)
  {
    ...
    FETCH_TRC_CF(data, cf, "handshake complete after %dms",
                 (int)Fetch_timediff(now, ctx->started_at));
  ...
  else
  {
    ...
    case SSL_ERROR_WANT_READ:
      ctx->q.last_io = now;
      FETCH_TRC_CF(data, cf, "QUIC SSL_connect() -> WANT_RECV");
      goto out;
    case SSL_ERROR_WANT_WRITE:
      ctx->q.last_io = now;
      FETCH_TRC_CF(data, cf, "QUIC SSL_connect() -> WANT_SEND");
      result = FETCHE_OK;
      goto out;
#ifdef SSL_ERROR_WANT_ASYNC
    case SSL_ERROR_WANT_ASYNC:
      ctx->q.last_io = now;
      FETCH_TRC_CF(data, cf, "QUIC SSL_connect() -> WANT_ASYNC");
      result = FETCHE_OK;
      goto out;
#endif
...
#endif
    default:
      result = cf_osslq_ssl_err(cf, data, detail, FETCHE_COULDNT_CONNECT);
      goto out;
    }
  }

out:
  if (result == FETCHE_RECV_ERROR && ctx->tls.ossl.ssl &&
  ...
  if (result || *done)
    FETCH_TRC_CF(data, cf, "connect -> %d, done=%d", result, *done);
  CF_DATA_RESTORE(cf, save);
  return result;
}

static ssize_t h3_stream_open(struct Fetch_cfilter *cf,
                              struct Fetch_easy *data,
  ...
  *err = cf_osslq_stream_open(&stream->s, ctx->tls.ossl.ssl, 0,
  ...
  if (rc)
  {
    ...
    case NGHTTP3_ERR_CONN_CLOSING:
      FETCH_TRC_CF(data, cf, "h3sid[%" FMT_PRId64 "] failed to send, "
                             "connection is closing",
                   stream->s.id);
      break;
    default:
      FETCH_TRC_CF(data, cf, "h3sid[%" FMT_PRId64 "] failed to send -> %d (%s)",
...
static ssize_t cf_osslq_send(struct Fetch_cfilter *cf, struct Fetch_easy *data,
                             ...
                             FETCHcode *err)
{
  ...
  *err = FETCHE_OK;
  ...
  if (!stream || stream->s.id < 0)
  {
    ...
    if (nwritten < 0)
    {
      FETCH_TRC_CF(data, cf, "failed to open stream -> %d", *err);
      goto out;
    }
    stream = H3_STREAM_CTX(ctx, data);
  }
  else if (stream->closed)
  {
    if (stream->resp_hds_complete)
    {
       ...
       * error situation. */
      FETCH_TRC_CF(data, cf, "[%" FMT_PRId64 "] discarding data"
    ...
    FETCH_TRC_CF(data, cf, "[%" FMT_PRId64 "] send_body(len=%zu) "
                           "-> stream closed",
                 stream->s.id, len);
    *err = FETCHE_HTTP3;
    nwritten = -1;
  ...
  {
    nwritten = Fetch_bufq_write(&stream->sendbuf, buf, len, err);
    FETCH_TRC_CF(data, cf, "[%" FMT_PRId64 "] cf_send, add to "
                           "sendbuf(len=%zu) -> %zd, %d",
...
out:
  result = check_and_set_expiry(cf, data);
  FETCH_TRC_CF(data, cf, "[%" FMT_PRId64 "] cf_send(len=%zu) -> %zd, %d",
...
static ssize_t cf_osslq_recv(struct Fetch_cfilter *cf, struct Fetch_easy *data,
                             char *buf, size_t len, FETCHcode *err)
{
  ...
  *err = FETCHE_OK;
  ...
  if (!Fetch_bufq_is_empty(&stream->recvbuf))
  {
    ...
    if (nread < 0)
    {
      FETCH_TRC_CF(data, cf, "[%" FMT_PRId64 "] read recvbuf(len=%zu) "
                             "-> %zd, %d",
...
out:
  ...
  {
    FETCHcode result2 = check_and_set_expiry(cf, data);
    if (result2)
    {
      *err = result2;
      nread = -1;
  ...
  FETCH_TRC_CF(data, cf, "[%" FMT_PRId64 "] cf_recv(len=%zu) -> %zd, %d",
 ...
 */
static bool cf_osslq_data_pending(struct Fetch_cfilter *cf,
...
static FETCHcode cf_osslq_data_event(struct Fetch_cfilter *cf,
                                     struct Fetch_easy *data,
                                     int event, int arg1, void *arg2)
{
  ...
  case CF_CTRL_DATA_DONE_SEND:
  {
    ...
    {
      stream->send_closed = TRUE;
      stream->upload_left = Fetch_bufq_len(&stream->sendbuf) -
                            stream->sendbuf_len_in_flight;
      (void)nghttp3_conn_resume_stream(ctx->h3.conn, stream->s.id);
    }
    break;
  ...
  {
    struct h3_stream_ctx *stream = H3_STREAM_CTX(ctx, data);
    FETCH_TRC_CF(data, cf, "data idle");
    if (stream && !stream->closed)
    {
      result = check_and_set_expiry(cf, data);
    }
...
static bool cf_osslq_conn_is_alive(struct Fetch_cfilter *cf,
                                   struct Fetch_easy *data,
                                   bool *input_pending)
{
  struct cf_osslq_ctx *ctx = cf->ctx;
  ...
  *input_pending = FALSE;
  ...
  /* Added in OpenSSL v3.3.x */
  {
    ...
    {
      FETCH_TRC_CF(data, cf, "error getting negotiated idle timeout, "
                             "assume connection is dead.");
      goto out;
    }
    FETCH_TRC_CF(data, cf, "negotiated idle timeout: %zums", (size_t)idle_ms);
    idletime = Fetch_timediff(Fetch_now(), ctx->q.last_io);
  ...
  if (*input_pending)
  {
    ...
    *input_pending = FALSE;
    result = cf_progress_ingress(cf, data);
    FETCH_TRC_CF(data, cf, "is_alive, progress ingress -> %d", result);
...
static void cf_osslq_adjust_pollset(struct Fetch_cfilter *cf,
                                    struct Fetch_easy *data,
                                    struct easy_pollset *ps)
{
  ...
  else if (!cf->connected)
  {
    ...
    Fetch_pollset_set(data, ps, ctx->q.sockfd,
...
static FETCHcode cf_osslq_query(struct Fetch_cfilter *cf,
                                struct Fetch_easy *data,
                                int query, int *pres1, void *pres2)
{
  ...
  case CF_QUERY_MAX_CONCURRENT:
  {
    ...
    {
      FETCH_TRC_CF(data, cf, "error getting available local bidi streams");
      return FETCHE_HTTP3;
    }
    ...
    *pres1 = 100;
#endif
    FETCH_TRC_CF(data, cf, "query max_conncurrent -> %d", *pres1);
    return FETCHE_OK;
  }
...
FETCHcode Fetch_cf_osslq_create(struct Fetch_cfilter **pcf,
                               struct Fetch_easy *data,
...
bool Fetch_conn_is_osslq(const struct Fetch_easy *data,
 ...
 */
void Fetch_osslq_ver(char *p, size_t len)
{
