                               ...
                               void *cb_user_data)
{
  ...
  if (wolfSSL_CTX_set_cipher_list(ctx->wssl.ctx, conn_config->cipher_list13 ? conn_config->cipher_list13 : QUIC_CIPHERS) != 1)
  {
  ...
  if (wolfSSL_CTX_set1_groups_list(ctx->wssl.ctx, conn_config->curves ? conn_config->curves : (char *)QUIC_GROUPS) != 1)
  {
                                     ...
                                     struct ssl_peer *peer)
{
  ...
  if (conn_config->verifyhost)
  {
    if (peer->sni)
    {
      ...
      if (wolfSSL_X509_check_host(cert, peer->sni, strlen(peer->sni), 0, NULL) == WOLFSSL_FAILURE)
      {
