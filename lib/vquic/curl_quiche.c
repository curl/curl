                                ...
                                struct Curl_easy *data)
{
  ...
  if((data->req.keepon & KEEP_SENDBITS) == KEEP_SEND) {
    ...
    if(H3_STREAM_ID(data) >= 0)
      CURL_LOG_CF(data, cf, "[h3sid=%"PRId64"] suspend sending",
                  H3_STREAM_ID(data));
    else
      CURL_LOG_CF(data, cf, "[%s] suspend sending", data->state.url);
  }
}
                               ...
                               struct Curl_easy *data)
{
  ...
  if(stream_send_is_suspended(data)) {
    ...
    --ctx->sends_on_hold;
    if(H3_STREAM_ID(data) >= 0)
      CURL_LOG_CF(data, cf, "[h3sid=%"PRId64"] resume sending",
                  H3_STREAM_ID(data));
    else
      CURL_LOG_CF(data, cf, "[%s] resume sending", data->state.url);
                              ...
                              struct Curl_easy *data)
{
  ...
  CURL_LOG_CF(data, cf, "data setup");
  return CURLE_OK;
}
...
static void h3_data_done(struct Curl_cfilter *cf, struct Curl_easy *data)
{
  ...
  (void)cf;
  if(stream) {
    CURL_LOG_CF(data, cf, "[h3sid=%"PRId64"] easy handle is done", stream->id);
    if(stream_send_is_suspended(data)) {
      data->req.keepon &= ~KEEP_SEND_HOLD;
      --ctx->sends_on_hold;
    }
                             ...
                             struct Curl_easy *data)
{
  ...
  if(nwritten < 0 && result != CURLE_AGAIN) {
    CURL_LOG_CF(data, cf, "[h3sid=%"PRId64"] recv_body error %zd",
                stream->id, nwritten);
                                 ...
                                 quiche_h3_event *ev)
{
  ...
  case QUICHE_H3_EVENT_HEADERS:
    ...
    CURL_LOG_CF(data, cf, "[h3sid=%"PRId64"][HEADERS]", stream3_id);
    break;
  ...
  case QUICHE_H3_EVENT_RESET:
    CURL_LOG_CF(data, cf, "[h3sid=%"PRId64"][RESET]", stream3_id);
    stream->closed = TRUE;
    stream->reset = TRUE;
    stream->send_closed = TRUE;
    streamclose(cf->conn, "Reset of stream");
  ...
  case QUICHE_H3_EVENT_FINISHED:
    CURL_LOG_CF(data, cf, "[h3sid=%"PRId64"][FINISHED]", stream3_id);
    if(!stream->resp_hds_complete) {
      result = write_resp_raw(cf, data, "\r\n", 2);
      if(result)
        return result;
  ...
  case QUICHE_H3_EVENT_GOAWAY:
    CURL_LOG_CF(data, cf, "[h3sid=%"PRId64"][GOAWAY]", stream3_id);
    break;

  default:
    CURL_LOG_CF(data, cf, "[h3sid=%"PRId64"] recv, unhandled event %d",
                               ...
                               struct Curl_easy *data)
{
  ...
  while(ctx->h3c) {
    ...
    if(stream3_id == QUICHE_H3_ERR_DONE) {
      ...
      CURL_LOG_CF(data, cf, "[h3sid=%"PRId64"] error poll: %"PRId64,
    ...
    if(!sdata) {
      CURL_LOG_CF(data, cf, "[h3sid=%"PRId64"] discard event %s for "
      ...
      if(result) {
        CURL_LOG_CF(data, cf, "[h3sid=%"PRId64"] error processing event %s "
                    "for [h3sid=%"PRId64"] -> %d",
                         ...
                         void *userp)
{
  ...
  if(nread < 0) {
    if(QUICHE_ERR_DONE == nread) {
      CURL_LOG_CF(r->data, r->cf, "ingress, quiche is DONE");
      return CURLE_OK;
    }
    ...
    CURL_LOG_CF(r->data, r->cf, "ingress, quiche only read %zd/%zu bytes",
                nread, pktlen);
  }
                                  ...
                                  CURLcode *err)
{
  ...
  if(stream->reset) {
    ...
    *err = stream->resp_got_header? CURLE_PARTIAL_FILE : CURLE_RECV_ERROR;
    CURL_LOG_CF(data, cf, "[h3sid=%" PRId64 "] cf_recv, was reset -> %d",
                stream->id, *err);
  }
    ...
    *err = CURLE_RECV_ERROR;
    CURL_LOG_CF(data, cf, "[h3sid=%" PRId64 "] cf_recv, closed incomplete"
                " -> %d", stream->id, *err);
  }
  else {
    *err = CURLE_OK;
    nread = 0;
    CURL_LOG_CF(data, cf, "[h3sid=%" PRId64 "] cf_recv, closed ok"
                " -> %d", stream->id, *err);
  }
  return nread;
}
                              ...
                              char *buf, size_t len, CURLcode *err)
{
  ...
  if(!Curl_bufq_is_empty(&stream->recvbuf)) {
    ...
    CURL_LOG_CF(data, cf, "[h3sid=%" PRId64 "] read recvbuf(len=%zu) "
  ...
  if(cf_process_ingress(cf, data)) {
    CURL_LOG_CF(data, cf, "cf_recv, error on ingress");
    *err = CURLE_RECV_ERROR;
    nread = -1;
    goto out;
  }
...
out:
  ...
  if(result) {
    CURL_LOG_CF(data, cf, "cf_recv, flush egress failed");
    *err = result;
    nread = -1;
  }
  if(nread > 0)
    ctx->data_recvd += nread;
  CURL_LOG_CF(LOG_CF(data, cf, "[h3sid=%"PRId64"] cf_recv(total=%"
                              ...
                              CURLcode *err)
{
  ...
  *err = Curl_http_req_to_h2(&h2_headers, h1.req, data);
  ...
  if(stream3_id < 0) {
    if(QUICHE_H3_ERR_STREAM_BLOCKED == stream3_id) {
      ...
      CURL_LOG_CF(data, cf, "send_request(%s) rejected with BLOCKED",
                  data->state.url);
      ...
      *err = CURLE_AGAIN;
      ...
      CURL_LOG_CF(data, cf, "send_request(%s) -> %" PRId64,
  ...
  *err = CURLE_OK;
  ...
  CURL_LOG_CF(data, cf, "[h3sid=%" PRId64 "] opened for %s",
  ...
  if(!stream || stream->id < 0) {
    ...
    if(nwritten == QUICHE_H3_ERR_DONE || (nwritten == 0 && len > 0)) {
      ...
      if(!quiche_conn_stream_writable(ctx->qconn, stream->id, len)) {
        CURL_LOG_CF(data, cf, "[h3sid=%" PRId64 "] send_body(len=%zu) "
      ...
      *err = CURLE_SEND_ERROR;
      ...
      CURL_LOG_CF(data, cf, "[h3sid=%" PRId64 "] send body(len=%zu, "
...
out:
  ...
  CURL_LOG_CF(data, cf, "[h3sid=%" PRId64 "] cf_send(len=%zu) -> %zd, %d",
                                     ...
                                     int event, int arg1, void *arg2)
{
  ...
  case CF_CTRL_DATA_DONE_SEND: {
    ...
    if(stream && !stream->send_closed) {
      ...
      CURL_LOG_CF(data, cf, "[h3sid=%"PRId64"] DONE_SEND -> %zd, %d",
  ...
  case CF_CTRL_DATA_IDLE:
    result = cf_flush_egress(cf, data);
    if(result)
      CURL_LOG_CF(data, cf, "data idle, flush egress -> %d", result);
    break;
  default:
    break;
  }
                               ...
                               struct Curl_easy *data)
{
  ...
  if(cf->conn->ssl_config.verifyhost) {
    ...
    CURL_LOG_CF(data, cf, "Skipped certificate verification");
                                 ...
                                 struct Curl_easy *data)
{
...

  {
    ...
    CURL_LOG_CF(data, cf, "Sent QUIC client Initial, ALPN: %s",
                alpn_protocols + 1);
  }
                                  ...
                                  bool blocking, bool *done)
{
  ...
  *done = FALSE;
  ...
  if(ctx->reconnect_at.tv_sec && Curl_timediff(now, ctx->reconnect_at) < 0) {
    ...
    CURL_LOG_CF(data, cf, "waiting for reconnect time");
    goto out;
  }
  ...
  if(quiche_conn_is_established(ctx->qconn)) {
    CURL_LOG_CF(data, cf, "handshake complete after %dms",
                (int)Curl_timediff(now, ctx->started_at));
    ctx->handshake_at = now;
    result = cf_verify_peer(cf, data);
    if(!result) {
      CURL_LOG_CF(data, cf, "peer verified");
      cf->connected = TRUE;
    ...
    CURL_LOG_CF(data, cf, "connect, remote closed, reconnect after %dms",
                reconn_delay_ms);
                                ...
                                int query, int *pres1, void *pres2)
{
  ...
  case CF_QUERY_MAX_CONCURRENT: {
    ...
    *pres1 = (max_streams > INT_MAX)? INT_MAX : (int)max_streams;
    CURL_LOG_CF(data, cf, "query: MAX_CONCURRENT -> %d", *pres1);
    return CURLE_OK;
  }
