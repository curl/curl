                                 ...
                                 quiche_h3_event *ev)
{
  ...
  case QUICHE_H3_EVENT_HEADERS:
    ...
    CURL_TRC_CF(data, cf, "[%"PRId64"] <- [HEADERS]", stream->id);
    break;
  ...
  case QUICHE_H3_EVENT_RESET:
    CURL_TRC_CF(data, cf, "[%"PRId64"] RESET", stream->id);
    stream->closed = TRUE;
    stream->reset = TRUE;
    stream->send_closed = TRUE;
    streamclose(cf->conn, "Reset of stream");
  ...
  case QUICHE_H3_EVENT_GOAWAY:
    CURL_TRC_CF(data, cf, "[%"PRId64"] <- [GOAWAY]", stream->id);
    break;
                               ...
                               struct Curl_easy *data)
{
  ...
  while(ctx->h3c) {
    ...
    if(stream3_id == QUICHE_H3_ERR_DONE) {
      ...
      CURL_TRC_CF(data, cf, "error poll: %"PRId64, stream3_id);
      return CURLE_HTTP3;
    }

    sdata = get_stream_easy(cf, data, stream3_id, &stream);
    if(!sdata || !stream) {
      CURL_TRC_CF(data, cf, "discard event %s for unknown [%"PRId64"]",
