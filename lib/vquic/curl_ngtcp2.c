...
static int cf_ngtcp2_handshake_completed(ngtcp2_conn *tconn, void *user_data)
{
  ...
  data->state.now = ctx->handshake_at = curlx_now(); /* real change */
                                 ...
                                 struct h3_stream_ctx *stream)
{
  ...
  avail = Curl_rlimit_avail(&data->progress.dl.rlimit, &data->state.now);
  ...
  *input_pending = FALSE;
  ...
  if(rp && rp->max_idle_timeout) {
    timediff_t idletime_ms =
      curlx_timediff_ms(data->state.now, ctx->q.last_io);
    if(idletime_ms > 0) {
