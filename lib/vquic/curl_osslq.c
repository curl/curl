                                 ...
                                 bool *done)
{
  ...
  *done = FALSE;
  ...
  if(err == 1) {
    ...
    ctx->handshake_at = data->state.now;
    ctx->q.last_io = data->state.now;
    CURL_TRC_CF(data, cf, "handshake complete after %" FMT_TIMEDIFF_T "ms",
                curlx_timediff_ms(data->state.now, ctx->started_at));
                                   ...
                                   bool *input_pending)
{
  ...
  *input_pending = FALSE;
  ...
  /* Added in OpenSSL v3.3.x */
  {
    ...
    idletime = curlx_timediff_ms(data->state.now, ctx->q.last_io);
