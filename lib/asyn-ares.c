                                ...
                                struct Curl_dns_entry **dns)
{
  ...
  *dns = NULL;
  ...
  if(ares->num_pending &&
     ...
     (curlx_timediff_ms(data->state.now, ares->happy_eyeballs_dns_time) >=
  ...
  *entry = NULL; /* clear on entry */
  ...
  while(!result) {
    ...
    if(Curl_pgrsUpdate(data))
      ...
      timediff_t elapsed_ms = curlx_timediff_ms(now, data->state.now);
      if(elapsed_ms <= 0)
      ...
      data->state.now = now; /* for next loop */
    }
    if(timeout_ms < 0)
                                     ...
                                     struct hostent *hostent)
{
  ...
  if(ares->num_pending
    ...
    */
    ares->happy_eyeballs_dns_time = data->state.now;
