 ...
 * breaking compatibility, so it is better not to override the defaults in fetch
 */
...
static FETCHcode ossl_certchain(struct Fetch_easy *data, SSL *ssl);

static FETCHcode push_certinfo(struct Fetch_easy *data,
                               ...
                               BIO *mem, const char *label, int num)
{
  ...
  FETCHcode result = Fetch_ssl_push_certinfo_len(data, num, label, ptr, len);
  (void)BIO_reset(mem);
  return result;
}

static FETCHcode pubkey_show(struct Fetch_easy *data,
                             BIO *mem,
                             int num,
                             const char *type,
                             const char *name,
...
static FETCHcode X509V3_ext(struct Fetch_easy *data,
                            int certnum,
                            CONST_EXTS STACK_OF(X509_EXTENSION) * exts)
{
  ...
  for (i = 0; i < (int)sk_X509_EXTENSION_num(exts); i++)
  {
    ...
    result = Fetch_ssl_push_certinfo_len(data, certnum, namebuf, biomem->data,
                                        biomem->length);
    BIO_free(bio_out);
    if (result)
      break;
...
static FETCHcode ossl_certchain(struct Fetch_easy *data, SSL *ssl)
{
  ...
  result = Fetch_ssl_init_certinfo(data, (int)numcerts);
  if (result)
    return result;
...
{
  struct Fetch_cfilter *cf = BIO_get_data(bio);
  long ret = 1;
...
static int ossl_bio_cf_out_write(BIO *bio, const char *buf, int blen)
{
  ...
  ssize_t nwritten;
  FETCHcode result = FETCHE_SEND_ERROR;

  DEBUGASSERT(data);
  if (blen < 0)
    return 0;

  nwritten = Fetch_conn_cf_send(cf->next, data, buf, (size_t)blen, FALSE,
                               &result);
  FETCH_TRC_CF(data, cf, "ossl_bio_cf_out_write(len=%d) -> %d, err=%d",
...
static int ossl_bio_cf_in_read(BIO *bio, char *buf, int blen)
{
  ...
  struct Fetch_easy *data = CF_DATA_CURRENT(cf);
  ssize_t nread;
  FETCHcode result = FETCHE_RECV_ERROR;
  ...
  nread = Fetch_conn_cf_recv(cf->next, data, buf, (size_t)blen, &result);
  FETCH_TRC_CF(data, cf, "ossl_bio_cf_in_read(len=%d) -> %d, err=%d",
  ...
  {
    result = Fetch_ssl_setup_x509_store(cf, data, octx->ssl_ctx);
    if (result)
    {
      octx->io_result = result;
      return -1;
...
#define ossl_bio_cf_method_free(m) Fetch_nop_stmt
...
#else
/*
 * ossl_log_tls12_secret is called by libfetch to make the CLIENT_RANDOMs if the
...
ossl_log_tls12_secret(const SSL *ssl, bool *keylog_done)
{
   ...
   * session (when fetch was built with older OpenSSL headers and running with
   * newer OpenSSL runtime libraries). */
  if (master_key_length <= 0)
    return;

  *keylog_done = TRUE;
  Fetch_tls_keylog_write("CLIENT_RANDOM", client_random,
...
static char *ossl_strerror(unsigned long error, char *buf, size_t size)
{
  ...
  *buf = '\0';
  ...
  len = Fetch_ossl_version(buf, size);
                           ...
                           void *global_passwd)
{
  ...
  if (!encrypting && num >= 0)
  {
    int klen = fetchx_uztosi(strlen((char *)global_passwd));
    if (num > klen)
    {
...
static FETCHcode ossl_seed(struct Fetch_easy *data)
{
  ...
  do
  {
    ...
    size_t i, i_max;
    for (i = 0, i_max = len / sizeof(struct fetchtime); i < i_max; ++i)
    {
      struct fetchtime tv = Fetch_now();
      Fetch_wait_ms(1);
      tv.tv_sec *= (time_t)i + 1;
      tv.tv_usec *= (int)i + 2;
      tv.tv_sec ^= ((Fetch_now().tv_sec + (time_t)Fetch_now().tv_usec) *
                    (time_t)(i + 3))
                   << 8;
      tv.tv_usec ^= (int)((Fetch_now().tv_sec + (time_t)Fetch_now().tv_usec) *
                          (time_t)(i + 4))
                    << 16;
      memcpy(&randb[i * sizeof(struct fetchtime)], &tv,
             sizeof(struct fetchtime));
    }
  ...
  infof(data, "libfetch is now using a weak random seed");
  return rand_enough() ? FETCHE_OK : FETCHE_SSL_CONNECT_ERROR; /* confusing error code */
#endif
}
...
static FETCHcode ossl_set_engine(struct Fetch_easy *data, const char *engine);
#if !defined(USE_OPENSSL_ENGINE) && defined(OPENSSL_HAS_PROVIDERS)
static FETCHcode ossl_set_provider(struct Fetch_easy *data,
                                   const char *provider);
#endif

static int use_certificate_blob(SSL_CTX *ctx, const struct fetch_blob *blob,
                                int type, const char *key_passwd)
{
  int ret = 0;
  X509 *x = NULL;
  /* the typecast of blob->len is fine since it is guaranteed to never be
     larger than FETCH_MAX_INPUT_LENGTH */
...
static int use_privatekey_blob(SSL_CTX *ctx, const struct fetch_blob *blob,
                               int type, const char *key_passwd)
{
  int ret = 0;
  EVP_PKEY *pkey = NULL;
...
static int
use_certificate_chain_blob(SSL_CTX *ctx, const struct fetch_blob *blob,
...
static int cert_stuff(struct Fetch_easy *data,
                      SSL_CTX *ctx,
                      char *cert_file,
                      const struct fetch_blob *cert_blob,
                      const char *cert_type,
                      char *key_file,
                      const struct fetch_blob *key_blob,
                      const char *key_type,
                      char *key_passwd)
{
      ...
      (file_type == SSL_FILETYPE_PROVIDER))
  {
    ...
    case SSL_FILETYPE_PEM:
      ...
      cert_use_result = cert_blob ? use_certificate_chain_blob(ctx, cert_blob, key_passwd) : SSL_CTX_use_certificate_chain_file(ctx, cert_file);
      if (cert_use_result != 1)
      {
        failf(data,
              ...
              (cert_blob ? "FETCHOPT_SSLCERT_BLOB" : cert_file),
    ...
    case SSL_FILETYPE_ASN1:
      ...
      cert_use_result = cert_blob ? use_certificate_blob(ctx, cert_blob, file_type, key_passwd) : SSL_CTX_use_certificate_file(ctx, cert_file, file_type);
      if (cert_use_result != 1)
      {
        failf(data,
...
#if defined(USE_OPENSSL_ENGINE) && defined(ENGINE_CTRL_GET_CMD_FROM_NAME)
    {
      ...
      if (!data->state.engine)
      {
        ...
        {
          if (ossl_set_engine(data, "pkcs11") != FETCHE_OK)
          {
            return 0;
          }
        }
    ...
    case SSL_FILETYPE_PROVIDER:
    {
      ...
      if (!data->state.provider)
      {
        if (is_pkcs11_uri(cert_file))
        {
          if (ossl_set_provider(data, "pkcs11") != FETCHE_OK)
          {
            return 0;
          }
        }
    ...
    case SSL_FILETYPE_PEM:
      ...
      FALLTHROUGH();
    case SSL_FILETYPE_ASN1:
      cert_use_result = key_blob ? use_privatekey_blob(ctx, key_blob, file_type, key_passwd) : SSL_CTX_use_PrivateKey_file(ctx, key_file, file_type);
      if (cert_use_result != 1)
      {
...
#ifdef USE_OPENSSL_ENGINE
    {
      ...
      {
        UI_METHOD *ui_method =
            UI_create_method((char *)"fetch user interface");
        if (!ui_method)
        {
...
static FETCHcode x509_name_oneline(X509_NAME *a, struct dynbuf *d)
{
  BIO *bio_out = BIO_new(BIO_s_mem());
  BUF_MEM *biomem;
  int rc;
  ...
  if (bio_out)
  {
    ...
    {
      BIO_get_mem_ptr(bio_out, &biomem);
      result = Fetch_dyn_addn(d, biomem->data, biomem->length);
      BIO_free(bio_out);
    }
  }
  return result;
...
#endif
#ifdef FETCH_DISABLE_OPENSSL_AUTO_LOAD_CONFIG
...
#ifndef FETCH_DISABLE_OPENSSL_AUTO_LOAD_CONFIG
...
static FETCHcode ossl_set_engine(struct Fetch_easy *data, const char *engine)
{
  ...
  if (!e)
  {
    ...
    return FETCHE_SSL_ENGINE_NOTFOUND;
  }
  ...
  if (!ENGINE_init(e))
  {
    ...
    return FETCHE_SSL_ENGINE_INITFAILED;
  }
  data->state.engine = e;
  return FETCHE_OK;
#else
 ...
 */
static FETCHcode ossl_set_engine_default(struct Fetch_easy *data)
{
#ifdef USE_OPENSSL_ENGINE
  if (data->state.engine)
  {
    ...
    else
    {
      failf(data, "set default crypto engine '%s' failed",
            ENGINE_get_id(data->state.engine));
      return FETCHE_SSL_ENGINE_SETFAILED;
    }
  }
#else
  (void)data;
 ...
 */
static struct fetch_slist *ossl_engines_list(struct Fetch_easy *data)
{
  ...
  for (e = ENGINE_get_first(); e; e = ENGINE_get_next(e))
  {
    beg = fetch_slist_append(list, ENGINE_get_id(e));
    if (!beg)
    {
 ...
 */
static FETCHcode ossl_set_provider(struct Fetch_easy *data, const char *provider)
{
...
static FETCHcode ossl_shutdown(struct Fetch_cfilter *cf,
  ...
  *done = FALSE;
  if (!(SSL_get_shutdown(octx->ssl) & SSL_SENT_SHUTDOWN))
  {
    ...
    for (i = 0; i < 10; ++i)
    {
      ...
      FETCH_TRC_CF(data, cf, "SSL shutdown not sent, read -> %d", nread);
      if (nread <= 0)
        break;
    }
    ...
    if (!nread && err == SSL_ERROR_ZERO_RETURN)
    {
      ...
      if (!send_shutdown)
      {
        FETCH_TRC_CF(data, cf, "SSL shutdown received, not sending");
        *done = TRUE;
        goto out;
      }
      else if (!cf->next->cft->is_alive(cf->next, data, &input_pending))
      {
         ...
         * send it. We are done. */
        connssl->peer_closed = TRUE;
        FETCH_TRC_CF(data, cf, "peer closed connection");
        *done = TRUE;
        goto out;
      }
    }
  ...
  {
    ERR_clear_error();
    FETCH_TRC_CF(data, cf, "send SSL close notify");
    if (SSL_shutdown(octx->ssl) == 1)
    {
      FETCH_TRC_CF(data, cf, "SSL shutdown finished");
      *done = TRUE;
      goto out;
    }
    if (SSL_ERROR_WANT_WRITE == SSL_get_error(octx->ssl, nread))
    {
      FETCH_TRC_CF(data, cf, "SSL shutdown still wants to send");
  ...
  for (i = 0; i < 10; ++i)
  {
    ERR_clear_error();
    nread = SSL_read(octx->ssl, buf, (int)sizeof(buf));
    FETCH_TRC_CF(data, cf, "SSL shutdown read -> %d", nread);
    if (nread <= 0)
      break;
  }
  err = SSL_get_error(octx->ssl, nread);
  ...
  case SSL_ERROR_ZERO_RETURN: /* no more data */
    if (SSL_shutdown(octx->ssl) == 1)
      ...
      FETCH_TRC_CF(data, cf, "SSL shutdown not received, but closed");
    *done = TRUE;
    break;
  ...
  case SSL_ERROR_WANT_READ:
    ...
    FETCH_TRC_CF(data, cf, "SSL shutdown sent, want receive");
    connssl->io_need = FETCH_SSL_IO_NEED_RECV;
    break;
  case SSL_ERROR_WANT_WRITE:
    FETCH_TRC_CF(data, cf, "SSL shutdown send blocked");
    connssl->io_need = FETCH_SSL_IO_NEED_SEND;
    break;
  default:
    ...
    FETCH_TRC_CF(data, cf, "SSL shutdown, ignore recv error: '%s', errno %d",
                 (sslerr ? ossl_strerror(sslerr, buf, sizeof(buf)) : SSL_ERROR_to_str(err)),
                 SOCKERRNO);
    *done = TRUE;
    result = FETCHE_OK;
    break;
...
static void ossl_close(struct Fetch_cfilter *cf, struct Fetch_easy *data)
{
  struct ssl_connect_data *connssl = cf->ctx;
  struct ossl_ctx *octx = (struct ossl_ctx *)connssl->backend;
 ...
 */
static void ossl_close_all(struct Fetch_easy *data)
{
#ifdef USE_OPENSSL_ENGINE
  if (data->state.engine)
  {
 ...
 */
static bool subj_alt_hostcheck(struct Fetch_easy *data,
                               ...
                               const char *dispname)
{
  ...
  (void)data;
#endif
  if (Fetch_cert_hostcheck(match_pattern, matchlen, hostname, hostlen))
  {
...
*/
static FETCHcode ossl_verifyhost(struct Fetch_easy *data,
                                 ...
                                 struct ssl_peer *peer, X509 *server_cert)
{
  ...
  switch (peer->type)
  {
  case FETCH_SSL_PEER_IPV4:
    if (!Fetch_inet_pton(AF_INET, peer->hostname, &addr))
      return FETCHE_PEER_FAILED_VERIFICATION;
    target = GEN_IPADD;
    addrlen = sizeof(struct in_addr);
    break;
#ifdef USE_IPV6
  case FETCH_SSL_PEER_IPV6:
    if (!Fetch_inet_pton(AF_INET6, peer->hostname, &addr))
      return FETCHE_PEER_FAILED_VERIFICATION;
    target = GEN_IPADD;
    addrlen = sizeof(struct in6_addr);
    break;
#endif
  case FETCH_SSL_PEER_DNS:
    target = GEN_DNS;
    break;
  default:
    DEBUGASSERT(0);
  ...
  {
    const char *tname = (peer->type == FETCH_SSL_PEER_DNS) ? "hostname" : (peer->type == FETCH_SSL_PEER_IPV4) ? "ipv4 address"
                                                                                                              : "ipv6 address";
  ...
  else
  {
    ...
    else if (!Fetch_cert_hostcheck((const char *)cn, cnlen,
                                  peer->hostname, hostlen))
    {
...
static FETCHcode verifystatus(struct Fetch_cfilter *cf,
                              struct Fetch_easy *data,
                              struct ossl_ctx *octx)
{
  ...
  if (!status)
  {
    ...
    result = FETCHE_SSL_INVALIDCERTSTATUS;
    goto end;
  }
  p = status;
                       ...
                       void *userp)
{
  ...
  )
  {
    ...
    Fetch_debug(data, FETCHINFO_TEXT, ssl_buf, (size_t)txt_len);
  }

  Fetch_debug(data, (direction == 1) ? FETCHINFO_SSL_DATA_OUT : FETCHINFO_SSL_DATA_IN, (char *)buf, len);
  (void)ssl;
}
#endif
...
static FETCHcode
ossl_set_ssl_version_min_max(struct Fetch_cfilter *cf, SSL_CTX *ctx)
{
  struct ssl_primary_config *conn_config = Fetch_ssl_cf_get_primary_config(cf);
  /* first, TLS min version... */
  long fetch_ssl_version_min = conn_config->version;
  long fetch_ssl_version_max;

  /* convert fetch min SSL version option to OpenSSL constant */
...
#endif
  switch (fetch_ssl_version_min)
  {
  case FETCH_SSLVERSION_TLSv1: /* TLS 1.x */
  ...
  /* FETCH_SSLVERSION_DEFAULT means that no option was selected.
  ...
  */
  if (fetch_ssl_version_min != FETCH_SSLVERSION_DEFAULT)
  {
  ...
  fetch_ssl_version_max = (long)conn_config->version_max;

  /* convert fetch max SSL version option to OpenSSL constant */
  switch (fetch_ssl_version_max)
  {
  ...
  case FETCH_SSLVERSION_MAX_TLSv1_3:
    ...
    break;
#endif
  case FETCH_SSLVERSION_MAX_NONE:    /* none selected */
  case FETCH_SSLVERSION_MAX_DEFAULT: /* max selected */
  default:
                                ...
                                const char *alpn)
{
  ...
  config = Fetch_ssl_cf_get_config(cf, data);
  if (config->primary.cache_session)
  {
    ...
    result = Fetch_ssl_session_create(der_session_buf, der_session_size,
                                     ietf_tls_id, alpn,
                                     (fetch_off_t)time(NULL) +
    ...
    if (!result)
    {
      result = Fetch_ssl_scache_put(cf, data, ssl_peer_key, sc_session);
...
{
  struct Fetch_cfilter *cf = (struct Fetch_cfilter *)SSL_get_app_data(ssl);
  if (cf)
  {
    struct Fetch_easy *data = CF_DATA_CURRENT(cf);
    struct ssl_connect_data *connssl = cf->ctx;
    Fetch_ossl_add_session(cf, data, connssl->peer.scache_key, ssl_sessionid,
                          SSL_version(ssl), connssl->negotiated.alpn);
  }
  return 0;
}

static FETCHcode load_cacert_from_memory(X509_STORE *store,
                                         const struct fetch_blob *ca_info_blob)
{
  ...
  return (count > 0) ? FETCHE_OK : FETCHE_SSL_CACERT_BADFILE;
}

#if defined(USE_WIN32_CRYPTO)
static FETCHcode import_windows_cert_store(struct Fetch_easy *data,
                                           const char *name,
                                           X509_STORE *store,
                                           bool *imported)
{
...
static FETCHcode ossl_populate_x509_store(struct Fetch_cfilter *cf,
                                          struct Fetch_easy *data,
                                          X509_STORE *store)
{
  struct ssl_primary_config *conn_config = Fetch_ssl_cf_get_primary_config(cf);
  ...
  FETCH_TRC_CF(data, cf, "ossl_populate_x509_store, path=%s, blob=%d",
...
static bool
ossl_cached_x509_store_expired(const struct Fetch_easy *data,
                               const struct ossl_x509_share *mb)
{
  ...
  else
  {
    ...
    timediff_t elapsed_ms = Fetch_timediff(now, mb->time);
...
static bool
ossl_cached_x509_store_different(struct Fetch_cfilter *cf,
                                 const struct ossl_x509_share *mb)
{
  struct ssl_primary_config *conn_config = Fetch_ssl_cf_get_primary_config(cf);
...
static X509_STORE *ossl_get_cached_x509_store(struct Fetch_cfilter *cf,
                                              const struct Fetch_easy *data)
{
  ...
  DEBUGASSERT(multi);
  share = multi ? Fetch_hash_pick(&multi->proto_hash,
...
static void ossl_set_cached_x509_store(struct Fetch_cfilter *cf,
                                       const struct Fetch_easy *data,
                                       X509_STORE *store)
{
  struct ssl_primary_config *conn_config = Fetch_ssl_cf_get_primary_config(cf);
                                    ...
                                    SSL_CTX *ssl_ctx)
{
  ...
  else
  {
    ...
    if (result == FETCHE_OK && cache_criteria_met)
    {
                             ...
                             void *ssl_user_data)
{
...
#endif
      break;
    case FETCH_SSLVERSION_SSLv2:
      failf(data, "No SSLv2 support");
      return FETCHE_NOT_BUILT_IN;
    case FETCH_SSLVERSION_SSLv3:
      failf(data, "No SSLv3 support");
      return FETCHE_NOT_BUILT_IN;
    default:
      failf(data, "Unrecognized parameter passed via FETCHOPT_SSLVERSION");
      return FETCHE_SSL_CONNECT_ERROR;
    }
    break;
  case TRNSPRT_QUIC:
    if (conn_config->version_max &&
        (conn_config->version_max != FETCH_SSLVERSION_MAX_TLSv1_3))
    {
  ...
  /* OpenSSL contains code to work around lots of bugs and flaws in various
     ...
     libfetch stops a connection and then resumes a TLS session. Also, sending
     the session data is some overhead. I suggest that you just use your
     proposed patch (which explicitly disables TICKET).

     If someone writes an application with libfetch and OpenSSL who wants to
  ...
  */
  ...
  case FETCH_SSLVERSION_DEFAULT:
  case FETCH_SSLVERSION_TLSv1:   /* TLS >= version 1.0 */
  case FETCH_SSLVERSION_TLSv1_0: /* TLS >= version 1.0 */
  case FETCH_SSLVERSION_TLSv1_1: /* TLS >= version 1.1 */
  case FETCH_SSLVERSION_TLSv1_2: /* TLS >= version 1.2 */
  case FETCH_SSLVERSION_TLSv1_3: /* TLS >= version 1.3 */
  ...
  if (ssl_config->primary.username && Fetch_auth_allowed_to_host(data))
  {
  ...
  if (ECH_ENABLED(data))
  {
    ...
    if (data->set.tls_ech & FETCHECH_GREASE)
    {
    ...
    else if (data->set.tls_ech & FETCHECH_CLA_CFG)
    {
#if defined(OPENSSL_IS_BORINGSSL) || defined(OPENSSL_IS_AWSLC)
      ...
      result = Fetch_base64_decode(b64, &ech_config, &ech_config_len);
      if (result || !ech_config)
      {
        infof(data, "ECH: cannot base64 decode ECHConfig from command line");
        if (data->set.tls_ech & FETCHECH_HARD)
          return result;
      }
    ...
    else
    {
      ...
      if (peer->hostname)
        dns = Fetch_fetch_addr(data, peer->hostname, peer->port);
      if (!dns)
      {
      ...
      {
        struct Fetch_https_rrinfo *rinfo = NULL;
    ...
    if (trying_ech_now && SSL_set_min_proto_version(octx->ssl, TLS1_3_VERSION) != 1)
    {
  ...
  if (ssl_config->primary.cache_session)
  {
    ...
    result = Fetch_ssl_scache_take(cf, data, peer->scache_key, &sc_session);
    ...
    Fetch_ssl_scache_return(cf, data, peer->scache_key, sc_session);
  }

  return FETCHE_OK;
}

static FETCHcode ossl_connect_step1(struct Fetch_cfilter *cf,
                                    struct Fetch_easy *data)
{
  struct ssl_connect_data *connssl = cf->ctx;
  struct ossl_ctx *octx = (struct ossl_ctx *)connssl->backend;
  ...
  result = Fetch_ossl_ctx_init(octx, cf, data, &connssl->peer,
...
static void ossl_trace_ech_retry_configs(struct Fetch_easy *data, SSL *ssl,
                                         int reason)
{
  FETCHcode result = FETCHE_OK;
  size_t rcl = 0;
  ...
  if (rv && rcs)
  {
    ...
    result = Fetch_base64_encode((const char *)rcs, rcl, &b64str, &blen);
    if (!result && b64str)
    {
...
static FETCHcode ossl_connect_step2(struct Fetch_cfilter *cf,
                                    struct Fetch_easy *data)
{
  int err;
  struct ssl_connect_data *connssl = cf->ctx;
  ...
  if (!octx->x509_store_setup)
  {
    ...
    FETCHcode result = Fetch_ssl_setup_x509_store(cf, data, octx->ssl_ctx);
    if (result)
      return result;
    octx->x509_store_setup = TRUE;
  }
   ...
   */
  if (Fetch_tls_keylog_enabled() && !octx->keylog_done)
  ...
  if (1 != err)
  {
    ...
    FETCH_TRC_CF(data, cf, "SSL_connect() -> err=%d, detail=%d", err, detail);

    if (SSL_ERROR_WANT_READ == detail)
    {
      FETCH_TRC_CF(data, cf, "SSL_connect() -> want recv");
      connssl->io_need = FETCH_SSL_IO_NEED_RECV;
      return FETCHE_OK;
    }
    if (SSL_ERROR_WANT_WRITE == detail)
    {
      FETCH_TRC_CF(data, cf, "SSL_connect() -> want send");
      connssl->io_need = FETCH_SSL_IO_NEED_SEND;
      return FETCHE_OK;
    }
    ...
    {
      FETCH_TRC_CF(data, cf, "SSL_connect() -> want async");
      connssl->io_need = FETCH_SSL_IO_NEED_RECV;
      connssl->connecting_state = ssl_connect_2;
      return FETCHE_OK;
    }
    ...
    {
      FETCH_TRC_CF(data, cf, "SSL_connect() -> want retry_verify");
      connssl->io_need = FETCH_SSL_IO_NEED_RECV;
      connssl->connecting_state = ssl_connect_2;
      return FETCHE_OK;
    }
    ...
    else
    {
      ...
      {
        result = FETCHE_SSL_CONNECT_ERROR;
       ...
       */
      if (FETCHE_SSL_CONNECT_ERROR == result && errdetail == 0)
      {
        char extramsg[80] = "";
        int sockerr = SOCKERRNO;

        if (sockerr && detail == SSL_ERROR_SYSCALL)
          Fetch_strerror(sockerr, extramsg, sizeof(extramsg));
  ...
  else
  {
    ...
    if (ECH_ENABLED(data))
    {
      ...
      if (rv != SSL_ECH_STATUS_SUCCESS && data->set.tls_ech & FETCHECH_HARD)
      {
    ...
    if (connssl->alpn)
    {
      ...
      return Fetch_alpn_set_negotiated(cf, data, connssl, neg_protocol, len);
    }
#endif
 ...
 */
static FETCHcode ossl_pkp_pin_peer_pubkey(struct Fetch_easy *data, X509 *cert,
                                          const char *pinnedpubkey)
{
  ...
  do
  {
    ...
    result = Fetch_pin_peer_pubkey(data, pinnedpubkey, buff1, len1);
  } while (0);
  ...
  return result;
    ...
    !defined(OPENSSL_IS_BORINGSSL) &&           \
    !defined(OPENSSL_IS_AWSLC) &&               \
    !defined(FETCH_DISABLE_VERBOSE_STRINGS)
static void infof_certstack(struct Fetch_easy *data, const SSL *ssl)
{
                                   ...
                                   struct ssl_peer *peer)
{
  ...
  Fetch_dyn_init(&dname, MAX_CERT_NAME_LENGTH);
  ...
  infof(data, "%s certificate:",
        Fetch_ssl_cf_is_proxy(cf) ? "Proxy" : "Server");

  result = x509_name_oneline(X509_get_subject_name(octx->server_cert),
                             &dname);
  infof(data, " subject: %s", result ? "[NONE]" : Fetch_dyn_ptr(&dname));
  ...
  {
    infof(data, " issuer: %s", Fetch_dyn_ptr(&dname));
    Fetch_dyn_free(&dname);
    ...
    if (conn_config->issuercert || conn_config->issuercert_blob)
    {
      ...
      else
      {
        ...
        if (BIO_read_filename(fp, conn_config->issuercert) <= 0)
        {
          ...
          X509_free(octx->server_cert);
          octx->server_cert = NULL;
          return FETCHE_SSL_ISSUER_ERROR;
        }
      }
  ...
  ptr = Fetch_ssl_cf_is_proxy(cf) ? data->set.str[STRING_SSL_PINNEDPUBLICKEY_PROXY] : data->set.str[STRING_SSL_PINNEDPUBLICKEY];
#else
...
static FETCHcode ossl_connect_step3(struct Fetch_cfilter *cf,
                                    struct Fetch_easy *data)
{
  FETCHcode result = FETCHE_OK;
  struct ssl_connect_data *connssl = cf->ctx;
  ...
  result = Fetch_oss_check_peer_cert(cf, data, octx, &connssl->peer);
  if (!result)
    connssl->connecting_state = ssl_connect_done;
  else
    /* on error, remove sessions we might have in the pool */
    Fetch_ssl_scache_remove_all(cf, data, connssl->peer.scache_key);

  return result;
}

static FETCHcode ossl_connect_common(struct Fetch_cfilter *cf,
                                     struct Fetch_easy *data,
                                     bool nonblocking,
                                     bool *done)
{
...
static FETCHcode ossl_connect_nonblocking(struct Fetch_cfilter *cf,
                                          struct Fetch_easy *data,
                                          bool *done)
{
...
static FETCHcode ossl_connect(struct Fetch_cfilter *cf,
                              struct Fetch_easy *data)
{
  FETCHcode result;
  bool done = FALSE;
...
static bool ossl_data_pending(struct Fetch_cfilter *cf,
                              const struct Fetch_easy *data)
{
  struct ssl_connect_data *connssl = cf->ctx;
  struct ossl_ctx *octx = (struct ossl_ctx *)connssl->backend;
...
static ssize_t ossl_send(struct Fetch_cfilter *cf,
                         struct Fetch_easy *data,
                         const void *mem,
                         size_t len,
                         FETCHcode *fetchcode)
{
  ...
  if (rc <= 0)
  {
    ...
    case SSL_ERROR_SYSCALL:
    {
      ...
      if (octx->io_result == FETCHE_AGAIN)
      {
        *fetchcode = FETCHE_AGAIN;
        rc = -1;
        goto out;
      ...
      if (sslerror)
        ...
        Fetch_strerror(sockerr, error_buffer, sizeof(error_buffer));
      else
...
static ssize_t ossl_recv(struct Fetch_cfilter *cf,
  ...
  if (nread <= 0)
  {
    ...
    default:
      ...
      if (err == SSL_ERROR_SYSCALL)
      {
        ...
        failf(data, OSSL_PACKAGE " SSL_read: %s, errno %d"
                                 " (Fatal because this is a fetch debug build)",
              error_buffer, sockerr);
        *fetchcode = FETCHE_RECV_ERROR;
        nread = -1;
        goto out;
...
static FETCHcode ossl_get_channel_binding(struct Fetch_easy *data, int sockindex,
                                          struct dynbuf *binding)
{
  ...
  do
  {
    const struct Fetch_cftype *cft = cf->cft;
    struct ssl_connect_data *connssl = cf->ctx;
  ...
  if (!OBJ_find_sigid_algs(X509_get_signature_nid(cert), &algo_nid, NULL))
  {
    ...
    return FETCHE_SSL_INVALIDCERTSTATUS;
  }
  ...
  if (Fetch_dyn_addn(binding, prefix, sizeof(prefix) - 1) != FETCHE_OK)
    return FETCHE_OUT_OF_MEMORY;
  /* Append digest */
  if (Fetch_dyn_addn(binding, buf, length))
    return FETCHE_OUT_OF_MEMORY;
...
#endif
#elif defined(OPENSSL_IS_BORINGSSL)
#ifdef FETCH_BORINGSSL_VERSION
  return msnprintf(buffer, size, "%s/%s",
                   OSSL_PACKAGE,
                   FETCH_BORINGSSL_VERSION);
#else
...
static FETCHcode ossl_random(struct Fetch_easy *data,
                             unsigned char *entropy, size_t length)
{
  ...
  if (data)
  {
    if (ossl_seed(data))         /* Initiate the seed if not already done */
      return FETCHE_FAILED_INIT; /* could not seed for some reason */
  }
  else
  {
    if (!rand_enough())
  ...
  rc = RAND_bytes(entropy, (ossl_valsize_t)fetchx_uztosi(length));
  return rc == 1 ? FETCHE_OK : FETCHE_FAILED_INIT;
}

#if (OPENSSL_VERSION_NUMBER >= 0x0090800fL) && !defined(OPENSSL_NO_SHA256)
static FETCHcode ossl_sha256sum(const unsigned char *tmp, /* input */
                                size_t tmplen,
...
static void *ossl_get_internals(struct ssl_connect_data *connssl,
                                FETCHINFO info)
{
  /* Legacy: FETCHINFO_TLS_SESSION must return an SSL_CTX pointer. */
  struct ossl_ctx *octx = (struct ossl_ctx *)connssl->backend;
  DEBUGASSERT(octx);
  return info == FETCHINFO_TLS_SESSION ? (void *)octx->ssl_ctx : (void *)octx->ssl;
}

const struct Fetch_ssl Fetch_ssl_openssl = {
    {FETCHSSLBACKEND_OPENSSL, "openssl"}, /* info */
