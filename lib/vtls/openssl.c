 ...
 */
    ...
    ! defined(LIBRESSL_VERSION_NUMBER)) || defined(BORINGSSL_201512)
...
#ifdef HAVE_KEYLOG_CALLBACK
  keylog_file_path = curl_getenv("SSLKEYLOGFILE");
  if (keylog_file_path) {
    keylog_file = fopen(keylog_file_path, "a");
    if (keylog_file && ftell(keylog_file) == 0) {
        fprintf(keylog_file,
                "# SSL/TLS secrets log file, generated by OpenSSL\n");
    }
    free((void *)keylog_file_path);
  }
#endif
...
#ifdef HAVE_KEYLOG_CALLBACK
static void
ossl_keylog_callback(const SSL *ssl, const char *line)
{
    if (!keylog_file)
        return;
