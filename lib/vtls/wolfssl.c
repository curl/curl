...
static CURLcode wssl_connect_common(struct Curl_cfilter *cf,
                               ...
                               bool nonblocking)
{
  ...
  for(;;) {
    ...
    if(connssl->io_need && !nonblocking) {
    ...
    CURL_TRC_CF(data, cf, "wssl_handshake_step() -> %d", result);

    if(result == CURLE_AGAIN) {  /* handshake ongoing */
...
out:
  CURL_TRC_CF(data, cf, "wssl_handshake(blocking=%d) -> %d",
              !nonblocking, result);
  return result;
}
                                      ...
                                      bool *done)
{
  ...
  *done = FALSE;
  ...
  return wssl_connect_common(cf, data, TRUE, done);
}
                                    ...
                                    bool *done)
{
  ...
  *done = FALSE;
  ...
  if(connssl->connecting_state == ssl_connect_2) {
    ...
    result = wssl_handshake(cf, data, nonblocking);
...
static CURLcode wssl_connect_nonblocking(struct Curl_cfilter *cf,
                                         struct Curl_easy *data,
                                         bool *done)
{
  return wssl_connect_common(cf, data, TRUE, done);
                             ...
                             struct Curl_easy *data)
{
  ...
  result = wssl_connect_common(cf, data, FALSE, &done);
  if(result)
    return result;
...
const struct Curl_ssl Curl_ssl_wolfssl = {
  ...
  wssl_connect_nonblocking,        /* connect_nonblocking */
