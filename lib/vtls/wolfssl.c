...
/* To determine what functions are available we rely on one or both of:
   ...
   - the symbols detected by fetch's configure
                              ...
                              int secretSz, void *ctx)
{
  ...
  if (!ssl || !Fetch_tls_keylog_enabled())
  {
    return 0;
  }
  ...
  Fetch_tls_keylog_write(label, client_random, secret, secretSz);
  return 0;
}
...
wolfssl_log_tls12_secret(WOLFSSL *ssl)
{
   ...
   * fetch rejects the server certificate and aborts the handshake.
   */
  for (i = 0; i < msLen; i++)
  {
    x |= ms[i];
  ...
  Fetch_tls_keylog_write("CLIENT_RANDOM", cr, ms, msLen);
}
#endif /* OPENSSL_EXTRA */
...
{
  struct Fetch_cfilter *cf = wolfSSL_BIO_get_data(bio);
  long ret = 1;
                                    ...
                                    const char *buf, int blen)
{
      ...
      (backend->io_send_blocked_len < blen))
  {
    ...
    FETCH_TRC_CF(data, cf, "bio_write, shutdown restrict send of %d"
                           " to %d bytes",
                 blen, backend->io_send_blocked_len);
    skiplen = (ssize_t)(blen - backend->io_send_blocked_len);
    blen = backend->io_send_blocked_len;
  }
  nwritten = Fetch_conn_cf_send(cf->next, data, buf, blen, FALSE, &result);
  backend->io_result = result;
  FETCH_TRC_CF(data, cf, "bio_write(len=%d) -> %zd, %d",
               blen, nwritten, result);
#ifdef USE_FULL_BIO
  wolfSSL_BIO_clear_retry_flags(bio);
#endif
  if (nwritten < 0 && FETCHE_AGAIN == result)
  {
...
static int wolfssl_bio_cf_in_read(WOLFSSL_BIO *bio, char *buf, int blen)
{
  ...
  nread = Fetch_conn_cf_recv(cf->next, data, buf, blen, &result);
  backend->io_result = result;
  FETCH_TRC_CF(data, cf, "bio_read(len=%d) -> %zd, %d", blen, nread, result);
#ifdef USE_FULL_BIO
  wolfSSL_BIO_clear_retry_flags(bio);
#endif
  if (nread < 0 && FETCHE_AGAIN == result)
...
#define wolfssl_bio_cf_init_methods() Fetch_nop_stmt
#define wolfssl_bio_cf_free_methods() Fetch_nop_stmt
                                  ...
                                  const char *alpn)
{
  ...
  if (sdata_len <= 0)
  {
    FETCH_TRC_CF(data, cf, "fail to assess session length: %u", sdata_len);
  ...
  if (sdata_len <= 0)
  {
    FETCH_TRC_CF(data, cf, "fail to serialize session: %u", sdata_len);
    result = FETCHE_FAILED_INIT;
    goto out;
  }

  result = Fetch_ssl_session_create(sdata, sdata_len,
                                   ietf_tls_id, alpn,
...
static int wssl_vtls_new_session_cb(WOLFSSL *ssl, WOLFSSL_SESSION *session)
{
  ...
  cf = (struct Fetch_cfilter *)wolfSSL_get_app_data(ssl);
  DEBUGASSERT(cf != NULL);
  if (cf && session)
  {
    struct ssl_connect_data *connssl = cf->ctx;
    ...
    if (connssl && data)
    {
      (void)Fetch_wssl_cache_session(cf, data, connssl->peer.scache_key,
                                  ...
                                  const char *ssl_peer_key)
{
  ...
  result = Fetch_ssl_scache_take(cf, data, ssl_peer_key, &sc_session);
  if (!result && sc_session && sc_session->sdata && sc_session->sdata_len)
  {
    ...
    if (session)
    {
      ...
      {
        Fetch_ssl_session_destroy(sc_session);
        sc_session = NULL;
  ...
  Fetch_ssl_scache_return(cf, data, ssl_peer_key, sc_session);
  return result;
}

static FETCHcode wssl_populate_x509_store(struct Fetch_cfilter *cf,
                                          struct Fetch_easy *data,
                                          ...
                                          struct wolfssl_ctx *wssl)
{
  ...
  FETCH_TRC_CF(data, cf, "wssl_populate_x509_store, path=%s, blob=%d",
...
static bool
wssl_cached_x509_store_expired(const struct Fetch_easy *data,
...
static bool
wssl_cached_x509_store_different(struct Fetch_cfilter *cf,
                                 const struct wssl_x509_share *mb)
{
  struct ssl_primary_config *conn_config = Fetch_ssl_cf_get_primary_config(cf);
...
static WOLFSSL_X509_STORE *wssl_get_cached_x509_store(struct Fetch_cfilter *cf,
...
static void wssl_set_cached_x509_store(struct Fetch_cfilter *cf,
                                       const struct Fetch_easy *data,
                                       WOLFSSL_X509_STORE *store)
{
  struct ssl_primary_config *conn_config = Fetch_ssl_cf_get_primary_config(cf);
...
wssl_add_default_ciphers(bool tls13, struct dynbuf *buf)
{
  ...
  for (i = 0; (str = wolfSSL_get_cipher_list(i)); i++)
  {
    ...
    if (Fetch_dyn_len(buf))
    {
      FETCHcode result = Fetch_dyn_addn(buf, ":", 1);
      if (result)
        return result;
    }

    n = strlen(str);
    if (Fetch_dyn_addn(buf, str, n))
      return FETCHE_OUT_OF_MEMORY;
  }
 ...
 */
static FETCHcode
wolfssl_connect_step1(struct Fetch_cfilter *cf, struct Fetch_easy *data)
{
  int res;
  char *curves;
  struct ssl_connect_data *connssl = cf->ctx;
  struct wolfssl_ctx *backend =
      (struct wolfssl_ctx *)connssl->backend;
  struct ssl_primary_config *conn_config = Fetch_ssl_cf_get_primary_config(cf);
  const struct ssl_config_data *ssl_config = Fetch_ssl_cf_get_config(cf, data);
...
#define MAX_CIPHER_LEN 4096
  if (conn_config->cipher_list || conn_config->cipher_list13)
  {
    ...
    struct dynbuf c;
    FETCHcode result;
    Fetch_dyn_init(&c, MAX_CIPHER_LEN);

    if (ciphers13)
      result = Fetch_dyn_add(&c, ciphers13);
    else
    ...
    if (!result)
    {
      ...
      {
        if (Fetch_dyn_len(&c))
          result = Fetch_dyn_addn(&c, ":", 1);
        if (!result)
          result = Fetch_dyn_add(&c, ciphers12);
      }
      else
    ...
    if (!wolfSSL_CTX_set_cipher_list(backend->ctx, Fetch_dyn_ptr(&c)))
    {
      failf(data, "failed setting cipher list: %s", Fetch_dyn_ptr(&c));
      Fetch_dyn_free(&c);
      return FETCHE_SSL_CIPHER;
    }
    infof(data, "Cipher selection: %s", Fetch_dyn_ptr(&c));
    Fetch_dyn_free(&c);
  }
#endif
  ...
  if (ssl_config->primary.cert_blob || ssl_config->primary.clientcert)
  {
    ...
    const struct fetch_blob *cert_blob = ssl_config->primary.cert_blob;
    const struct fetch_blob *key_blob = ssl_config->key_blob;
    ...
    case WOLFSSL_FILETYPE_PEM:
      rc = cert_blob ? wolfSSL_CTX_use_certificate_chain_buffer(backend->ctx,
                                                                cert_blob->data,
                                                                (long)cert_blob->len)
                     : wolfSSL_CTX_use_certificate_chain_file(backend->ctx, cert_file);
      break;
    case WOLFSSL_FILETYPE_ASN1:
      rc = cert_blob ? wolfSSL_CTX_use_certificate_buffer(backend->ctx, cert_blob->data,
    ...
    rc = key_blob ? wolfSSL_CTX_use_PrivateKey_buffer(backend->ctx, key_blob->data,
  ...
  wolfSSL_CTX_set_verify(backend->ctx,
                         conn_config->verifypeer ? WOLFSSL_VERIFY_PEER : WOLFSSL_VERIFY_NONE, NULL);
  ...
  if (data->set.ssl.fsslctx)
  {
    ...
    {
      result = Fetch_wssl_setup_x509_store(cf, data, backend);
      if (result)
        return result;
    }
  ...
  else if (conn_config->verifypeer)
  {
    failf(data, "SSL: Certificates cannot be loaded because wolfSSL was built"
                ...
                " (insecure) or if you are building an application with libfetch you"
                " can load certificates via FETCHOPT_SSL_CTX_FUNCTION.");
    return FETCHE_SSL_CONNECT_ERROR;
  }
#endif
  ...
  if (connssl->alpn)
  {
    ...
    result = Fetch_alpn_to_proto_str(&proto, connssl->alpn);
    if (result ||
  ...
  if (ssl_config->primary.cache_session)
  {
    ...
    (void)Fetch_wssl_setup_session(cf, data, backend, connssl->peer.scache_key);
  ...
  if (ECH_ENABLED(data))
  {
    ...
    if (data->set.tls_ech & FETCHECH_CLA_CFG && data->set.str[STRING_ECH_CONFIG])
    {
      char *b64val = data->set.str[STRING_ECH_CONFIG];
      word32 b64len = 0;

      b64len = (word32)strlen(b64val);
      if (b64len && wolfSSL_SetEchConfigsBase64(backend->handle, b64val, b64len) != WOLFSSL_SUCCESS)
      {
    ...
    else
    {
      ...
      dns = Fetch_fetch_addr(data, connssl->peer.hostname, connssl->peer.port);
      if (!dns)
      {
  ...
  if (!wolfSSL_set_fd(backend->handle,
                      (int)Fetch_conn_cf_get_socket(cf, data)))
  {
...
static FETCHcode
wolfssl_connect_step2(struct Fetch_cfilter *cf, struct Fetch_easy *data)
{
  int ret = -1;
  struct ssl_connect_data *connssl = cf->ctx;
  struct wolfssl_ctx *backend =
  ...
  const char *const pinnedpubkey = Fetch_ssl_cf_is_proxy(cf) ? data->set.str[STRING_SSL_PINNEDPUBLICKEY_PROXY] : data->set.str[STRING_SSL_PINNEDPUBLICKEY];
#else
  ...
  {
    char *snihost = connssl->peer.sni ? connssl->peer.sni : connssl->peer.hostname;
  ...
  if (ret != 1)
  {
    ...
    else if (-1 == detail)
    {
      ...
      else
      {
        ...
        rv = Fetch_base64_encode((const char *)echConfigs, echConfigsLen,
                                &b64str, &blen);
        if (!rv && b64str)
...
#endif
    else if (backend->io_result == FETCHE_AGAIN)
    {
      return FETCHE_OK;
    }
    else
  ...
  if (pinnedpubkey)
  {
    ...
    if (!x509)
    {
      ...
      return FETCHE_SSL_PINNEDPUBKEYNOTMATCH;
    }
  ...
  if (connssl->alpn)
  {
    ...
    {
      Fetch_alpn_set_negotiated(cf, data, connssl,
...
static ssize_t wolfssl_send(struct Fetch_cfilter *cf,
                            struct Fetch_easy *data,
                            const void *mem,
                            size_t len,
                            FETCHcode *fetchcode)
{
  ...
  if (rc <= 0)
  {
    ...
    case WOLFSSL_ERROR_WANT_WRITE:
      ...
      FETCH_TRC_CF(data, cf, "wolfssl_send(len=%zu) -> AGAIN", len);
      *fetchcode = FETCHE_AGAIN;
      return -1;
    default:
      if (backend->io_result == FETCHE_AGAIN)
      {
        FETCH_TRC_CF(data, cf, "wolfssl_send(len=%zu) -> AGAIN", len);
        *fetchcode = FETCHE_AGAIN;
        return -1;
      }
      FETCH_TRC_CF(data, cf, "wolfssl_send(len=%zu) -> %d, %d", len, rc, err);
      {
        char error_buffer[256];
  ...
  FETCH_TRC_CF(data, cf, "wolfssl_send(len=%zu) -> %d", len, rc);
  return rc;
}

static FETCHcode wolfssl_shutdown(struct Fetch_cfilter *cf,
                                  struct Fetch_easy *data,
                                  bool send_shutdown, bool *done)
{
  ...
  *done = FALSE;
  if (!(wolfSSL_get_shutdown(wctx->handle) & WOLFSSL_SENT_SHUTDOWN))
  {
    ...
    FETCH_TRC_CF(data, cf, "wolfSSL_read, nread=%d, err=%d", nread, err);
  ...
  case WOLFSSL_ERROR_ZERO_RETURN: /* no more data */
    FETCH_TRC_CF(data, cf, "SSL shutdown received");
    *done = TRUE;
    break;
  ...
  default:
    ...
    FETCH_TRC_CF(data, cf, "SSL shutdown, error: '%s'(%d)",
                 wolfssl_strerror((unsigned long)err, error_buffer,
                                  sizeof(error_buffer)),
                 detail);
...
static void wolfssl_close(struct Fetch_cfilter *cf, struct Fetch_easy *data)
{
  struct ssl_connect_data *connssl = cf->ctx;
  struct wolfssl_ctx *backend =
      (struct wolfssl_ctx *)connssl->backend;
...
static ssize_t wolfssl_recv(struct Fetch_cfilter *cf,
                            struct Fetch_easy *data,
                            ...
                            FETCHcode *fetchcode)
{
  ...
  *fetchcode = FETCHE_OK;
  ...
  if (nread <= 0)
  {
    ...
    {
    case WOLFSSL_ERROR_ZERO_RETURN: /* no more data */
      FETCH_TRC_CF(data, cf, "wolfssl_recv(len=%zu) -> CLOSED", blen);
      *fetchcode = FETCHE_OK;
      return 0;
    ...
    case WOLFSSL_ERROR_WANT_WRITE:
      ...
      FETCH_TRC_CF(data, cf, "wolfssl_recv(len=%zu) -> AGAIN", blen);
      *fetchcode = FETCHE_AGAIN;
      return -1;
    default:
      if (backend->io_result == FETCHE_AGAIN)
  ...
  FETCH_TRC_CF(data, cf, "wolfssl_recv(len=%zu) -> %d", blen, nread);
  return nread;
}
...
static bool wolfssl_data_pending(struct Fetch_cfilter *cf,
                                 const struct Fetch_easy *data)
{
...
static FETCHcode
wolfssl_connect_common(struct Fetch_cfilter *cf,
                       struct Fetch_easy *data,
                       bool nonblocking,
                       bool *done)
{
  ...
  while (ssl_connect_2 == connssl->connecting_state)
  {
    ...
    if (connssl->io_need)
    {
      ...
      int what = Fetch_socket_check(readfd, FETCH_SOCKET_BAD, writefd,
...
static FETCHcode wolfssl_connect_nonblocking(struct Fetch_cfilter *cf,
                                             struct Fetch_easy *data,
                                             bool *done)
{
...
static FETCHcode wolfssl_connect(struct Fetch_cfilter *cf,
                                 struct Fetch_easy *data)
{
  FETCHcode result;
  bool done = FALSE;
...
static FETCHcode wolfssl_random(struct Fetch_easy *data,
...
static FETCHcode wolfssl_sha256sum(const unsigned char *tmp, /* input */
                                   size_t tmplen,
...
const struct Fetch_ssl Fetch_ssl_wolfssl = {
    {FETCHSSLBACKEND_WOLFSSL, "wolfssl"}, /* info */
