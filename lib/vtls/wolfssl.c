...
static int bio_cf_out_write(WOLFSSL_BIO *bio, const char *buf, int blen)
{
  ...
  CURL_LOG_CF(data, cf, "bio_write(len=%d) -> %zd, %d",
...
static int bio_cf_in_read(WOLFSSL_BIO *bio, char *buf, int blen)
{
  ...
  backend->io_result = result;
  CURL_LOG_CF(data, cf, "bio_read(len=%d) -> %zd, %d", blen, nread, result);
  wolfSSL_BIO_clear_retry_flags(bio);
                            ...
                            CURLcode *curlcode)
{
  ...
  if(rc <= 0) {
    ...
    case SSL_ERROR_WANT_WRITE:
      ...
      CURL_LOG_CF(data, cf, "wolfssl_send(len=%zu) -> AGAIN", len);
    ...
    default:
      ...
      CURL_LOG_CF(data, cf, "wolfssl_send(len=%zu) -> %d, %d", len, rc, err);
  ...
  CURL_LOG_CF(data, cf, "wolfssl_send(len=%zu) -> %d", len, rc);
  return rc;
}
  ...
  *curlcode = CURLE_OK;
  ...
  if(nread <= 0) {
    ...
    case SSL_ERROR_ZERO_RETURN: /* no more data */
      CURL_LOG_CF(data, cf, "wolfssl_recv(len=%zu) -> CLOSED", blen);
      *curlcode = CURLE_OK;
      return 0;
    case SSL_ERROR_NONE:
      /* FALLTHROUGH */
    ...
    case SSL_ERROR_WANT_WRITE:
      ...
      CURL_LOG_CF(data, cf, "wolfssl_recv(len=%zu) -> AGAIN", blen);
      *curlcode = CURLE_AGAIN;
      return -1;
    default:
      if(backend->io_result == CURLE_AGAIN) {
  ...
  CURL_LOG_CF(data, cf, "wolfssl_recv(len=%zu) -> %d", blen, nread);
  return nread;
}
