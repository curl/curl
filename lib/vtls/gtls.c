...
gnutls_set_ssl_version_min_max(struct Curl_easy *data,
                               ...
                               bool tls13support)
{
  long ssl_version = config->version;
  long ssl_version_max = config->version_max;
                                    ...
                                    gnutls_certificate_credentials_t creds)
{
  ...
  if(gctx->config->ca_info_blob) {
    gnutls_datum_t ca_info_datum;
    if(gctx->config->ca_info_blob->len > (size_t)UINT_MAX) {
      failf(data, "certificate blob too long: %zu bytes",
            gctx->config->ca_info_blob->len);
      return CURLE_SSL_CACERT_BADFILE;
    }
    ca_info_datum.data = gctx->config->ca_info_blob->data;
    ca_info_datum.size = (unsigned int)gctx->config->ca_info_blob->len;
  ...
  /* CURLOPT_CAINFO_BLOB overrides CURLOPT_CAINFO */
    ...
    if(rc < 0) {
      infof(data, "error reading CA cert file %s (%s)%s",
            gctx->config->CAfile, gnutls_strerror(rc),
            (creds_are_empty ? "" : ", continuing anyway"));
      if(creds_are_empty) {
        ssl_config->certverifyresult = rc;
        return CURLE_SSL_CACERT_BADFILE;
      ...
      infof(data, "  CAfile: %d certificates in %s", rc, gctx->config->CAfile);
  }

  if(gctx->config->CApath) {
    /* set the trusted CA cert directory */
    rc = gnutls_certificate_set_x509_trust_dir(creds, gctx->config->CApath,
                                               GNUTLS_X509_FMT_PEM);
    creds_are_empty = creds_are_empty && (rc <= 0);
    if(rc < 0) {
      infof(data, "error reading CA cert file %s (%s)%s",
            gctx->config->CApath, gnutls_strerror(rc),
            (creds_are_empty ? "" : ", continuing anyway"));
      if(creds_are_empty) {
        ssl_config->certverifyresult = rc;
        return CURLE_SSL_CACERT_BADFILE;
      ...
      infof(data, "  CApath: %d certificates in %s", rc, gctx->config->CApath);
  }

  if(creds_are_empty)
    infof(data, "  no trust anchors configured");

  if(gctx->config->CRLfile) {
    /* set the CRL list file */
    rc = gnutls_certificate_set_x509_crl_file(creds, gctx->config->CRLfile,
                                              GNUTLS_X509_FMT_PEM);
    if(rc < 0) {
      failf(data, "error reading CRL file %s (%s)",
            gctx->config->CRLfile, gnutls_strerror(rc));
      return CURLE_SSL_CRL_BADFILE;
    }
    else
      infof(data, "  CRLfile: %d CRL in %s", rc, gctx->config->CRLfile);
  }
...
static bool gtls_shared_creds_different(struct gtls_ctx *gctx,
                                        const struct gtls_shared_creds *sc)
{
  if(!sc->CAfile || !gctx->config->CAfile)
    return sc->CAfile != gctx->config->CAfile;

  return strcmp(sc->CAfile, gctx->config->CAfile);
}

static struct gtls_shared_creds *gtls_get_cached_creds(struct Curl_easy *data,
                                                       struct gtls_ctx *gctx)
{
  ...
  if(data->multi) {
    ...
    if(shared_creds && shared_creds->creds &&
       ...
       !gtls_shared_creds_different(gctx, shared_creds)) {
      return shared_creds;
    }
  }
  return NULL;
...
static void gtls_set_cached_creds(struct Curl_easy *data,
                                  struct gtls_ctx *gctx,
                                  struct gtls_shared_creds *sc)
{
  ...
  if(gctx->config->CAfile) {
    sc->CAfile = curlx_strdup(gctx->config->CAfile);
    if(!sc->CAfile)
      return;
  }
                                      ...
                                      struct gtls_ctx *gctx)
{
  ...
  cache_criteria_met = (data->set.general_ssl.ca_cache_timeout != 0) &&
    gctx->config->verifypeer &&
    !gctx->config->CApath &&
    !gctx->config->ca_info_blob &&
    !ssl_config->primary.CRLfile &&
    !ssl_config->native_ca_store &&
    !gctx->config->clientcert; /* GnuTLS adds client cert to credentials! */

  if(cache_criteria_met)
    cached_creds = gtls_get_cached_creds(data, gctx);

  if(cached_creds && !Curl_gtls_shared_creds_up_ref(cached_creds)) {
    CURL_TRC_CF(data, cf, "using shared trust anchors and CRLs");
    Curl_gtls_shared_creds_free(&gctx->shared_creds);
    gctx->shared_creds = cached_creds;
    rc = gnutls_credentials_set(gctx->session, GNUTLS_CRD_CERTIFICATE,
                                gctx->shared_creds->creds);
    if(rc != GNUTLS_E_SUCCESS) {
    ...
    result = gtls_populate_creds(cf, data, gctx, gctx->shared_creds->creds);
    if(result)
      return result;
    gctx->shared_creds->trust_setup = TRUE;
    if(cache_criteria_met)
      gtls_set_cached_creds(data, gctx, gctx->shared_creds);
  }
  return CURLE_OK;
}
                                 ...
                                 size_t quic_tp_len)
{
  ...
  if(!result) {
    result = Curl_ssl_scache_put(cf, data, gctx->config,
                                 ssl_peer_key, sc_session);
                                          ...
                                          gnutls_session_t session)
{
  ...
  struct gtls_ssl_backend_data *backend =
    (struct gtls_ssl_backend_data *)connssl->backend;
  return Curl_gtls_cache_session(cf, data, &backend->gtls,
...
static CURLcode gtls_set_priority(struct Curl_easy *data,
                                  struct gtls_ctx *gctx,
                                  const char *priority)
{
  struct dynbuf buf;
  ...
  if(gctx->config->cipher_list) {
    if((gctx->config->cipher_list[0] == '+') ||
       (gctx->config->cipher_list[0] == '-') ||
       (gctx->config->cipher_list[0] == '!')) {
      /* add it to out own */
      ...
      result = curlx_dyn_addf(&buf, ":%s", gctx->config->cipher_list);
      if(result)
        goto out;
      priority = curlx_dyn_ptr(&buf);
    }
    else /* replace our own completely */
      priority = gctx->config->cipher_list;
  }

  infof(data, "GnuTLS priority: %s", priority);
  rc = gnutls_priority_set_direct(gctx->session, priority, &err);
  if(rc != GNUTLS_E_SUCCESS) {
                                 ...
                                 size_t earlydata_max)
{
  ...
  if(gctx->config->version == CURL_SSLVERSION_SSLv2 ||
     gctx->config->version == CURL_SSLVERSION_SSLv3) {
    failf(data, "GnuTLS does not support SSLv2 or SSLv3");
    return CURLE_SSL_CONNECT_ERROR;
  }

  /* allocate a shared creds struct */
  result = Curl_gtls_shared_creds_create(data, &gctx->shared_creds);
  if(result)
    return result;

#ifdef USE_GNUTLS_SRP
  if(gctx->config->username && Curl_auth_allowed_to_host(data)) {
    infof(data, "Using TLS-SRP username: %s", gctx->config->username);

    rc = gnutls_srp_allocate_client_credentials(&gctx->srp_client_cred);
    ...
    rc = gnutls_srp_set_client_credentials(gctx->srp_client_cred,
                                           gctx->config->username,
                                           gctx->config->password);
    if(rc != GNUTLS_E_SUCCESS) {
  ...
  rc = gnutls_init(&gctx->session, init_flags);
  if(rc != GNUTLS_E_SUCCESS) {
  ...
  if(peer->sni) {
    if(gnutls_server_name_set(gctx->session, GNUTLS_NAME_DNS,
  ...
  rc = gnutls_set_default_priority(gctx->session);
  if(rc != GNUTLS_E_SUCCESS)
    return CURLE_SSL_CONNECT_ERROR;
  ...
  if(gctx->config->version == CURL_SSLVERSION_TLSv1_3) {
    if(!tls13support) {
  ...
  result = gnutls_set_ssl_version_min_max(data, peer, gctx->config,
                                          &prioritylist, tls13support);
  if(result)
    return result;

  result = gtls_set_priority(data, gctx, prioritylist);
  if(result)
    return result;

  if(gctx->config->clientcert) {
    if(!gctx->shared_creds->trust_setup) {
      result = Curl_gtls_client_trust_setup(cf, data, gctx);
      if(result)
        return result;
    }
    if(ssl_config->cert_type && curl_strequal(ssl_config->cert_type, "P12")) {
      rc = gnutls_certificate_set_x509_simple_pkcs12_file(
        gctx->shared_creds->creds, gctx->config->clientcert,
        GNUTLS_X509_FMT_DER,
      ...
      rc = gnutls_certificate_set_x509_key_file2(
           gctx->shared_creds->creds,
           gctx->config->clientcert,
           ssl_config->key ? ssl_config->key : gctx->config->clientcert,
  ...
  if(gctx->config->username) {
    rc = gnutls_credentials_set(gctx->session, GNUTLS_CRD_SRP,
                                gctx->srp_client_cred);
    if(rc != GNUTLS_E_SUCCESS) {
      failf(data, "gnutls_credentials_set() failed: %s", gnutls_strerror(rc));
      return CURLE_SSL_CONNECT_ERROR;
    }
  ...
  if(gctx->config->verifystatus) {
    rc = gnutls_ocsp_status_request_enable_client(gctx->session,
                                                  NULL, 0, NULL);
    if(rc != GNUTLS_E_SUCCESS) {
                            ...
                            Curl_gtls_init_session_reuse_cb *sess_reuse_cb)
{
  ...
  if(config->cache_session && !config->verifystatus) {
    result = Curl_ssl_scache_take(cf, data, config, peer->scache_key, &scs);
    if(result)
      goto out;
    ...
    if(scs && scs->sdata && scs->sdata_len &&
      ...
      result = gtls_client_init(cf, data, gctx, peer, scs->earlydata_max);
      if(result)
        goto out;
  ...
  if(!gtls_session_setup) {
    result = gtls_client_init(cf, data, gctx, peer, 0);
    if(result)
      goto out;
  }
...
out:
  Curl_ssl_scache_return(cf, data, gctx->config, peer->scache_key, scs);
  return result;
}
                                  ...
                                  bool *pverified)
{
  ...
  result = Curl_vtls_apple_verify(cf, data, peer, config, chain->num_certs,
                                ...
                                const char *pinned_key)
{
  ...
  chain.certs = gnutls_certificate_get_peers(gctx->session, &chain.num_certs);
  if(!chain.certs) {
    if(gctx->config->verifypeer ||
       gctx->config->verifyhost ||
       gctx->config->issuercert) {
#ifdef USE_GNUTLS_SRP
      if(ssl_config->primary.username && !gctx->config->verifypeer &&
         gnutls_cipher_get(gctx->session)) {
  ...
  if(gctx->config->verifypeer) {
    ...
    rc = gnutls_certificate_verify_peers2(gctx->session, &verify_status);
    if(rc < 0) {
    ...
    *certverifyresult = verify_status;
    ...
    if(!verified && ssl_config->native_ca_store) {
      result = glts_apple_verify(cf, data, peer, config, &chain, &verified);
    ...
    if(!verified) {
      ...
      failf(data, "SSL certificate verification failed: %s. (CAfile: %s "
            "CRLfile: %s)", cause,
            gctx->config->CAfile ? gctx->config->CAfile : "none",
            ssl_config->primary.CRLfile ?
  ...
  if(gctx->config->verifystatus) {
    result = gtls_verify_ocsp_status(data, gctx->session);
    if(result)
      goto out;
  }
  else
  ...
  if(gctx->config->issuercert) {
    ...
    issuerp = load_file(gctx->config->issuercert);
    ...
    if(rc <= 0) {
      failf(data, "server certificate issuer check failed (IssuerCert: %s)",
            gctx->config->issuercert ? gctx->config->issuercert : "none");
      result = CURLE_SSL_ISSUER_ERROR;
      goto out;
    }
  ...
  rc = (int)gnutls_x509_crt_check_hostname(x509_cert,
                                           peer->sni ? peer->sni :
                                           peer->hostname);
  result = (!rc && gctx->config->verifyhost) ?
    CURLE_PEER_FAILED_VERIFICATION : CURLE_OK;
  gtls_msg_verify_result(data, peer, x509_cert, rc, gctx->config->verifyhost);
  if(result)
    goto out;
                                  ...
                                  struct gtls_ctx *gctx)
{
  ...
  result = Curl_gtls_verifyserver(cf, data, gctx, ssl_config,
  ...
  if(gnutls_protocol_get_version(gctx->session) < GNUTLS_TLS1_3)
    result = cf_gtls_update_session_id(cf, data, gctx->session);
#endif
                                    ...
                                    bool *done)
{
  ...
  *done = FALSE;
  ...
  if(connssl->connecting_state == ssl_connect_3) {
    ...
    result = gtls_verifyserver(cf, data, &backend->gtls);
    if(result)
      goto out;
