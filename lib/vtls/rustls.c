            ...
            char *plainbuf, size_t plainlen, CURLcode *err)
{
  ...
  if(io_error == EAGAIN || io_error == EWOULDBLOCK) {
    LOG_CF_DEBUG(data, cf, "cr_recv: EAGAIN or EWOULDBLOCK");
  }
  else if(io_error) {
  ...
  LOG_CF_DEBUG(data, cf, "cr_recv: read %ld TLS bytes", tls_bytes_read);
  ...
  while(plain_bytes_copied < plainlen) {
    ...
    if(rresult == RUSTLS_RESULT_PLAINTEXT_EMPTY) {
      LOG_CF_DEBUG(data, cf, "cr_recv: got PLAINTEXT_EMPTY. "
      ...
      *err = CURLE_READ_ERROR;
      ...
      LOG_CF_DEBUG(data, cf, "cr_recv: got %ld plain bytes", n);
        ...
        const void *plainbuf, size_t plainlen, CURLcode *err)
{
  ...
  LOG_CF_DEBUG(data, cf, "cr_send: %ld plain bytes", plainlen);
  ...
  while(rustls_connection_wants_write(rconn)) {
    io_error = rustls_connection_write_tls(rconn, write_cb, &io_ctx,
                                           &tlswritten);
    if(io_error == EAGAIN || io_error == EWOULDBLOCK) {
      LOG_CF_DEBUG(data, cf, "cr_send: EAGAIN after %zu bytes",
                   tlswritten_total);
      *err = CURLE_AGAIN;
      return -1;
    }
    ...
    LOG_CF_DEBUG(data, cf, "cr_send: wrote %zu TLS bytes", tlswritten);
