...
/* For a given rustls_result error code, return the best-matching FETCHcode. */
static FETCHcode map_error(rustls_result r)
{
...
static bool
cr_data_pending(struct Fetch_cfilter *cf, const struct Fetch_easy *data)
{
...
read_cb(void *userdata, uint8_t *buf, uintptr_t len, uintptr_t *out_n)
{
  ...
  FETCHcode result;
  int ret = 0;
  ssize_t nread = Fetch_conn_cf_recv(io_ctx->cf->next, io_ctx->data,
                                    (char *)buf, len, &result);
  if (nread < 0)
  {
    nread = 0;
  ...
  *out_n = (uintptr_t)nread;
  FETCH_TRC_CF(io_ctx->data, io_ctx->cf, "cf->next recv(len=%zu) -> %zd, %d",
               len, nread, result);
  return ret;
}
...
write_cb(void *userdata, const uint8_t *buf, uintptr_t len, uintptr_t *out_n)
{
  ...
  FETCHcode result;
  int ret = 0;
  ssize_t nwritten = Fetch_conn_cf_send(io_ctx->cf->next, io_ctx->data,
  ...
  *out_n = (uintptr_t)nwritten;
  FETCH_TRC_CF(io_ctx->data, io_ctx->cf, "cf->next send(len=%zu) -> %zd, %d",
               len, nwritten, result);
  return ret;
}

static ssize_t tls_recv_more(struct Fetch_cfilter *cf,
                             struct Fetch_easy *data, FETCHcode *err)
{
  ...
  else if (io_error)
  {
    ...
    failf(data, "reading from socket: %s",
          Fetch_strerror(io_error, buffer, sizeof(buffer)));
 ...
 */
static ssize_t
cr_recv(struct Fetch_cfilter *cf, struct Fetch_easy *data,
        char *plainbuf, size_t plainlen, FETCHcode *err)
{
  struct ssl_connect_data *const connssl = cf->ctx;
  struct rustls_ssl_backend_data *const backend =
      (struct rustls_ssl_backend_data *)connssl->backend;
...
out:
  FETCH_TRC_CF(data, cf, "cf_recv(len=%zu) -> %zd, %d",
               plainlen, nread, *err);
  return nread;
}

static FETCHcode cr_flush_out(struct Fetch_cfilter *cf, struct Fetch_easy *data,
                              struct rustls_connection *rconn)
{
  ...
  while (rustls_connection_wants_write(rconn))
  {
    ...
    if (io_error == EAGAIN || io_error == EWOULDBLOCK)
    {
      FETCH_TRC_CF(data, cf, "cf_send: EAGAIN after %zu bytes",
                   tlswritten_total);
      return FETCHE_AGAIN;
    }
    else if (io_error)
    ...
    FETCH_TRC_CF(data, cf, "cf_send: wrote %zu TLS bytes", tlswritten);
 ...
 */
static ssize_t
cr_send(struct Fetch_cfilter *cf, struct Fetch_easy *data,
        const void *plainbuf, size_t plainlen, FETCHcode *err)
{
  struct ssl_connect_data *const connssl = cf->ctx;
  struct rustls_ssl_backend_data *const backend =
      (struct rustls_ssl_backend_data *)connssl->backend;
  ...
  FETCH_TRC_CF(data, cf, "cf_send(len=%zu)", plainlen);
  ...
  if (backend->plain_out_buffered)
  {
    *err = cr_flush_out(cf, data, rconn);
    FETCH_TRC_CF(data, cf, "cf_send: flushing %zu previously added bytes -> %d",
  ...
  *err = cr_flush_out(cf, data, rconn);
  if (*err)
  {
    if (FETCHE_AGAIN == *err)
    {
      ...
      FETCH_TRC_CF(data, cf, "cf_send: EAGAIN, remember we added %zu plain"
                             " bytes already to Rustls",
                   blen);
  ...
  FETCH_TRC_CF(data, cf, "cf_send(len=%zu) -> %d, %zd",
               ...
               struct dynbuf *out)
{
  ...
  while (!feof(f))
  {
    ...
    if (rr == 0 ||
        FETCHE_OK != Fetch_dyn_addn(out, buf, rr))
    {
      fclose(f);
      return 0;
    }
...
static void
cr_get_selected_ciphers(struct Fetch_easy *data,
...
static FETCHcode
cr_init_backend(struct Fetch_cfilter *cf, struct Fetch_easy *data,
                struct rustls_ssl_backend_data *const backend)
{
  ...
  else if (ca_info_blob || ssl_cafile)
  {
    ...
    if (conn_config->CRLfile)
    {
      ...
      result = rustls_web_pki_server_cert_verifier_builder_add_crl(
          verifier_builder,
          Fetch_dyn_uptr(&crl_contents),
          Fetch_dyn_len(&crl_contents));
      Fetch_dyn_free(&crl_contents);
      if (result != RUSTLS_RESULT_OK)
      {
...
static void
cr_set_negotiated_alpn(struct Fetch_cfilter *cf, struct Fetch_easy *data,
                       const struct rustls_connection *rconn)
{
  ...
  Fetch_alpn_set_negotiated(cf, data, connssl, protocol, len);
}
 ...
 */
static FETCHcode
cr_connect_common(struct Fetch_cfilter *cf,
                  struct Fetch_easy *data,
                  bool blocking,
                  bool *done)
{
  ...
  struct rustls_ssl_backend_data *const backend =
      (struct rustls_ssl_backend_data *)connssl->backend;
  struct rustls_connection *rconn = NULL;
  FETCHcode tmperr = FETCHE_OK;
  int result;
  int what;
  bool wants_read;
  bool wants_write;
  fetch_socket_t writefd;
  fetch_socket_t readfd;
  timediff_t timeout_ms;
  timediff_t socket_check_timeout;

  DEBUGASSERT(backend);

  FETCH_TRC_CF(data, cf, "cr_connect_common, state=%d", connssl->state);
  *done = FALSE;
  if (!backend->conn)
  {
    result = cr_init_backend(cf, data,
                             (struct rustls_ssl_backend_data *)connssl->backend);
    FETCH_TRC_CF(data, cf, "cr_connect_common, init backend -> %d", result);
    if (result != FETCHE_OK)
    {
      return result;
    }
  ...
  for (;;)
  {
    ...
    if (!rustls_connection_is_handshaking(rconn))
    {
      ...
      if (tmperr == FETCHE_AGAIN)
      {
        connssl->io_need = FETCH_SSL_IO_NEED_SEND;
        return FETCHE_OK;
      }
      else if (tmperr != FETCHE_OK)
      {
        return tmperr;
      }
      /* REALLY Done with the handshake. */
      {
        ...
        if (proto == RUSTLS_TLS_VERSION_TLSV1_2)
          ver = "TLSv1.2";
        Fetch_cipher_suite_get_str(cipher, buf, sizeof(buf), TRUE);
    ...
    writefd = wants_write ? sockfd : FETCH_SOCKET_BAD;
    readfd = wants_read ? sockfd : FETCH_SOCKET_BAD;
    ...
    if (0 == what)
    {
      FETCH_TRC_CF(data, cf, "Fetch_socket_check: %s would block",
                   wants_read && wants_write ? "writing and reading" : wants_write ? "writing"
                                                                                   : "reading");
      if (wants_write)
        connssl->io_need |= FETCH_SSL_IO_NEED_SEND;
      if (wants_read)
    ...
    if (wants_write)
    {
      FETCH_TRC_CF(data, cf, "rustls_connection wants us to write_tls.");
      cr_send(cf, data, NULL, 0, &tmperr);
      if (tmperr == FETCHE_AGAIN)
      {
        FETCH_TRC_CF(data, cf, "writing would block");
        /* fall through */
      }
      else if (tmperr != FETCHE_OK)
      {
    ...
    if (wants_read)
    {
      FETCH_TRC_CF(data, cf, "rustls_connection wants us to read_tls.");
      if (tls_recv_more(cf, data, &tmperr) < 0)
      {
        if (tmperr == FETCHE_AGAIN)
        {
          FETCH_TRC_CF(data, cf, "reading would block");
          /* fall through */
        }
        else if (tmperr == FETCHE_RECV_ERROR)
        {
          return FETCHE_SSL_CONNECT_ERROR;
        }
        else
...
static FETCHcode
cr_connect_nonblocking(struct Fetch_cfilter *cf,
...
static FETCHcode
cr_connect_blocking(struct Fetch_cfilter *cf, struct Fetch_easy *data)
{
  bool done; /* unused */
...
static FETCHcode
cr_shutdown(struct Fetch_cfilter *cf,
            struct Fetch_easy *data,
            bool send_shutdown, bool *done)
{
  ...
  *done = FALSE;
  ...
  if (nwritten < 0)
  {
    ...
    DEBUGASSERT(result);
    FETCH_TRC_CF(data, cf, "shutdown send failed: %d", result);
    goto out;
  }
...
static void
cr_close(struct Fetch_cfilter *cf, struct Fetch_easy *data)
{
  struct ssl_connect_data *connssl = cf->ctx;
  struct rustls_ssl_backend_data *backend =
      (struct rustls_ssl_backend_data *)connssl->backend;
...
static FETCHcode
cr_random(struct Fetch_easy *data, unsigned char *entropy, size_t length)
{
  rustls_result rresult = 0;
  (void)data;
  rresult =
...
const struct Fetch_ssl Fetch_ssl_rustls = {
    {FETCHSSLBACKEND_RUSTLS, "rustls"},
