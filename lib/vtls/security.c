 ...
 * Network.framework. No code but networkfmwk.c or sectransp.c should ever
 ...
 */
bool security_is_file(const char *filename)
{
  struct_stat st;
...
OSStatus security_copy_identity(struct Curl_easy *data,
                                ...
                                SecIdentityRef *identity)
{
  ...
  if(ssl_cert || ssl_cert_blob) {
    ...
    bool is_cert_file = (!is_cert_data) && security_is_file(ssl_cert);
...
CURLcode security_copy_cert_subject(struct Curl_easy *data,
                                     ...
                                     CFMutableArrayRef array)
{
  ...
  result = security_copy_cert_subject(data, cacert, &certp);
  switch(result) {
    case CURLE_OK:
      break;
...
CURLcode security_collect_cert_single(struct Curl_cfilter *cf,
                                      ...
                                      SecCertificateRef cert, CFIndex idx)
{
  ...
  if(data->set.verbose) {
    char *certp;
    result = security_copy_cert_subject(data, cert, &certp);
    if(!result) {
...
CURLcode security_collect_cert_trust(struct Curl_cfilter *cf,
                                     struct Curl_easy *data,
                                     SecTrustRef trust)
{
  CURLcode result = CURLE_OK;
  ...
  if(ssl_config->certinfo) {
    ...
    for(i = 0L ; !result && (i < count) ; i++) {
      ...
      result = security_collect_cert_single(cf, data, server_cert, i);
    }
  }
...
#endif /* defined(USE_SECTRANSP) || defined(USE_NETWORKFMWK) */
