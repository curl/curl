...
#endif /* HAVE_EXCLUSIVE_TRUST_MODE */
...
#ifdef HAVE_EXCLUSIVE_TRUST_MODE
  ca_file = conn->ssl_config.CAfile;
  if(result == CURLE_OK && ca_file && BACKEND->use_manual_cred_validation) {
    /*
    ...
    if(!trust_store) {
      ...
      result = add_certs_to_store(trust_store, ca_file, conn);
    }
