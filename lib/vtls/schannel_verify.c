...
/* Search the substring (sub, size_sub) into string (str, str_limit)
 * String don't need to be terminated by a '\0'.
 * Similar of memmem (not available on Visual Studio).
 * Return position of beginning of first occurence or NULL if not found
 */
static const char *c_memstr(const char *str, const char *str_limit,
                            const char *sub, size_t size_sub)
{
  const char *p;
  char first;
  if((str + size_sub) > str_limit)
    return NULL;
  if(size_sub == 0)
    return str;
  first = *sub;
  for(p = str; p <= (str_limit - size_sub); p++)
    if(((*p) == first) && (memcmp(p, sub, size_sub) == 0))
      return p;
                                        ...
                                        struct Curl_easy *data)
{
  ...
  while(more_certs && (current_ca_file_ptr<ca_buffer_limit)) {
    const char *begin_cert_ptr = c_memstr(current_ca_file_ptr,
                                          ca_buffer_limit,
                                          BEGIN_CERT,
                                          begin_cert_len);
    if(!begin_cert_ptr || !is_cr_or_lf(begin_cert_ptr[begin_cert_len])) {
      ...
      const char *end_cert_ptr = c_memstr(begin_cert_ptr,
                                          ca_buffer_limit,
                                          END_CERT,
                                          begin_cert_len);
      if(!end_cert_ptr) {
