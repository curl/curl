                                        ...
                                        struct Curl_easy *data)
{
  ...
  err = security_copy_identity(data, ssl_config, &cert_and_key);
  if(cert_and_key) {
  ...
  */
  ...
  if((ssl_cafile || ssl_cablob) && verifypeer) {
    ...
    bool is_cert_file = (!is_cert_data) && security_is_file(ssl_cafile);
...
#pragma unused(count)
  ...
  if(err == noErr && trust) {
    security_collect_cert_trust(cf, data, trust);
  }
#else
