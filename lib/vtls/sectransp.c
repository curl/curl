                               ...
                               size_t *dataLength)  /* IN/OUT */
{
  ...
  CURL_LOG_CF(data, cf, "bio_read(len=%zu) -> %zd, result=%d",
                                 ...
                                 size_t *dataLength)  /* IN/OUT */
{
  ...
  CURL_LOG_CF(data, cf, "bio_send(len=%zu) -> %zd, result=%d",
                                        ...
                                        struct Curl_easy *data)
{
  ...
  CURL_LOG_CF(data, cf, "connect_step1");
  GetDarwinVersionNumber(&darwinver_maj, &darwinver_min);
#endif /* CURL_BUILD_MAC */
                                ...
                                SSLContextRef ctx)
{
  ...
  while(offset < buflen) {
    ...
    if(res == 0 && offset == 0) {
      ...
      if(rc != CURLE_OK) {
        CURL_LOG_CF(data, cf, "append_cert for CA failed");
        result = rc;
        goto out;
      }
      break;
  ...
  CURL_LOG_CF(data, cf, "setting %d trust anchors", n);
  ...
  switch(trust_eval) {
    case kSecTrustResultUnspecified:
      ...
      CURL_LOG_CF(data, cf, "trust result: Unspecified");
      result = CURLE_OK;
      goto out;
    case kSecTrustResultProceed:
      CURL_LOG_CF(data, cf, "trust result: Proceed");
      result = CURLE_OK;
      goto out;
                            ...
                            SSLContextRef ctx)
{
  ...
  if(ca_info_blob) {
    CURL_LOG_CF(data, cf, "verify_peer, CA from config blob");
    ...
    CURL_LOG_CF(data, cf, "verify_peer, CA from file '%s'", cafile);
                                        ...
                                        struct Curl_easy *data)
{
  ...
  DEBUGASSERT(backend);
  CURL_LOG_CF(data, cf, "connect_step2");
                                        ...
                                        struct Curl_easy *data)
{
  ...
  CURL_LOG_CF(data, cf, "connect_step3");
  /* There is no step 3!
