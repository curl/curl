...
void Curl_dnscache_prune(struct Curl_easy *data)
{
  ...
  do {
    ...
    timediff_t oldest_ms =
      dnscache_prune(&dnscache->entries, timeout_ms, data->state.now);
...
clean_up:
  ...
  if(prev_alarm) {
    ...
    timediff_t elapsed_secs = curlx_timediff_ms(data->state.now,
