...
#ifdef USE_SSL
struct curl_trc_feat Curl_trc_feat_scache = {
  "SCACHE",
  CURL_LOG_LVL_NONE,
};

void Curl_trc_scache(struct Curl_easy *data, const char *fmt, ...)
{
  DEBUGASSERT(!strchr(fmt, '\n'));
  if(Curl_trc_ft_is_verbose(data, &Curl_trc_feat_scache)) {
    va_list ap;
    va_start(ap, fmt);
    trc_infof(data, &Curl_trc_feat_scache, fmt, ap);
    va_end(ap);
  }
}
#endif /* USE_SSL */
...
#endif
#ifdef USE_SSL
  { &Curl_trc_feat_scache,    TRC_CT_NETWORK },
#endif
#if !defined(CURL_DISABLE_WEBSOCKETS) && !defined(CURL_DISABLE_HTTP)
