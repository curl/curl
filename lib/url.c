...
CURLcode Curl_disconnect(struct connectdata *conn, bool dead_connection)
{
  ...
  /* encoded proxy name buffers, must be freed with idn_free() since this was
     allocated by libidn */
  if(conn->http_proxy.host.encalloc)
    idn_free(conn->http_proxy.host.encalloc);
  if(conn->socks_proxy.host.encalloc)
    idn_free(conn->socks_proxy.host.encalloc);

#elif defined(USE_WIN32_IDN)
  free(conn->host.encalloc); /* encoded host name buffer, must be freed with
                                free() since this was allocated by
                                curl_win32_idn_to_ascii */

  /* encoded proxy name buffers, must be freed with free() since this was
     allocated by curl_win32_idn_to_ascii */
  free(conn->http_proxy.host.encalloc);
  free(conn->socks_proxy.host.encalloc);
#endif
                   ...
                   const struct proxy_info* needle)
{
  if((data->proxytype == needle->proxytype) &&
     ...
     Curl_safe_strequal(data->host.name, needle->host.name) &&
     Curl_safe_strequal(data->user, needle->user) &&
     Curl_safe_strequal(data->passwd, needle->passwd))
    return TRUE;
                            ...
                            curl_proxytype proxytype)
{
  ...
  if(prox_portno) {
    *prox_portno = 0x0; /* cut off number from host name */
    ...
    port = strtol(prox_portno, NULL, 10);
  }
  else {
    if(proxyptr[0]=='/')
  ...
  if(*proxyptr) {
    struct proxy_info *proxyinfo = sockstype ? &conn->socks_proxy :
                                               &conn->http_proxy;
                                 ...
                                 struct connectdata *conn)
{
  ...
  conn->http_proxy.user = curl_easy_unescape(data, proxyuser, 0, NULL);
  if(!conn->http_proxy.user)
    return CURLE_OUT_OF_MEMORY;

  conn->http_proxy.passwd = curl_easy_unescape(data, proxypasswd, 0, NULL);
