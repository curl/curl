...
CURLcode curl_easy_perform(CURL *curl)
{
  ...
  if(CURL_API_EASY_ENTER(&guard, curl, "curl_easy_perform", &result)) {
...
CURLcode curl_easy_perform_ev(struct Curl_easy *easy)
{
  ...
  if(CURL_API_EASY_ENTER(&guard, easy, "curl_easy_perform_ev", &result)) {
...
void curl_easy_cleanup(CURL *curl)
{
  ...
  if(CURL_API_EASY_CLEANUP(&guard, curl)) {
...
CURLcode curl_easy_getinfo(CURL *curl, CURLINFO info, ...)
{
  ...
  if(CURL_API_EASY_REC_ENTER(&guard, curl, "curl_easy_getinfo", &result)) {
    struct Curl_easy *data = curl;
    va_list arg;
    void *paramp;
...
CURL *curl_easy_duphandle(CURL *curl)
{
  ...
  if(CURL_API_EASY_REC_ENTER(&guard, curl, "curl_easy_duphandle", NULL)) {
    struct Curl_easy *data = curl;
...
void curl_easy_reset(CURL *curl)
{
  ...
  if(CURL_API_EASY_ENTER(&guard, curl, "curl_easy_reset", NULL)) {
    struct Curl_easy *data = curl;
...
CURLcode curl_easy_recv(CURL *curl, void *buffer, size_t buflen, size_t *n)
{
  ...
  if(CURL_API_EASY_ENTER(&guard, curl, "curl_easy_recv", &result)) {
                        ...
                        size_t *n)
{
  ...
  if(CURL_API_EASY_ENTER(&guard, curl, "curl_easy_send", &result)) {
    struct Curl_easy *data = curl;
    size_t written = 0;
...
CURLcode curl_easy_upkeep(CURL *curl)
{
  ...
  if(CURL_API_EASY_ENTER(&guard, curl, "curl_easy_upkeep", &result)) {
                               ...
                               const unsigned char *sdata, size_t sdata_len)
{
  ...
  if(CURL_API_EASY_ENTER(&guard, curl, "curl_easy_ssls_import", &result)) {
                               ...
                               void *userptr)
{
  ...
  if(CURL_API_EASY_ENTER(&guard, curl, "curl_easy_ssls_export", &result)) {
