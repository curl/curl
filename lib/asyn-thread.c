...
{
  return &(data->resolve_async.tdata->tsd);
}
                                ...
                                const struct addrinfo *hints)
{
  ...
  int err = ENOMEM;
  struct Curl_async *asp = &data->resolve_async;

  data->resolve_async.tdata = td;
  if(!td)
    goto errno_exit;
                                   ...
                                   bool report)
{
  ...
  data->resolve_async.done = TRUE;

  if(entry)
    *entry = data->resolve_async.dns;

  if(!data->resolve_async.dns && report)
...
{
  struct thread_data *td = data->resolve_async.tdata;
...
int Curl_resolver_getsock(struct Curl_easy *data, curl_socket_t *socks)
{
  ...
  timediff_t milli;
  timediff_t ms;
  struct resdata *reslv = (struct resdata *)data->resolve_async.resolver;
#ifndef CURL_DISABLE_SOCKETPAIR
  struct thread_data *td = data->resolve_async.tdata;
#else
  (void)socks;
