...
#ifdef HAVE_SOCKETPAIR
  /* close socket pair */
  if(tsd->sock_pair[0] != INVALID_SOCKET) {
    close(tsd->sock_pair[0]);
  }

  if(tsd->sock_pair[1] != INVALID_SOCKET) {
                           ...
                           const struct addrinfo *hints)
{
  ...
  if(socketpair(AF_LOCAL, SOCK_STREAM, 0, &tsd->sock_pair[0]) < 0) {
    tsd->sock_pair[0] = INVALID_SOCKET;
    tsd->sock_pair[1] = INVALID_SOCKET;
    goto err_exit;
  }
#endif
...
#ifdef HAVE_SOCKETPAIR
  memset(socks, INVALID_SOCKET, sizeof(curl_socket_t) * numsocks);
  if(td != NULL) {
