...
static char user_flag(unsigned char flag)
{
...
static char passwd_flag(unsigned char flag)
{
  unsigned char pwd_flag = 0x40;
  flag |= pwd_flag;
  return flag;
}
...
static CURLcode mqtt_connect(struct Curl_easy *data)
{
  ...
  /*if user was provided then add it to the packet*/
  if(ulen > 0)
