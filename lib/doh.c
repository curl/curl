...
static void doh_master_dtor(void *key, size_t klen, void *e)
{
  (void)key;
  (void)klen;
  if(e) {
    struct doh_probes *dohp = e;
    int i;
    for(i = 0; i < DOH_SLOT_COUNT; ++i) {
...
{
  struct doh_probes *doh = Curl_meta_get(data, CURL_EZM_DOH_MASTER);
  if(doh && data->multi) {
    ...
    for(slot = 0; slot < DOH_SLOT_COUNT; slot++) {
      ...
      /* Curl_dyn_free(&doh->probe[slot].resp_body); */
...
void Curl_doh_cleanup(struct Curl_easy *data)
{
  ...
  if(dohp) {
    Curl_doh_close(data);
    Curl_meta_clear(data, CURL_EZM_DOH_MASTER);
  }
}
