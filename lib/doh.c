...
struct fetch_trc_feat Fetch_doh_trc = {
    "DoH",
    FETCH_LOG_LVL_NONE,
};
...
doh_write_cb(char *contents, size_t size, size_t nmemb, void *userp)
{
  ...
  if (Fetch_dyn_addn(mem, contents, realsize))
    return 0;
...
static void doh_print_buf(struct Fetch_easy *data,
                          const char *prefix,
                          unsigned char *buf, size_t len)
{
  ...
  if (len > (LOCAL_PB_HEXMAX / 2))
    truncated = TRUE;
  Fetch_hexencode(buf, len, hexstr, hlen);
  if (!truncated)
...
static int doh_done(struct Fetch_easy *doh, FETCHcode result)
{
  struct Fetch_easy *data; /* the transfer that asked for the DoH probe */

  data = Fetch_multi_get_handle(doh->multi, doh->set.dohfor_mid);
  if (!data)
  {
    DEBUGF(infof(doh, "doh_done: xfer for mid=%" FMT_OFF_T " not found", doh->set.dohfor_mid));
    DEBUGASSERT(0);
  }
  else
  {
    ...
    if (result)
      infof(doh, "DoH request %s", fetch_easy_strerror(result));
...
#define ERROR_CHECK_SETOPT(x, y)                    \
  do                                                \
  {                                                 \
    result = fetch_easy_setopt((FETCH *)doh, x, y); \
    if (result &&                                   \
...
static FETCHcode doh_run_probe(struct Fetch_easy *data,
                               struct doh_probe *p, DNStype dnstype,
                               const char *host,
                               const char *url, FETCHM *multi,
                               struct fetch_slist *headers)
{
  struct Fetch_easy *doh = NULL;
  ...
  p->dnstype = dnstype;
  Fetch_dyn_init(&p->resp_body, DYN_DOH_RESPONSE);
  ...
  /* Fetch_open() is the internal version of fetch_easy_init() */
  result = Fetch_open(&doh);
  if (result)
    goto error;
...
#ifndef FETCH_DISABLE_VERBOSE_STRINGS
  doh->state.feat = &Fetch_doh_trc;
#endif
  ERROR_CHECK_SETOPT(FETCHOPT_URL, url);
  ERROR_CHECK_SETOPT(FETCHOPT_DEFAULT_PROTOCOL, "https");
  ERROR_CHECK_SETOPT(FETCHOPT_WRITEFUNCTION, doh_write_cb);
  ERROR_CHECK_SETOPT(FETCHOPT_WRITEDATA, &p->resp_body);
  ERROR_CHECK_SETOPT(FETCHOPT_POSTFIELDS, p->req_body);
  ERROR_CHECK_SETOPT(FETCHOPT_POSTFIELDSIZE, (long)p->req_body_len);
  ERROR_CHECK_SETOPT(FETCHOPT_HTTPHEADER, headers);
#ifdef USE_HTTP2
  ERROR_CHECK_SETOPT(FETCHOPT_HTTP_VERSION, FETCH_HTTP_VERSION_2TLS);
  ERROR_CHECK_SETOPT(FETCHOPT_PIPEWAIT, 1L);
#endif
#ifndef DEBUGBUILD
  /* enforce HTTPS if not debug */
  ERROR_CHECK_SETOPT(FETCHOPT_PROTOCOLS, FETCHPROTO_HTTPS);
#else
  /* in debug mode, also allow http */
  ERROR_CHECK_SETOPT(FETCHOPT_PROTOCOLS, FETCHPROTO_HTTP | FETCHPROTO_HTTPS);
#endif
  ERROR_CHECK_SETOPT(FETCHOPT_TIMEOUT_MS, (long)timeout_ms);
  ERROR_CHECK_SETOPT(FETCHOPT_SHARE, (FETCHSH *)data->share);
  if (data->set.err && data->set.err != stderr)
    ERROR_CHECK_SETOPT(FETCHOPT_STDERR, data->set.err);
  if (Fetch_trc_ft_is_verbose(data, &Fetch_doh_trc))
    ERROR_CHECK_SETOPT(FETCHOPT_VERBOSE, 1L);
  if (data->set.no_signal)
    ERROR_CHECK_SETOPT(FETCHOPT_NOSIGNAL, 1L);

  ERROR_CHECK_SETOPT(FETCHOPT_SSL_VERIFYHOST,
                     data->set.doh_verifyhost ? 2L : 0L);
  ERROR_CHECK_SETOPT(FETCHOPT_SSL_VERIFYPEER,
                     data->set.doh_verifypeer ? 1L : 0L);
  ERROR_CHECK_SETOPT(FETCHOPT_SSL_VERIFYSTATUS,
  ...
  /* Inherit *some* SSL options from the user's transfer. This is a
     ...
     FETCHOPT_DOH_PROXY_SSL_VERIFYHOST and FETCHOPT_DOH_PROXY_SSL_VERIFYPEER.
     */
  if (data->set.ssl.falsestart)
    ERROR_CHECK_SETOPT(FETCHOPT_SSL_FALSESTART, 1L);
  if (data->set.str[STRING_SSL_CAFILE])
  {
    ERROR_CHECK_SETOPT(FETCHOPT_CAINFO,
                       data->set.str[STRING_SSL_CAFILE]);
  }
  if (data->set.blobs[BLOB_CAINFO])
  {
    ERROR_CHECK_SETOPT(FETCHOPT_CAINFO_BLOB,
                       data->set.blobs[BLOB_CAINFO]);
  }
  if (data->set.str[STRING_SSL_CAPATH])
  {
    ERROR_CHECK_SETOPT(FETCHOPT_CAPATH,
                       data->set.str[STRING_SSL_CAPATH]);
  }
  if (data->set.str[STRING_SSL_CRLFILE])
  {
    ERROR_CHECK_SETOPT(FETCHOPT_CRLFILE,
                       data->set.str[STRING_SSL_CRLFILE]);
  }
  if (data->set.ssl.certinfo)
    ERROR_CHECK_SETOPT(FETCHOPT_CERTINFO, 1L);
  if (data->set.ssl.fsslctx)
    ERROR_CHECK_SETOPT(FETCHOPT_SSL_CTX_FUNCTION, data->set.ssl.fsslctx);
  if (data->set.ssl.fsslctxp)
    ERROR_CHECK_SETOPT(FETCHOPT_SSL_CTX_DATA, data->set.ssl.fsslctxp);
  if (data->set.fdebug)
    ERROR_CHECK_SETOPT(FETCHOPT_DEBUGFUNCTION, data->set.fdebug);
  if (data->set.debugdata)
    ERROR_CHECK_SETOPT(FETCHOPT_DEBUGDATA, data->set.debugdata);
  if (data->set.str[STRING_SSL_EC_CURVES])
  {
    ERROR_CHECK_SETOPT(FETCHOPT_SSL_EC_CURVES,
                       data->set.str[STRING_SSL_EC_CURVES]);
  }

  {
    long mask =
        (data->set.ssl.enable_beast ? FETCHSSLOPT_ALLOW_BEAST : 0) |
        (data->set.ssl.no_revoke ? FETCHSSLOPT_NO_REVOKE : 0) |
        (data->set.ssl.no_partialchain ? FETCHSSLOPT_NO_PARTIALCHAIN : 0) |
        (data->set.ssl.revoke_best_effort ? FETCHSSLOPT_REVOKE_BEST_EFFORT : 0) |
        (data->set.ssl.native_ca_store ? FETCHSSLOPT_NATIVE_CA : 0) |
        (data->set.ssl.auto_client_cert ? FETCHSSLOPT_AUTO_CLIENT_CERT : 0);

    (void)fetch_easy_setopt(doh, FETCHOPT_SSL_OPTIONS, mask);
  }
  ...
  /* DoH handles must not inherit private_data. The handles may be passed to
     ...
     private_data via FETCHOPT_PRIVATE if they so choose. */
  DEBUGASSERT(!doh->set.private_data);

  if (fetch_multi_add_handle(multi, doh))
    goto error;
...
/*
 * Fetch_doh() resolves a name using DoH. It resolves a name and returns a
 * 'Fetch_addrinfo *' with the address information.
 */
...
#ifdef USE_IPV6
  if ((conn->ip_version != FETCH_IPRESOLVE_V4) && Fetch_ipv6works(data))
  {
                               ...
                               struct dohentry *d, uint16_t len)
{
  ...
  if (d->numhttps_rrs < DOH_MAX_HTTPS)
  {
    ...
    h->val = Fetch_memdup(&doh[index], len);
    if (!h->val)
                               ...
                               unsigned int index, struct dohentry *d)
{
  ...
  do
  {
    ...
    if (length)
    {
      if (Fetch_dyn_len(c))
      {
        if (Fetch_dyn_addn(c, STRCONST(".")))
          return DOH_OUT_OF_MEM;
      }
      if ((index + length) > dohlen)
        return DOH_DNS_BAD_LABEL;

      if (Fetch_dyn_addn(c, &doh[index], length))
        return DOH_OUT_OF_MEM;
      index += length;
    }
  } while (length && --loop);
...
UNITTEST void de_init(struct dohentry *de)
{
  ...
  for (i = 0; i < DOH_MAX_CNAME; i++)
    Fetch_dyn_init(&de->cname[i], DYN_DOH_CNAME);
}
...
#ifndef FETCH_DISABLE_VERBOSE_STRINGS
static void doh_show(struct Fetch_easy *data,
                     const struct dohentry *d)
{
  ...
  {
    infof(data, "CNAME: %s", Fetch_dyn_ptr(&d->cname[i]));
  }
}
#else
#define doh_show(x, y)
 ...
 * Fetch_addrinfo struct linked list filled with the data from a set of DoH
 * lookups. Fetch_addrinfo is meant to work like the addrinfo struct does for
...
static FETCHcode doh2ai(const struct dohentry *de, const char *hostname,
                        int port, struct Fetch_addrinfo **aip)
{
  ...
  for (i = 0; i < de->numaddr; i++)
  {
    ...
    ai = calloc(1, sizeof(struct Fetch_addrinfo) + ss_size + hostlen);
    if (!ai)
    {
    ...
    case AF_INET:
      ...
      addr->sin_family = (FETCH_SA_FAMILY_T)addrtype;
    ...
    case AF_INET6:
      ...
      addr6->sin6_family = (FETCH_SA_FAMILY_T)addrtype;
...
UNITTEST void de_cleanup(struct dohentry *d)
{
  ...
  for (i = 0; i < d->numcname; i++)
  {
    Fetch_dyn_free(&d->cname[i]);
  }
#ifdef USE_HTTPSRR
  for (i = 0; i < d->numhttps_rrs; i++)
    Fetch_safefree(d->https_rrs[i].val);
#endif
}
 ...
 */
static FETCHcode doh_decode_rdata_name(unsigned char **buf, size_t *remaining,
                                       char **dnsname)
{
  ...
  if (rem <= 0)
  {
    Fetch_dyn_free(&thename);
    return FETCHE_OUT_OF_MEMORY;
  }
  Fetch_dyn_init(&thename, FETCH_MAXLEN_host_name);
  cp = *buf;
  clen = *cp++;
  if (clen == 0)
  {
    /* special case - return "." as name */
    if (Fetch_dyn_addn(&thename, ".", 1))
      return FETCHE_OUT_OF_MEMORY;
  }
  while (clen)
  {
    ...
    if (Fetch_dyn_addn(&thename, cp, clen) ||
        Fetch_dyn_addn(&thename, ".", 1))
  ...
  *buf = cp;
  *remaining = rem - 1;
  *dnsname = Fetch_dyn_ptr(&thename);
  return FETCHE_OK;
}

#ifdef DEBUGBUILD
static FETCHcode doh_test_alpn_escapes(void)
{
  ...
  if (Fetch_httpsrr_decode_alpn(example, example_len, aval) != FETCHE_OK)
...
static FETCHcode doh_resp_decode_httpsrr(unsigned char *cp, size_t len,
                                         struct Fetch_https_rrinfo **hrr)
{
  uint16_t pcode = 0, plen = 0;
  struct Fetch_https_rrinfo *lhrr = NULL;
  char *dnsname = NULL;

#ifdef DEBUGBUILD
  /* a few tests of escaping, should not be here but ok for now */
  if (doh_test_alpn_escapes() != FETCHE_OK)
    return FETCHE_OUT_OF_MEMORY;
#endif
  if (len <= 2)
    return FETCHE_BAD_FUNCTION_ARGUMENT;
  lhrr = calloc(1, sizeof(struct Fetch_https_rrinfo));
  if (!lhrr)
    return FETCHE_OUT_OF_MEMORY;
  lhrr->priority = doh_get16bit(cp, 0);
  cp += 2;
  len -= 2;
  if (doh_decode_rdata_name(&cp, &len, &dnsname) != FETCHE_OK)
    goto err;
  lhrr->target = dnsname;
  ...
  while (len >= 4)
  {
    ...
    case HTTPS_RR_CODE_ALPN:
      if (Fetch_httpsrr_decode_alpn(cp, plen, lhrr->alpns) != FETCHE_OK)
        goto err;
      break;
    ...
    case HTTPS_RR_CODE_IPV4:
      if (!plen)
        goto err;
      lhrr->ipv4hints = Fetch_memdup(cp, plen);
      if (!lhrr->ipv4hints)
        goto err;
    ...
    case HTTPS_RR_CODE_ECH:
      if (!plen)
        goto err;
      lhrr->echconfiglist = Fetch_memdup(cp, plen);
    ...
    case HTTPS_RR_CODE_IPV6:
      if (!plen)
        goto err;
      lhrr->ipv6hints = Fetch_memdup(cp, plen);
      if (!lhrr->ipv6hints)
        goto err;
  ...
  *hrr = lhrr;
  return FETCHE_OK;
err:
  Fetch_safefree(lhrr->target);
  Fetch_safefree(lhrr->echconfiglist);
  Fetch_safefree(lhrr);
  return FETCHE_OUT_OF_MEMORY;
}

#ifdef DEBUGBUILD
static void doh_print_httpsrr(struct Fetch_easy *data,
                              struct Fetch_https_rrinfo *hrr)
{
  DEBUGASSERT(hrr);
                               ...
                               struct Fetch_dns_entry **dnsp)
{
  ...
  *dnsp = NULL; /* defaults to no response */
      ...
      dohp->probe[DOH_SLOT_IPV6].easy_mid < 0)
  {
    ...
    return CONN_IS_PROXIED(data->conn) ? FETCHE_COULDNT_RESOLVE_PROXY : FETCHE_COULDNT_RESOLVE_HOST;
  }
  else if (!dohp->pending)
  {
    ...
    for (slot = 0; slot < DOH_SLOT_COUNT; slot++)
    {
      ...
      if (!p->dnstype)
        continue;
      rc[slot] = doh_resp_decode(Fetch_dyn_uptr(&p->resp_body),
                                 Fetch_dyn_len(&p->resp_body),
                                 p->dnstype, &de);
      Fetch_dyn_free(&p->resp_body);
#ifndef FETCH_DISABLE_VERBOSE_STRINGS
      if (rc[slot])
      {
    ...
    result = FETCHE_COULDNT_RESOLVE_HOST; /* until we know better */
    if (!rc[DOH_SLOT_IPV4] || !rc[DOH_SLOT_IPV6])
    {
      ...
      dns = Fetch_cache_addr(data, ai, dohp->host, 0, dohp->port, FALSE);
      ...
      else
      {
        ...
        *dnsp = dns;
        result = FETCHE_OK; /* address resolution OK */
      }
    } /* address processing done */

    /* Now process any build-specific attributes retrieved from DNS */
#ifdef USE_HTTPSRR
    if (de.numhttps_rrs > 0 && result == FETCHE_OK && *dnsp)
    {
      struct Fetch_https_rrinfo *hrr = NULL;
...
void Fetch_doh_close(struct Fetch_easy *data)
{
  ...
  if (doh && data->multi)
  {
    ...
    for (slot = 0; slot < DOH_SLOT_COUNT; slot++)
    {
      ...
      probe_data = data->multi ? Fetch_multi_get_handle(data->multi, mid) : NULL;
      if (!probe_data)
      {
        DEBUGF(infof(data, "Fetch_doh_close: xfer for mid=%" FMT_OFF_T " not found!",
                     doh->probe[slot].easy_mid));
        continue;
      }
      /* data->multi might already be reset at this time */
      fetch_multi_remove_handle(data->multi, probe_data);
      Fetch_close(&probe_data);
    }
  }
}
...
void Fetch_doh_cleanup(struct Fetch_easy *data)
{
  ...
  {
    Fetch_doh_close(data);
    fetch_slist_free_all(doh->req_hds);
    data->req.doh->req_hds = NULL;
    Fetch_safefree(data->req.doh);
  }
}
