...
#include <fetch_base64.h> /* for base64 encoding/decoding */
...
#define sftp_libssh2_realpath(s, p, t, m)                     \
  libssh2_sftp_symlink_ex((s), (p), fetchx_uztoui(strlen(p)), \
...
static FETCHcode ssh_force_knownhost_key_type(struct Fetch_easy *data);
static FETCHcode ssh_connect(struct Fetch_easy *data, bool *done);
static FETCHcode ssh_multi_statemach(struct Fetch_easy *data, bool *done);
static FETCHcode ssh_do(struct Fetch_easy *data, bool *done);
static FETCHcode scp_done(struct Fetch_easy *data, FETCHcode c, bool premature);
...
static FETCHcode sftp_done(struct Fetch_easy *data, FETCHcode, bool premature);
static FETCHcode sftp_doing(struct Fetch_easy *data, bool *dophase_done);
static FETCHcode sftp_disconnect(struct Fetch_easy *data,
                                 struct connectdata *conn, bool dead);
static FETCHcode sftp_perform(struct Fetch_easy *data, bool *connected,
                              bool *dophase_done);
static int ssh_getsock(struct Fetch_easy *data, struct connectdata *conn,
                       fetch_socket_t *sock);
static FETCHcode ssh_setup_connection(struct Fetch_easy *data,
                                      struct connectdata *conn);
static void ssh_attach(struct Fetch_easy *data, struct connectdata *conn);
...
kbd_callback(const char *name, int name_len, const char *instruction,
             ...
             void **abstract)
{
  struct Fetch_easy *data = (struct Fetch_easy *)*abstract;

#ifdef FETCH_LIBSSH2_DEBUG
...
#endif /* FETCH_LIBSSH2_DEBUG */
  if (num_prompts == 1)
  {
    struct connectdata *conn = data->conn;
    responses[0].text = strdup(conn->passwd);
    responses[0].length =
        responses[0].text == NULL ? 0 : fetchx_uztoui(strlen(conn->passwd));
  }
  (void)prompts;
} /* kbd_callback */

static FETCHcode sftp_libssh2_error_to_FETCHE(unsigned long err)
{
  switch (err)
  {
  case LIBSSH2_FX_OK:
  ...
  case LIBSSH2_FX_QUOTA_EXCEEDED:
    return FETCHE_REMOTE_DISK_FULL;
...
static FETCHcode libssh2_session_error_to_FETCHE(int err)
{
  switch (err)
  {
...
/* These functions are made to use the libfetch memory functions - NOT the
...
static LIBSSH2_ALLOC_FUNC(my_libssh2_malloc)
{
  ...
  return Fetch_cmalloc(count);
}

static LIBSSH2_REALLOC_FUNC(my_libssh2_realloc)
{
  (void)abstract; /* arg not used */
  return Fetch_crealloc(ptr, count);
}
...
static void state(struct Fetch_easy *data, sshstate nowstate)
{
  struct connectdata *conn = data->conn;
...
static int sshkeycallback(FETCH *easy,
                          ...
                          void *clientp)
{
  ...
  return (match != FETCHKHMATCH_OK) ? FETCHKHSTAT_REJECT : FETCHKHSTAT_FINE;
}
#endif
...
#endif
static enum fetch_khtype convert_ssh2_keytype(int sshkeytype)
{
  enum fetch_khtype keytype = FETCHKHTYPE_UNKNOWN;
  switch (sshkeytype)
  {
  case LIBSSH2_HOSTKEY_TYPE_RSA:
    keytype = FETCHKHTYPE_RSA;
    break;
  case LIBSSH2_HOSTKEY_TYPE_DSS:
    keytype = FETCHKHTYPE_DSS;
    break;
#ifdef LIBSSH2_HOSTKEY_TYPE_ECDSA_256
  case LIBSSH2_HOSTKEY_TYPE_ECDSA_256:
    keytype = FETCHKHTYPE_ECDSA;
    break;
#endif
...
static FETCHcode ssh_knownhost(struct Fetch_easy *data)
{
  int sshkeytype = 0;
  size_t keylen = 0;
  int rc = 0;
  ...
  if (data->set.str[STRING_SSH_KNOWNHOSTS])
  {
    ...
    if (remotekey)
    {
      ...
      else
      {
         ...
         * fetch_khmatch enum are ever modified, we need to introduce a
         * translation table here!
         */
        keymatch = (enum fetch_khmatch)keycheck;
    ...
    case FETCHKHSTAT_REJECT:
      state(data, SSH_SESSION_FREE);
      FALLTHROUGH();
    case FETCHKHSTAT_DEFER:
      /* DEFER means bail out but keep the SSH_HOSTKEY state */
      result = sshc->actualcode = FETCHE_PEER_FAILED_VERIFICATION;
      break;
    case FETCHKHSTAT_FINE_REPLACE:
      ...
      if (keycheck != LIBSSH2_KNOWNHOST_CHECK_MATCH)
      {
        ...
        if (addrc)
          infof(data, "WARNING: adding the known host %s failed",
                conn->host.name);
        else if (rc == FETCHKHSTAT_FINE_ADD_TO_FILE ||
                 rc == FETCHKHSTAT_FINE_REPLACE)
        {
...
static FETCHcode ssh_check_fingerprint(struct Fetch_easy *data)
{
  struct connectdata *conn = data->conn;
  struct ssh_conn *sshc = &conn->proto.sshc;
  ...
  if (pubkey_sha256)
  {
    ...
    if (hostkey)
    {
      if (!Fetch_sha256it(hash, (const unsigned char *)hostkey, len))
    ...
    if (!fingerprint)
    {
      failf(data,
            ...
            "not available");
      state(data, SSH_SESSION_FREE);
      sshc->actualcode = FETCHE_PEER_FAILED_VERIFICATION;
      return sshc->actualcode;
    }

    /* The length of fingerprint is 32 bytes for SHA256.
     * See libssh2_hostkey_hash documentation. */
    if (Fetch_base64_encode(fingerprint, 32, &fingerprint_b64,
                           &fingerprint_b64_len) != FETCHE_OK)
    {
      state(data, SSH_SESSION_FREE);
      sshc->actualcode = FETCHE_PEER_FAILED_VERIFICATION;
      return sshc->actualcode;
  ...
  if (sshc->kh && !data->set.str[STRING_SSH_HOST_PUBLIC_KEY_MD5])
  {
    ...
    if (found)
    {
      ...
      if (rc)
      {
        ...
        result = libssh2_session_error_to_FETCHE(rc);
      }
    }
    else
    {
...
static FETCHcode sftp_quote(struct Fetch_easy *data,
...
static FETCHcode
sftp_upload_init(struct Fetch_easy *data,
                 struct ssh_conn *sshc,
                 struct SSHPROTO *sshp,
                 bool *blockp)
{
  ...
  if (data->state.resume_from)
  {
    ...
    if (data->state.resume_from < 0)
    {
      int rc = libssh2_sftp_stat_ex(sshc->sftp_session, sshp->path,
                                    fetchx_uztoui(strlen(sshp->path)),
      ...
      {
        fetch_off_t size = attrs.filesize;
        if (size < 0)
        {
  ...
  if (!sshc->sftp_handle)
  {
    ...
    if (sshc->secondCreateDirs)
    {
      ...
      sshc->actualcode = sftperr != LIBSSH2_FX_OK ? sftp_libssh2_error_to_FETCHE(sftperr) : FETCHE_SSH;
    ...
    if (!sshc->actualcode)
    {
      /* Sometimes, for some reason libssh2_sftp_last_error() returns zero
         ...
         work around that! */
      sshc->actualcode = FETCHE_SSH;
      sftperr = LIBSSH2_FX_OK;
    }
    failf(data, "Upload failed: %s (%lu/%d)",
          sftperr != LIBSSH2_FX_OK ? sftp_libssh2_strerror(sftperr) : "ssh error",
          sftperr, rc);
    return sshc->actualcode;
  }
...
static FETCHcode
sftp_pkey_init(struct Fetch_easy *data,
               struct ssh_conn *sshc)
{
  /*
...
static FETCHcode
sftp_quote_stat(struct Fetch_easy *data,
                struct ssh_conn *sshc,
                struct SSHPROTO *sshp,
                bool *blockp)
{
  ...
  /* if a command starts with an asterisk, which a legal SFTP command never
     ...
     cancels etc. It will cause libfetch to act as if the command is
  ...
  if (!strncasecompare(cmd, "chmod", 5))
  {
    ...
    int rc = libssh2_sftp_stat_ex(sshc->sftp_session, sshc->quote_path2,
                                  fetchx_uztoui(strlen(sshc->quote_path2)),
                                  LIBSSH2_SFTP_STAT,
                                  &sshp->quote_attrs);
    if (rc == LIBSSH2_ERROR_EAGAIN)
    {
...
static FETCHcode
sftp_download_stat(struct Fetch_easy *data,
                   struct ssh_conn *sshc,
                   struct SSHPROTO *sshp,
                   bool *blockp)
{
  ...
  else
  {
    ...
    if (data->state.use_range)
    {
      ...
      if (from > size)
      {
        failf(data, "Offset (%" FMT_OFF_T ") was beyond file size (%" FMT_OFF_T ")", from, (fetch_off_t)attrs.filesize);
        return FETCHE_BAD_DOWNLOAD_RESUME;
      }
      if (from > to)
      {
  ...
  if (data->state.resume_from)
  {
    if (data->state.resume_from < 0)
    {
      ...
      if ((fetch_off_t)attrs.filesize < -data->state.resume_from)
      {
        failf(data, "Offset (%" FMT_OFF_T ") was beyond file size (%" FMT_OFF_T ")",
              data->state.resume_from, (fetch_off_t)attrs.filesize);
        return FETCHE_BAD_DOWNLOAD_RESUME;
      }
      /* download from where? */
      data->state.resume_from += attrs.filesize;
...
static FETCHcode sftp_readdir(struct Fetch_easy *data,
                              struct ssh_conn *sshc,
                              struct SSHPROTO *sshp,
                              bool *blockp)
{
  ...
  int rc = libssh2_sftp_readdir_ex(sshc->sftp_handle,
                                   sshp->readdir_filename, FETCH_PATH_MAX,
                                   sshp->readdir_longentry, FETCH_PATH_MAX,
                                   &sshp->readdir_attrs);
  if (rc == LIBSSH2_ERROR_EAGAIN)
  {
    *blockp = TRUE;
  ...
  if (rc > 0)
  {
    ...
    {
      result = Fetch_dyn_add(&sshp->readdir, sshp->readdir_longentry);
      ...
      if (!result)
      {
        ...
        {
          Fetch_dyn_init(&sshp->readdir_link, FETCH_PATH_MAX);
          result = Fetch_dyn_addf(&sshp->readdir_link, "%s%s", sshp->path,
                                 sshp->readdir_filename);
  ...
  {
    unsigned long sftperr = libssh2_sftp_last_error(sshc->sftp_session);
    result = sftp_libssh2_error_to_FETCHE(sftperr);
    sshc->actualcode = result ? result : FETCHE_SSH;
...
static FETCHcode ssh_statemachine(struct Fetch_easy *data, bool *block)
{
  FETCHcode result = FETCHE_OK;
  struct connectdata *conn = data->conn;
  ...
  do
  {
    ...
    case SSH_S_STARTUP:
      ...
      if (rc)
      {
        ...
        state(data, SSH_SESSION_FREE);
        sshc->actualcode = FETCHE_FAILED_INIT;
        break;
      }
    ...
    case SSH_AUTHLIST:
      ...
      sshc->authlist = libssh2_userauth_list(sshc->ssh_session,
                                             conn->user,
                                             fetchx_uztoui(strlen(conn->user)));
      ...
      if (!sshc->authlist)
      {
        ...
        {
          state(data, SSH_SESSION_FREE);
          sshc->actualcode = libssh2_session_error_to_FETCHE(rc);
        }
        break;
      }
    ...
    case SSH_AUTH_HOST_INIT:
      if ((data->set.ssh_auth_types & FETCHSSH_AUTH_HOST) &&
    ...
    case SSH_AUTH_AGENT_INIT:
      ...
      if ((data->set.ssh_auth_types & FETCHSSH_AUTH_AGENT) && (strstr(sshc->authlist, "publickey") != NULL))
      {

        /* Connect to the ssh-agent */
        /* The agent could be shared by a fetch thread i believe
    ...
    case SSH_AUTH_KEY_INIT:
      if ((data->set.ssh_auth_types & FETCHSSH_AUTH_KEYBOARD) && (strstr(sshc->authlist, "keyboard-interactive") != NULL))
      {
    ...
    case SSH_AUTH_DONE:
      if (!sshc->authed)
      {
        ...
        state(data, SSH_SESSION_FREE);
        sshc->actualcode = FETCHE_LOGIN_DENIED;
        break;
      }
    ...
    case SSH_SFTP_REALPATH:
      ...
      else
      {
        ...
        if (sftperr)
          ...
          result = FETCHE_SSH;
        sshc->actualcode = result;
        DEBUGF(infof(data, "error = %lu makes libfetch = %d",
                     sftperr, (int)result));
        state(data, SSH_STOP);
        break;
      }
    ...
    case SSH_SFTP_QUOTE_INIT:
      ...
      result = Fetch_getworkingpath(data, sshc->homedir, &sshp->path);
      if (result)
      {
        sshc->actualcode = result;
        state(data, SSH_STOP);
    ...
    case SSH_SFTP_QUOTE_SYMLINK:
      rc = libssh2_sftp_symlink_ex(sshc->sftp_session, sshc->quote_path1,
                                   fetchx_uztoui(strlen(sshc->quote_path1)),
                                   sshc->quote_path2,
                                   fetchx_uztoui(strlen(sshc->quote_path2)),
    ...
    case SSH_SFTP_QUOTE_STATVFS:
    {
      ...
      {
#ifdef _MSC_VER
#define FETCH_LIBSSH2_VFS_SIZE_MASK "I64u"
#else
#define FETCH_LIBSSH2_VFS_SIZE_MASK "llu"
#endif
        char *tmp = aprintf("statvfs:\n"
                            "f_bsize: %" FETCH_LIBSSH2_VFS_SIZE_MASK "\n"
                            "f_frsize: %" FETCH_LIBSSH2_VFS_SIZE_MASK "\n"
                            "f_blocks: %" FETCH_LIBSSH2_VFS_SIZE_MASK "\n"
                            "f_bfree: %" FETCH_LIBSSH2_VFS_SIZE_MASK "\n"
                            "f_bavail: %" FETCH_LIBSSH2_VFS_SIZE_MASK "\n"
                            "f_files: %" FETCH_LIBSSH2_VFS_SIZE_MASK "\n"
                            "f_ffree: %" FETCH_LIBSSH2_VFS_SIZE_MASK "\n"
                            "f_favail: %" FETCH_LIBSSH2_VFS_SIZE_MASK "\n"
                            "f_fsid: %" FETCH_LIBSSH2_VFS_SIZE_MASK "\n"
                            "f_flag: %" FETCH_LIBSSH2_VFS_SIZE_MASK "\n"
                            "f_namemax: %" FETCH_LIBSSH2_VFS_SIZE_MASK "\n",
    ...
    case SSH_SFTP_READDIR_INIT:
      ...
      Fetch_dyn_init(&sshp->readdir, FETCH_PATH_MAX * 2);
    ...
    case SSH_SFTP_READDIR_LINK:
      rc =
          libssh2_sftp_symlink_ex(sshc->sftp_session,
                                  Fetch_dyn_ptr(&sshp->readdir_link),
                                  (unsigned int)
                                      Fetch_dyn_len(&sshp->readdir_link),
                                  sshp->readdir_filename,
                                  FETCH_PATH_MAX, LIBSSH2_SFTP_READLINK);
      if (rc == LIBSSH2_ERROR_EAGAIN)
      {
        break;
      }
      Fetch_dyn_free(&sshp->readdir_link);

      /* append filename and extra output */
      result = Fetch_dyn_addf(&sshp->readdir, " -> %s", sshp->readdir_filename);
    ...
    case SSH_SFTP_READDIR_BOTTOM:
      result = Fetch_dyn_addn(&sshp->readdir, "\n", 1);
      if (!result)
        result = Fetch_client_write(data, CLIENTWRITE_BODY,
                                   Fetch_dyn_ptr(&sshp->readdir),
                                   Fetch_dyn_len(&sshp->readdir));

      if (result)
      {
        Fetch_dyn_free(&sshp->readdir);
        state(data, SSH_STOP);
      }
      else
      {
        Fetch_dyn_reset(&sshp->readdir);
        state(data, SSH_SFTP_READDIR);
      }
      break;
    ...
    case SSH_SCP_UPLOAD_INIT:
      ...
      if (!sshc->ssh_channel)
      {
        ...
        state(data, SSH_SCP_CHANNEL_FREE);
        sshc->actualcode = libssh2_session_error_to_FETCHE(ssh_err);
        /* Map generic errors to upload failed */
        if (sshc->actualcode == FETCHE_SSH ||
            sshc->actualcode == FETCHE_REMOTE_FILE_NOT_FOUND)
          sshc->actualcode = FETCHE_UPLOAD_FAILED;
        break;
      }
    ...
    case SSH_SCP_DOWNLOAD_INIT:
    {
      ...
      /* download data */
      bytecount = (fetch_off_t)sb.st_size;
      data->req.maxdownload = (fetch_off_t)sb.st_size;
...
static int ssh_getsock(struct Fetch_easy *data,
                       struct connectdata *conn,
                       fetch_socket_t *sock)
{
 ...
 */
static void ssh_block2waitfor(struct Fetch_easy *data, bool block)
{
  struct connectdata *conn = data->conn;
  struct ssh_conn *sshc = &conn->proto.sshc;
  int dir = 0;
...
static FETCHcode ssh_block_statemach(struct Fetch_easy *data,
                                     struct connectdata *conn,
                                     bool disconnect)
{
  struct ssh_conn *sshc = &conn->proto.sshc;
  FETCHcode result = FETCHE_OK;
  struct fetchtime dis = Fetch_now();
  ...
  while ((sshc->state != SSH_STOP) && !result)
  {
    ...
    else if (Fetch_timediff(now, dis) > 1000)
    {
      /* disconnect timeout */
    ...
    if (block)
    {
      ...
      fetch_socket_t fd_read = FETCH_SOCKET_BAD;
      fetch_socket_t fd_write = FETCH_SOCKET_BAD;
      if (LIBSSH2_SESSION_BLOCK_INBOUND & dir)
        fd_read = sock;
      if (LIBSSH2_SESSION_BLOCK_OUTBOUND & dir)
        fd_write = sock;
      /* wait for the socket to become ready */
      (void)Fetch_socket_check(fd_read, FETCH_SOCKET_BAD, fd_write,
                              left > 1000 ? 1000 : left);
    }
  }
                            ...
                            size_t length, int flags, void **abstract)
{
  ...
  FETCHcode result;
  struct connectdata *conn = data->conn;
  Fetch_recv *backup = conn->recv[0];
  struct ssh_conn *ssh = &conn->proto.sshc;
  int socknum = Fetch_conn_sockindex(data, sock);
  (void)flags;

  /* swap in the TLS reader function for this call only, and then swap back
     the SSH one again */
  conn->recv[0] = ssh->tls_recv;
  result = Fetch_conn_recv(data, socknum, buffer, length, &nread);
  conn->recv[0] = backup;
  if (result == FETCHE_AGAIN)
    return -EAGAIN; /* magic return code for libssh2 */
  else if (result)
    return -1; /* generic error */
  Fetch_debug(data, FETCHINFO_DATA_IN, (char *)buffer, (size_t)nread);
  return nread;
}
                            ...
                            size_t length, int flags, void **abstract)
{
  ...
  result = Fetch_conn_send(data, socknum, buffer, length, FALSE, &nwrite);
  conn->send[0] = backup;
  if (result == FETCHE_AGAIN)
    return -EAGAIN; /* magic return code for libssh2 */
  else if (result)
    return -1; /* error */
  Fetch_debug(data, FETCHINFO_DATA_OUT, (char *)buffer, nwrite);
...
static FETCHcode ssh_connect(struct Fetch_easy *data, bool *done)
{
  ...
  /* libfetch MUST to set custom memory functions so that the kbd_callback
...
#ifndef FETCH_DISABLE_PROXY
  if (conn->http_proxy.proxytype == FETCHPROXY_HTTPS)
  {
    /*
...
static FETCHcode scp_doing(struct Fetch_easy *data,
                           bool *dophase_done)
{
  FETCHcode result;
  result = ssh_multi_statemach(data, dophase_done);
...
static FETCHcode ssh_done(struct Fetch_easy *data, FETCHcode status)
{
  FETCHcode result = FETCHE_OK;
  struct SSHPROTO *sshp = data->req.p.ssh;
  struct connectdata *conn = data->conn;
                        ...
                        const void *mem, size_t len, bool eos, FETCHcode *err)
{
  ...
  {
    *err = libssh2_session_error_to_FETCHE((int)nwrite);
    nwrite = -1;
  }
...
static FETCHcode sftp_doing(struct Fetch_easy *data,
                            bool *dophase_done)
{
  FETCHcode result = ssh_multi_statemach(data, dophase_done);
                         ...
                         char *mem, size_t len, FETCHcode *err)
{
  ...
  {
    *err = libssh2_session_error_to_FETCHE((int)nread);
  }
  return nread;
}
...
{
  (void)msnprintf(buffer, buflen, "libssh2/%s", FETCH_LIBSSH2_VERSION);
}
