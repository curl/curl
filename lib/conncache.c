...
void Curl_cpool_destroy(struct cpool *cpool)
{
  if(cpool && cpool->initialised && cpool->idata) {
    ...
    cpool->idata->state.now = curlx_now();
                            ...
                            struct connectdata *conn)
{
  ...
  cpool->idata->state.now = data->state.now;
  CPOOL_LOCK(cpool, cpool->idata);
  if(dest_limit) {
    size_t live;
    ...
    while((live + shutdowns) >= dest_limit) {
      if(shutdowns) {
        ...
        oldest_idle = cpool_bundle_get_oldest_idle(bundle, &data->state.now);
        if(!oldest_idle)
          break;
    ...
    while((cpool->num_conn + shutdowns) >= total_limit) {
      if(shutdowns) {
        ...
        struct connectdata *oldest_idle =
          cpool_get_oldest_idle(cpool, &data->state.now);
        if(!oldest_idle)
          break;
        /* disconnect the old conn and continue */
...
out:
  CPOOL_UNLOCK(cpool, cpool->idata);
  data->state.now = cpool->idata->state.now;
  return result;
}
                              ...
                              struct connectdata *conn)
{
  ...
  conn->lastused = data->state.now; /* it was used up until now */
  if(cpool && maxconnects) {
    ...
    if(cpool->num_conn > maxconnects) {
      ...
      oldest_idle = cpool_get_oldest_idle(cpool, &data->state.now);
...
void Curl_cpool_prune_dead(struct Curl_easy *data)
{
  ...
  CPOOL_LOCK(cpool, data);
  elapsed = curlx_timediff_ms(data->state.now, cpool->last_cleanup);

  if(elapsed >= 1000L) {
    while(cpool_foreach(data, cpool, NULL, cpool_reap_dead_cb))
      ;
    cpool->last_cleanup = data->state.now;
  }
  CPOOL_UNLOCK(cpool, data);
}
...
CURLcode Curl_cpool_upkeep(struct Curl_easy *data)
{
  ...
  CPOOL_LOCK(cpool, data);
  cpool_foreach(data, cpool, &data->state.now, conn_upkeep);
  CPOOL_UNLOCK(cpool, data);
  return CURLE_OK;
}
