 ...
 */
#define FETCH_SHA512_256_BLOCK_SIZE 128

/**
 * Size of the SHA-512/256 resulting digest in bytes.
 * This is the final digest size, not intermediate hash.
 */
#define FETCH_SHA512_256_DIGEST_SIZE FETCH_SHA512_256_DIGEST_LENGTH
...
Fetch_sha512_256_init(void *context)
{
  ...
  *ctx = EVP_MD_CTX_create();
  ...
  if (EVP_DigestInit_ex(*ctx, EVP_sha512_256(), NULL))
  {
    ...
    DEBUGASSERT(EVP_MD_CTX_size(*ctx) == FETCH_SHA512_256_DIGEST_SIZE);
    /* Check whether the block size is correct */
    DEBUGASSERT(EVP_MD_CTX_block_size(*ctx) == FETCH_SHA512_256_BLOCK_SIZE);
 ...
 *
 * @param context the calculation context
 * @param[out] digest set to the hash, must be #FETCH_SHA512_256_DIGEST_SIZE
 #             bytes
 * @return FETCHE_OK if succeed,
 *         error code otherwise
 */
                       ...
                       void *context)
{
  ...
  unsigned char tmp_digest[FETCH_SHA512_256_DIGEST_SIZE * 2];
  ret = EVP_DigestFinal_ex(*ctx,
                           tmp_digest, NULL)
            ? FETCHE_OK
            : FETCHE_SSL_CIPHER;
  if (ret == FETCHE_OK)
    memcpy(digest, tmp_digest, FETCH_SHA512_256_DIGEST_SIZE);
  explicit_memset(tmp_digest, 0, sizeof(tmp_digest));
#else  /* ! NEED_NETBSD_SHA512_256_WORKAROUND */
  ret = EVP_DigestFinal_ex(*ctx, digest, NULL) ? FETCHE_OK : FETCHE_SSL_CIPHER;
...
#define FETCH_SHA512_256_BLOCK_SIZE SHA512_256_BLOCK_SIZE
#define FETCH_SHA512_256_DIGEST_SIZE SHA512_256_DIGEST_SIZE
 ...
 * ** The author ported the code to libfetch. The ported code is provided ** *
 * ** under fetch license.                                                ** *
...
#define FETCH_FORCEINLINE FETCH_INLINE __attribute__((always_inline))
#endif
#endif
#endif

#if !defined(FETCH_FORCEINLINE) && \
    defined(_MSC_VER) && !defined(__GNUC__) && !defined(__clang__)
#define FETCH_FORCEINLINE __forceinline
#endif

#if !defined(FETCH_FORCEINLINE)
/* Assume that 'FETCH_INLINE' keyword works or the
 * macro was already defined correctly. */
#define FETCH_FORCEINLINE FETCH_INLINE
#endif

/* Bits manipulation macros and functions.
   Can be moved to other headers to reuse. */

#define FETCH_GET_64BIT_BE(ptr)                                  \
  (((fetch_uint64_t)(((const unsigned char *)(ptr))[0]) << 56) | \
   ((fetch_uint64_t)(((const unsigned char *)(ptr))[1]) << 48) | \
   ((fetch_uint64_t)(((const unsigned char *)(ptr))[2]) << 40) | \
   ((fetch_uint64_t)(((const unsigned char *)(ptr))[3]) << 32) | \
   ((fetch_uint64_t)(((const unsigned char *)(ptr))[4]) << 24) | \
   ((fetch_uint64_t)(((const unsigned char *)(ptr))[5]) << 16) | \
   ((fetch_uint64_t)(((const unsigned char *)(ptr))[6]) << 8) |  \
   (fetch_uint64_t)(((const unsigned char *)(ptr))[7]))

#define FETCH_PUT_64BIT_BE(ptr, val)                                              \
  do                                                                              \
  {                                                                               \
    ((unsigned char *)(ptr))[7] = (unsigned char)((fetch_uint64_t)(val));         \
    ((unsigned char *)(ptr))[6] = (unsigned char)(((fetch_uint64_t)(val)) >> 8);  \
    ((unsigned char *)(ptr))[5] = (unsigned char)(((fetch_uint64_t)(val)) >> 16); \
    ((unsigned char *)(ptr))[4] = (unsigned char)(((fetch_uint64_t)(val)) >> 24); \
    ((unsigned char *)(ptr))[3] = (unsigned char)(((fetch_uint64_t)(val)) >> 32); \
    ((unsigned char *)(ptr))[2] = (unsigned char)(((fetch_uint64_t)(val)) >> 40); \
    ((unsigned char *)(ptr))[1] = (unsigned char)(((fetch_uint64_t)(val)) >> 48); \
    ((unsigned char *)(ptr))[0] = (unsigned char)(((fetch_uint64_t)(val)) >> 56); \
  } while (0)
 ...
 * This is the final digest size, not intermediate hash.
 */
#define FETCH_SHA512_256_DIGEST_SIZE \
 ...
 * Size of the SHA-512/256 single processing block in bytes.
 */
#define FETCH_SHA512_256_BLOCK_SIZE (SHA512_256_BLOCK_SIZE_BITS / 8)
...
Fetch_sha512_256_init(void *context)
{
  ...
  DEBUGASSERT(sizeof(fetch_uint64_t) == 8);

  /* Initial hash values, see FIPS PUB 180-4 section 5.3.6.2 */
  /* Values generated by "IV Generation Function" as described in
   * section 5.3.6 */
  ctx->H[0] = FETCH_UINT64_C(0x22312194FC2BF72C);
  ctx->H[1] = FETCH_UINT64_C(0x9F555FA3C84C64C2);
  ctx->H[2] = FETCH_UINT64_C(0x2393B86B6F53B151);
  ctx->H[3] = FETCH_UINT64_C(0x963877195940EABD);
  ctx->H[4] = FETCH_UINT64_C(0x96283EE2A88EFFE3);
  ctx->H[5] = FETCH_UINT64_C(0xBE5E1E2553863992);
  ctx->H[6] = FETCH_UINT64_C(0x2B0199FC2C85B8AA);
  ctx->H[7] = FETCH_UINT64_C(0x0EB72DDC81C52CA2);

  /* Initialise number of bytes and high part of number of bits. */
  ctx->count = FETCH_UINT64_C(0);
  ctx->count_bits_hi = FETCH_UINT64_C(0);
                          ...
                          const void *data)
{
  ...
  fetch_uint64_t a = H[0];
  fetch_uint64_t b = H[1];
  fetch_uint64_t c = H[2];
  fetch_uint64_t d = H[3];
  fetch_uint64_t e = H[4];
  fetch_uint64_t f = H[5];
  fetch_uint64_t g = H[6];
  fetch_uint64_t h = H[7];
...
#define sig1(x) \
  ...
  if (1)
  {
    ...
    static const fetch_uint64_t K[80] = {
        FETCH_UINT64_C(0x428a2f98d728ae22), FETCH_UINT64_C(0x7137449123ef65cd),
        FETCH_UINT64_C(0xb5c0fbcfec4d3b2f), FETCH_UINT64_C(0xe9b5dba58189dbbc),
        FETCH_UINT64_C(0x3956c25bf348b538), FETCH_UINT64_C(0x59f111f1b605d019),
        FETCH_UINT64_C(0x923f82a4af194f9b), FETCH_UINT64_C(0xab1c5ed5da6d8118),
        FETCH_UINT64_C(0xd807aa98a3030242), FETCH_UINT64_C(0x12835b0145706fbe),
        FETCH_UINT64_C(0x243185be4ee4b28c), FETCH_UINT64_C(0x550c7dc3d5ffb4e2),
        FETCH_UINT64_C(0x72be5d74f27b896f), FETCH_UINT64_C(0x80deb1fe3b1696b1),
        FETCH_UINT64_C(0x9bdc06a725c71235), FETCH_UINT64_C(0xc19bf174cf692694),
        FETCH_UINT64_C(0xe49b69c19ef14ad2), FETCH_UINT64_C(0xefbe4786384f25e3),
        FETCH_UINT64_C(0x0fc19dc68b8cd5b5), FETCH_UINT64_C(0x240ca1cc77ac9c65),
        FETCH_UINT64_C(0x2de92c6f592b0275), FETCH_UINT64_C(0x4a7484aa6ea6e483),
        FETCH_UINT64_C(0x5cb0a9dcbd41fbd4), FETCH_UINT64_C(0x76f988da831153b5),
        FETCH_UINT64_C(0x983e5152ee66dfab), FETCH_UINT64_C(0xa831c66d2db43210),
        FETCH_UINT64_C(0xb00327c898fb213f), FETCH_UINT64_C(0xbf597fc7beef0ee4),
        FETCH_UINT64_C(0xc6e00bf33da88fc2), FETCH_UINT64_C(0xd5a79147930aa725),
        FETCH_UINT64_C(0x06ca6351e003826f), FETCH_UINT64_C(0x142929670a0e6e70),
        FETCH_UINT64_C(0x27b70a8546d22ffc), FETCH_UINT64_C(0x2e1b21385c26c926),
        FETCH_UINT64_C(0x4d2c6dfc5ac42aed), FETCH_UINT64_C(0x53380d139d95b3df),
        FETCH_UINT64_C(0x650a73548baf63de), FETCH_UINT64_C(0x766a0abb3c77b2a8),
        FETCH_UINT64_C(0x81c2c92e47edaee6), FETCH_UINT64_C(0x92722c851482353b),
        FETCH_UINT64_C(0xa2bfe8a14cf10364), FETCH_UINT64_C(0xa81a664bbc423001),
        FETCH_UINT64_C(0xc24b8b70d0f89791), FETCH_UINT64_C(0xc76c51a30654be30),
        FETCH_UINT64_C(0xd192e819d6ef5218), FETCH_UINT64_C(0xd69906245565a910),
        FETCH_UINT64_C(0xf40e35855771202a), FETCH_UINT64_C(0x106aa07032bbd1b8),
        FETCH_UINT64_C(0x19a4c116b8d2d0c8), FETCH_UINT64_C(0x1e376c085141ab53),
        FETCH_UINT64_C(0x2748774cdf8eeb99), FETCH_UINT64_C(0x34b0bcb5e19b48a8),
        FETCH_UINT64_C(0x391c0cb3c5c95a63), FETCH_UINT64_C(0x4ed8aa4ae3418acb),
        FETCH_UINT64_C(0x5b9cca4f7763e373), FETCH_UINT64_C(0x682e6ff3d6b2b8a3),
        FETCH_UINT64_C(0x748f82ee5defb2fc), FETCH_UINT64_C(0x78a5636f43172f60),
        FETCH_UINT64_C(0x84c87814a1f0ab72), FETCH_UINT64_C(0x8cc702081a6439ec),
        FETCH_UINT64_C(0x90befffa23631e28), FETCH_UINT64_C(0xa4506cebde82bde9),
        FETCH_UINT64_C(0xbef9a3f7b2c67915), FETCH_UINT64_C(0xc67178f2e372532b),
        FETCH_UINT64_C(0xca273eceea26619c), FETCH_UINT64_C(0xd186b8c721c0c207),
        FETCH_UINT64_C(0xeada7dd6cde0eb1e), FETCH_UINT64_C(0xf57d4f7fee6ed178),
        FETCH_UINT64_C(0x06f067aa72176fba), FETCH_UINT64_C(0x0a637dc5a2c898a6),
        FETCH_UINT64_C(0x113f9804bef90dae), FETCH_UINT64_C(0x1b710b35131c471b),
        FETCH_UINT64_C(0x28db77f523047d84), FETCH_UINT64_C(0x32caab7b40c72493),
        FETCH_UINT64_C(0x3c9ebe0a15c9bebc), FETCH_UINT64_C(0x431d67c49c100d4c),
        FETCH_UINT64_C(0x4cc5d4becb3e42b6), FETCH_UINT64_C(0x597f299cfc657e2a),
        FETCH_UINT64_C(0x5fcb6fab3ad6faec), FETCH_UINT64_C(0x6c44198c4a475817)};
...
#define Wgen(w, t) \
  (fetch_uint64_t)((w)[(t - 16) & 15] + sig1((w)[((t) - 2) & 15]) + (w)[((t) - 7) & 15] + sig0((w)[((t) - 15) & 15]))
                       ...
                       size_t length)
{
  ...
  if (0 == length)
    return FETCHE_OK; /* Shortcut, do nothing */

  /* Note: (count & (FETCH_SHA512_256_BLOCK_SIZE-1))
     equals (count % FETCH_SHA512_256_BLOCK_SIZE) for this block size. */
  bytes_have = (unsigned int)(ctx->count & (FETCH_SHA512_256_BLOCK_SIZE - 1));
  ctx->count += length;
  if (length > ctx->count)
    ctx->count_bits_hi += 1U << 3; /* Value wrap */
  ctx->count_bits_hi += ctx->count >> 61;
  ctx->count &= FETCH_UINT64_C(0x1FFFFFFFFFFFFFFF);

  if (0 != bytes_have)
  {
    unsigned int bytes_left = FETCH_SHA512_256_BLOCK_SIZE - bytes_have;
  ...
  while (FETCH_SHA512_256_BLOCK_SIZE <= length)
  {
                       ...
                       void *context)
{
  ...
  if (FETCH_SHA512_256_BLOCK_SIZE - bytes_have < SHA512_256_SIZE_OF_LEN_ADD)
  {
    /* No space in the current block to put the total length of message.
       Pad the current block with zeros and process it. */
    if (bytes_have < FETCH_SHA512_256_BLOCK_SIZE)
  ...
  memset(((unsigned char *)ctx_buf) + bytes_have, 0,
         FETCH_SHA512_256_BLOCK_SIZE - SHA512_256_SIZE_OF_LEN_ADD - bytes_have);
  /* Put high part of number of bits in processed message and then lower
     part of number of bits as big-endian values.
     See FIPS PUB 180-4 section 5.1.2. */
  /* Note: the target location is predefined and buffer is always aligned */
  FETCH_PUT_64BIT_BE(((unsigned char *)ctx_buf) + FETCH_SHA512_256_BLOCK_SIZE - SHA512_256_SIZE_OF_LEN_ADD,
                     ctx->count_bits_hi);
  ...
  FETCH_PUT_64BIT_BE((digest + 0 * SHA512_256_BYTES_IN_WORD), ctx->H[0]);
  FETCH_PUT_64BIT_BE((digest + 1 * SHA512_256_BYTES_IN_WORD), ctx->H[1]);
  FETCH_PUT_64BIT_BE((digest + 2 * SHA512_256_BYTES_IN_WORD), ctx->H[2]);
