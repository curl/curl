...
out:
  if(result) {
    ...
    ctx->connected_at = data->state.now;
    cf->connected = TRUE;
  }
                                   ...
                                   struct cf_socket_ctx *ctx)
{
  ...
  if(curlx_timediff_ms(data->state.now, ctx->last_sndbuf_query_at) > 1000) {
    ...
    ctx->last_sndbuf_query_at = data->state.now;
  }
}
                               ...
                               char *buf, size_t len, size_t *pnread)
{
  ...
  *pnread = 0;
  ...
  if(!result && !ctx->got_first_byte) {
    ctx->first_byte_at = data->state.now;
                                         ...
                                         struct Curl_easy *data)
{
  ...
  if(other_ms && (other_ms < timeout_ms))
    ...
    timeout_ms -= curlx_timediff_ms(data->state.now, ctx->started_at);
    if(!timeout_ms)
