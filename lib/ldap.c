...
#if !defined(FETCH_DISABLE_LDAP) && !defined(USE_OPENLDAP)
 ...
 * libfetch is built with USE_OPENLDAP defined the libfetch source code that
...
static int _ldap_url_parse(struct Fetch_easy *data,
...
static void _ldap_trace(const char *fmt, ...) FETCH_PRINTF(1, 2);
#else
#define LDAP_TRACE(x) Fetch_nop_stmt
#endif
...
static FETCHcode ldap_do(struct Fetch_easy *data, bool *done);

/*
 * LDAP protocol handler.
 */

const struct Fetch_handler Fetch_handler_ldap = {
    "ldap",          /* scheme */
    ...
    ZERO_NULL,       /* follow */
    PORT_LDAP,       /* defport */
    FETCHPROTO_LDAP, /* protocol */
    FETCHPROTO_LDAP, /* family */
    PROTOPT_NONE     /* flags */
};
...
const struct Fetch_handler Fetch_handler_ldaps = {
    "ldaps",          /* scheme */
    ZERO_NULL,        /* setup_connection */
    ldap_do,          /* do_it */
    ZERO_NULL,        /* done */
                              ...
                              const char *passwd, unsigned long authflags)
{
  ...
  if (authflags & FETCHAUTH_NEGOTIATE)
  {
  ...
  {
    FETCHcode res = Fetch_create_sspi_identity(user, passwd, &cred);
    rc = (int)res;
    if (!rc)
    {
      rc = (int)ldap_bind_s(server, NULL, (TCHAR *)&cred, method);
      Fetch_sspi_free_identity(&cred);
    }
  }
  else
  {
...
static int ldap_win_bind(struct Fetch_easy *data, LDAP *server,
                         const char *user, const char *passwd)
{
  ...
  if (user && passwd && (data->set.httpauth & FETCHAUTH_BASIC))
  {
    inuser = fetchx_convert_UTF8_to_tchar((char *)user);
    inpass = fetchx_convert_UTF8_to_tchar((char *)passwd);

    rc = (int)ldap_simple_bind_s(server, inuser, inpass);

    fetchx_unicodefree(inuser);
    fetchx_unicodefree(inpass);
  }
#if defined(USE_WINDOWS_SSPI)
  else
  {
...
#if defined(USE_WIN32_LDAP)
#define FREE_ON_WINLDAP(x) fetchx_unicodefree(x)
#define fetch_ldap_num_t ULONG
#else
#define FREE_ON_WINLDAP(x)
#define fetch_ldap_num_t int
#endif
...
static FETCHcode ldap_do(struct Fetch_easy *data, bool *done)
{
  ...
  *done = TRUE; /* unconditionally */
  ...
  if (rc)
  {
    failf(data, "Bad LDAP URL: %s", ldap_err2string((fetch_ldap_num_t)rc));
...
#if defined(USE_WIN32_LDAP)
  host = fetchx_convert_UTF8_to_tchar(conn->host.name);
  if (!host)
  {
  ...
  if (ldap_ssl)
  {
    ...
    server = ldap_sslinit(host, (fetch_ldap_num_t)conn->primary.remote_port, 1);
    ldap_set_option(server, LDAP_OPT_SSL, LDAP_OPT_ON);
#else
    int ldap_option;
    char *ldap_ca = conn->ssl_config.CAfile;
#if defined(FETCH_HAS_NOVELL_LDAPSDK)
...
*/
  ...
  {
    server = ldap_init(host, (fetch_ldap_num_t)conn->primary.remote_port);
    if (!server)
    {
      failf(data, "LDAP local: Cannot connect to %s:%u",
            conn->host.dispname, conn->primary.remote_port);
  ...
  if (rc)
  {
    ...
    failf(data, "LDAP local: bind via ldap_simple_bind_s %s",
          ldap_err2string(rc));
#endif
    result = FETCHE_LDAP_CANNOT_BIND;
    goto quit;
  }

  Fetch_pgrsSetDownloadCounter(data, 0);
  rc = (int)ldap_search_s(server, ludp->lud_dn,
                          (fetch_ldap_num_t)ludp->lud_scope,
                          ludp->lud_filter, ludp->lud_attrs, 0, &ldapmsg);

  if (rc && rc != LDAP_SIZELIMIT_EXCEEDED)
  {
    failf(data, "LDAP remote: %s", ldap_err2string((fetch_ldap_num_t)rc));
    result = FETCHE_LDAP_SEARCH_FAILED;
    goto quit;
  }
       ...
       entryIterator = ldap_next_entry(server, entryIterator), num++)
  {
    ...
    /* Get the DN and write it to the client */
    {
      ...
      name = fetchx_convert_tchar_to_UTF8(dn);
      if (!name)
      {
        ldap_memfree(dn);
      ...
      result = Fetch_client_write(data, CLIENTWRITE_BODY, (char *)"DN: ", 4);
      if (result)
      {
      ...
      result = Fetch_client_write(data, CLIENTWRITE_BODY, name, name_len);
      if (result)
      {
        FREE_ON_WINLDAP(name);
        ldap_memfree(dn);
      ...
      result = Fetch_client_write(data, CLIENTWRITE_BODY, (char *)"\n", 1);
      if (result)
      {
        FREE_ON_WINLDAP(name);
        ldap_memfree(dn);
         ...
         attribute = ldap_next_attribute(server, entryIterator, ber))
    {
      ...
      size_t attr_len;
#if defined(USE_WIN32_LDAP)
      char *attr = fetchx_convert_tchar_to_UTF8(attribute);
      if (!attr)
      {
        if (ber)
          ber_free(ber, 0);
      ...
      if (vals)
      {
        ...
        {
          result = Fetch_client_write(data, CLIENTWRITE_BODY, (char *)"\t", 1);
          if (result)
          {
          ...
          result = Fetch_client_write(data, CLIENTWRITE_BODY, attr, attr_len);
          if (result)
          {
            ldap_value_free_len(vals);
            FREE_ON_WINLDAP(attr);
          ...
          result = Fetch_client_write(data, CLIENTWRITE_BODY, (char *)": ", 2);
          if (result)
          {
            ldap_value_free_len(vals);
            FREE_ON_WINLDAP(attr);
              ...
              (strcmp(";binary", attr + (attr_len - 7)) == 0))
          {
            ...
            result = Fetch_base64_encode(vals[i]->bv_val, vals[i]->bv_len,
                                        &val_b64, &val_b64_sz);
            if (result)
            {
              ldap_value_free_len(vals);
            ...
            if (val_b64_sz > 0)
            {
              result = Fetch_client_write(data, CLIENTWRITE_BODY, val_b64,
                                         val_b64_sz);
              free(val_b64);
              if (result)
              {
          ...
          {
            result = Fetch_client_write(data, CLIENTWRITE_BODY, vals[i]->bv_val,
                                       vals[i]->bv_len);
            if (result)
            {
              ldap_value_free_len(vals);
...
#if defined(HAVE_LDAP_SSL) && defined(FETCH_HAS_NOVELL_LDAPSDK)
  if (ldap_ssl)
    ldapssl_client_deinit();
#endif /* HAVE_LDAP_SSL && FETCH_HAS_NOVELL_LDAPSDK */
...
static void _ldap_trace(const char *fmt, ...)
{
  ...
  if (do_trace == -1)
  {
    const char *env = getenv("FETCH_TRACE");
...
static bool split_str(char *str, char ***out, size_t *count)
{
  ...
  for (i = 0, s = Fetch_strtok_r(str, ",", &lasts); s && i < items;
       s = Fetch_strtok_r(NULL, ",", &lasts), i++)
    res[i] = s;
 ...
 */
static int _ldap_url_parse2(struct Fetch_easy *data,
                            const struct connectdata *conn, LDAPURLDesc *ludp)
{
  ...
  if (*p)
  {
    ...
    /* Unescape the DN */
    result = Fetch_urldecode(dn, 0, &unescaped, NULL, REJECT_ZERO);
    if (result)
    {
      rc = LDAP_NO_MEMORY;
    ...
    ludp->lud_dn = fetchx_convert_UTF8_to_tchar(unescaped);
  ...
  if (*p)
  {
    ...
    for (i = 0; i < count; i++)
    {
      ...
      result = Fetch_urldecode(attributes[i], 0, &unescaped, NULL,
                              REJECT_ZERO);
      if (result)
      {
        free(attributes);
...
#if defined(USE_WIN32_LDAP)
      /* Convert the unescaped string to a tchar */
      ludp->lud_attrs[i] = fetchx_convert_UTF8_to_tchar(unescaped);
  ...
  if (*p)
  {
    ...
    /* Unescape the filter */
    result = Fetch_urldecode(filter, 0, &unescaped, NULL, REJECT_ZERO);
    if (result)
    {
      rc = LDAP_NO_MEMORY;
...
#if defined(USE_WIN32_LDAP)
    /* Convert the unescaped string to a tchar */
    ludp->lud_filter = fetchx_convert_UTF8_to_tchar(unescaped);
...
static void _ldap_free_urldesc(LDAPURLDesc *ludp)
{
...
#if defined(USE_WIN32_LDAP)
  fetchx_unicodefree(ludp->lud_dn);
  fetchx_unicodefree(ludp->lud_filter);
#else
  free(ludp->lud_dn);
  ...
  if (ludp->lud_attrs)
  {
    ...
    {
#if defined(USE_WIN32_LDAP)
      fetchx_unicodefree(ludp->lud_attrs[i]);
#else
...
#endif /* !FETCH_DISABLE_LDAP && !USE_OPENLDAP */
