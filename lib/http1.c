...
void Fetch_h1_req_parse_init(struct h1_req_parser *parser, size_t max_line_len)
{
  ...
  Fetch_dyn_init(&parser->scratch, max_line_len);
}
...
void Fetch_h1_req_parse_free(struct h1_req_parser *parser)
{
  if (parser)
  {
    Fetch_http_req_free(parser->req);
    Fetch_dyn_free(&parser->scratch);
    parser->req = NULL;
    parser->done = FALSE;
  }
}
                         ...
                         FETCHcode *err)
{
  ...
  if (parser->line)
  {
    parser->line = NULL;
    parser->line_len = 0;
    Fetch_dyn_reset(&parser->scratch);
  }

  nread = detect_line(parser, buf, buflen, err);
  if (nread >= 0)
  {
    if (Fetch_dyn_len(&parser->scratch))
    {
      /* append detected line to scratch to have the complete line */
      *err = Fetch_dyn_addn(&parser->scratch, parser->line, parser->line_len);
      if (*err)
        return -1;
      parser->line = Fetch_dyn_ptr(&parser->scratch);
      parser->line_len = Fetch_dyn_len(&parser->scratch);
    }
  ...
  else if (*err == FETCHE_AGAIN)
  {
    ...
    *err = Fetch_dyn_addn(&parser->scratch, (const unsigned char *)buf, buflen);
                           ...
                           const char *scheme_default, int options)
{
  ...
  else
  {
    ...
    if (Fetch_is_absolute_url(tmp, NULL, 0, FALSE))
    {
  ...
  if (url)
  {
    result = Fetch_http_req_make2(&parser->req, m, m_len, url, scheme_default);
  }
  else
  {
    ...
    result = Fetch_http_req_make(&parser->req, m, m_len, scheme, scheme_len,
                               ...
                               FETCHcode *err)
{
  ...
  *err = FETCHE_OK;
  while (!parser->done)
  {
    ...
    {
      *err = Fetch_dynhds_h1_add_line(&parser->req->headers,
                                 ...
                                 struct dynbuf *dbuf)
{
  ...
  result = Fetch_dyn_addf(dbuf, "%s %s%s%s%s HTTP/1.%d\r\n",
                         req->method,
  ...
  result = Fetch_dynhds_h1_dprint(&req->headers, dbuf);
  if (result)
    goto out;

  result = Fetch_dyn_addn(dbuf, STRCONST("\r\n"));
