                                   ...
                                   tftp_event_t event)
{
  ...
  case TFTP_STATE_START:
    DEBUGF(infof(state->data, "TFTP_STATE_START"));
    result = tftp_send_first(state, event);
    break;
  case TFTP_STATE_RX:
    DEBUGF(infof(state->data, "TFTP_STATE_RX"));
    result = tftp_rx(state, event);
    break;
  case TFTP_STATE_TX:
    DEBUGF(infof(state->data, "TFTP_STATE_TX"));
    result = tftp_tx(state, event);
    break;
  case TFTP_STATE_FIN:
    infof(state->data, "%s", "TFTP finished");
    break;
  default:
    DEBUGF(infof(state->data, "STATE: %d", state->state));
    failf(state->data, "%s", "Internal state machine error");
    result = CURLE_TFTP_ILLEGAL;
    break;
  }
