...
typedef enum
{
  ...
  /* The remaining error codes are internal to fetch */
  TFTP_ERR_NONE = -100,
  TFTP_ERR_TIMEOUT,
  TFTP_ERR_NORESPONSE
} tftp_error_t;
...
struct tftp_state_data
{
  ...
  int retry_max;
  time_t rx_time;
  struct Fetch_sockaddr_storage local_addr;
  struct Fetch_sockaddr_storage remote_addr;
  fetch_socklen_t remote_addrlen;
  int rbytes;
  int sbytes;
  int blksize;
...
static FETCHcode tftp_rx(struct tftp_state_data *state, tftp_event_t event);
static FETCHcode tftp_tx(struct tftp_state_data *state, tftp_event_t event);
static FETCHcode tftp_connect(struct Fetch_easy *data, bool *done);
static FETCHcode tftp_disconnect(struct Fetch_easy *data,
                                 struct connectdata *conn,
                                 bool dead_connection);
static FETCHcode tftp_do(struct Fetch_easy *data, bool *done);
static FETCHcode tftp_done(struct Fetch_easy *data,
                           FETCHcode, bool premature);
static FETCHcode tftp_setup_connection(struct Fetch_easy *data,
                                       struct connectdata *conn);
static FETCHcode tftp_multi_statemach(struct Fetch_easy *data, bool *done);
static FETCHcode tftp_doing(struct Fetch_easy *data, bool *dophase_done);
static int tftp_getsock(struct Fetch_easy *data, struct connectdata *conn,
                        fetch_socket_t *socks);
static FETCHcode tftp_translate_code(tftp_error_t error);

/*
 * TFTP protocol handler.
 */

const struct Fetch_handler Fetch_handler_tftp = {
    "tftp",                                 /* scheme */
    ...
    ZERO_NULL,                              /* follow */
    PORT_TFTP,                              /* defport */
    FETCHPROTO_TFTP,                        /* protocol */
    FETCHPROTO_TFTP,                        /* family */
 ...
 *
 **********************************************************/
static FETCHcode tftp_set_timeouts(struct tftp_state_data *state)
{
  ...
  timeout_ms = Fetch_timeleft(state->data, NULL, start);
...
static FETCHcode tftp_parse_option_ack(struct tftp_state_data *state,
                                       const char *ptr, int len)
{
  const char *tmp = ptr;
  struct Fetch_easy *data = state->data;
  ...
  while (tmp < ptr + len)
  {
    ...
    else if (checkprefix(TFTP_OPTION_TSIZE, option))
    {
      ...
      if (!data->state.upload)
      {
        ...
        Fetch_pgrsSetDownloadSize(data, tsize);
      }
    }
  }
...
static FETCHcode tftp_option_add(struct tftp_state_data *state, size_t *csize,
...
static FETCHcode tftp_connect_for_tx(struct tftp_state_data *state,
                                     tftp_event_t event)
{
  FETCHcode result;
...
static FETCHcode tftp_connect_for_rx(struct tftp_state_data *state,
                                     tftp_event_t event)
{
  FETCHcode result;
#ifndef FETCH_DISABLE_VERBOSE_STRINGS
...
static FETCHcode tftp_send_first(struct tftp_state_data *state,
                                 tftp_event_t event)
{
  size_t sbytes;
  ssize_t senddata;
  ...
  case TFTP_EVENT_TIMEOUT: /* Resend the first packet out */
    ...
    /* As RFC3617 describes the separator slash is not actually part of the
       ...
       string. */
    result = Fetch_urldecode(&state->data->state.up.path[1], 0,
    ...
    if (strlen(filename) > (state->blksize - strlen(mode) - 4))
    {
      ...
      free(filename);
      return FETCHE_TFTP_ILLEGAL; /* too long filename field */
    }
    ...
    if (!data->set.tftp_no_options)
    {
      ...
      msnprintf(buf, sizeof(buf), "%" FMT_OFF_T,
                data->state.upload && (data->state.infilesize != -1) ? data->state.infilesize : 0);
    ...
    senddata = sendto(state->sockfd, (void *)state->spacket.data,
                      ...
                      (struct sockaddr *)&data->conn->remote_addr->fetch_sa_addr,
                      (fetch_socklen_t)data->conn->remote_addr->addrlen);
    if (senddata != (ssize_t)sbytes)
    {
      char buffer[STRERROR_LEN];
      failf(data, "%s", Fetch_strerror(SOCKERRNO, buffer, sizeof(buffer)));
    }
    free(filename);
    break;
 ...
 *
 **********************************************************/
static FETCHcode tftp_rx(struct tftp_state_data *state,
                         tftp_event_t event)
{
  ssize_t sbytes;
  int rblock;
  ...
  default:
    ...
    return FETCHE_TFTP_ILLEGAL; /* not really the perfect return code for
                                  this */
  }
  return FETCHE_OK;
}
...
static FETCHcode tftp_tx(struct tftp_state_data *state, tftp_event_t event)
{
  ...
  case TFTP_EVENT_OACK:
    ...
    do
    {
      result = Fetch_client_read(data, bufptr, state->blksize - state->sbytes,
                                &cb, &eos);
      if (result)
        return result;
      state->sbytes += (int)cb;
    ...
    Fetch_pgrsSetUploadCounter(data, k->writebytecount);
    break;
 ...
 * tftp_translate_code
 *
 * Translate internal error codes to FETCH error codes
 *
 **********************************************************/
static FETCHcode tftp_translate_code(tftp_error_t error)
{
  ...
  if (error != TFTP_ERR_NONE)
  {
    ...
    case TFTP_ERR_NOTFOUND:
      result = FETCHE_TFTP_NOTFOUND;
      break;
    case TFTP_ERR_PERM:
      result = FETCHE_TFTP_PERM;
      break;
    case TFTP_ERR_DISKFULL:
      result = FETCHE_REMOTE_DISK_FULL;
      break;
    case TFTP_ERR_UNDEF:
    case TFTP_ERR_ILLEGAL:
      result = FETCHE_TFTP_ILLEGAL;
      break;
    case TFTP_ERR_UNKNOWNID:
      result = FETCHE_TFTP_UNKNOWNID;
      break;
    case TFTP_ERR_EXISTS:
      result = FETCHE_REMOTE_FILE_EXISTS;
      break;
    case TFTP_ERR_NOSUCHUSER:
      result = FETCHE_TFTP_NOSUCHUSER;
      break;
 ...
 *
 **********************************************************/
static FETCHcode tftp_state_machine(struct tftp_state_data *state,
                                    tftp_event_t event)
{
  FETCHcode result = FETCHE_OK;
  struct Fetch_easy *data = state->data;
                                 ...
                                 struct connectdata *conn, bool dead_connection)
{
  ...
  if (state)
  {
    Fetch_safefree(state->rpacket.data);
    Fetch_safefree(state->spacket.data);
    free(state);
  }
...
static FETCHcode tftp_connect(struct Fetch_easy *data, bool *done)
{
  ...
  ((struct sockaddr *)&state->local_addr)->sa_family =
      (FETCH_SA_FAMILY_T)(conn->remote_addr->family);
  ...
  if (!conn->bits.bound)
  {
    ...
    int rc = bind(state->sockfd, (struct sockaddr *)&state->local_addr,
                  (fetch_socklen_t)conn->remote_addr->addrlen);
    if (rc)
    {
      char buffer[STRERROR_LEN];
 ...
 *
 **********************************************************/
static FETCHcode tftp_done(struct Fetch_easy *data, FETCHcode status,
                           bool premature)
{
  FETCHcode result = FETCHE_OK;
  struct connectdata *conn = data->conn;
 ...
 *
 **********************************************************/
static int tftp_getsock(struct Fetch_easy *data,
 ...
 *
 **********************************************************/
static FETCHcode tftp_receive_packet(struct Fetch_easy *data)
{
  fetch_socklen_t fromlen;
  FETCHcode result = FETCHE_OK;
  struct connectdata *conn = data->conn;
  struct tftp_state_data *state = conn->proto.tftpc;
 ...
 *
 **********************************************************/
static timediff_t tftp_state_timeout(struct Fetch_easy *data,
                                     tftp_event_t *event)
{
  time_t current;
  struct connectdata *conn = data->conn;
  ...
  timeout_ms = Fetch_timeleft(state->data, NULL,
                             (state->state == TFTP_STATE_START));
  if (timeout_ms < 0)
  {
    state->error = TFTP_ERR_TIMEOUT;
  ...
  *done = FALSE;
  ...
  else
  {
    ...
    if (rc == -1)
    {
      ...
      int error = SOCKERRNO;
      char buffer[STRERROR_LEN];
      failf(data, "%s", Fetch_strerror(error, buffer, sizeof(buffer)));
 ...
 *
 **********************************************************/
static FETCHcode tftp_perform(struct Fetch_easy *data, bool *dophase_done)
{
  FETCHcode result = FETCHE_OK;
  struct connectdata *conn = data->conn;
  struct tftp_state_data *state = conn->proto.tftpc;
                                       ...
                                       struct connectdata *conn)
{
  ...
  if (type)
  {
    ...
    *type = 0; /* it was in the middle of the hostname */
    command = Fetch_raw_toupper(type[6]);
