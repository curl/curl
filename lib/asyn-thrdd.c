...
async_thrdd_item_create(const char *hostname,
                        uint16_t port, uint8_t ip_version)
{
  size_t hostlen = strlen(hostname);
  struct async_thrdd_item *item = curlx_calloc(1, sizeof(*item) + hostlen);
  if(!item)
    return NULL;
                          ...
                          struct Curl_dns_entry **pdns)
{
  ...
  CURL_TRC_DNS(data, "resolve await %s", async->hostname);
                                     ...
                                     uint32_t idle_time_ms)
{
  ...
  return Curl_thrdq_create(&multi->resolv_thrdq, "async-dns", 0,
                                ...
                                struct Curl_resolv_async *async)
{
  ...
  item = async_thrdd_item_create(async->hostname, async->port,
                                 async->ip_version);
  if(!item) {
...
out:
  if(item)
    async_thrdd_item_free(item);
  CURL_TRC_DNS(data, "queueing resolve of %s:%d -> %d",
                                ...
                                struct Curl_dns_entry **pdns)
{
  ...
  *pdns = NULL;
  ...
  CURL_TRC_DNS(data, "async_take %s:%d: %s -> %d",
               async->hostname, async->port,
               result ? "errored" : "resolved", result);
  return result;
}
