...
static CURLcode client_write(struct Curl_easy *data, const char *prefix1,
                             const char *prefix2,
...
static CURLcode sieve_follow(struct Curl_easy *data, CURLcode status)
{
  ...
  if(!result) {
    ...
    Curl_xfer_setup(data, FIRSTSOCKET, 0, FALSE, -1);
    ...
    if(items->type != SIEVE_ITEM_STRING)
      ...
      for(; itemcount--; items++) {
        ...
        case SIEVE_ITEM_ATOM:
          ...
          result = client_write(data, sep, NULL, ptr,
                                Curl_bufref_len(&items->str), NULL);
          break;
        case SIEVE_ITEM_STRING:
          ptr = (const char *) Curl_bufref_ptr(&items->str);
          if(can_quote(ptr, Curl_bufref_len(&items->str)))
            result = client_write(data, sep, "\"", ptr,
            ...
            if(!s)
              ...
              result = client_write(data, sep, s, ptr,
                                    Curl_bufref_len(&items->str), NULL);
              free(s);
            }
          }
        ...
        case SIEVE_ITEM_NUMBER:
          ...
          if(!s)
            ...
            result = client_write(data, sep, s, NULL, 0, NULL);
            free(s);
          }
          break;
        case SIEVE_ITEM_LPARENT:
          result = client_write(data, sep, "(", NULL, 0, NULL);
          nextsep = "";
          break;
        case SIEVE_ITEM_RPARENT:
          result = client_write(data, ")", NULL, NULL, 0, NULL);
          break;
        default:
          result = unexpected_response(data, "LISTSCRIPTS", sieve->custom);
          break;
                                           ...
                                           size_t prefixsize)
{
  ...
  if(prefixsize) {
    result = client_write(data, NULL, NULL, prefix, prefixsize, NULL);
  ...
  if(items->type == SIEVE_ITEM_STRING) {
    ...
    if(len) {
      if(warnings || len >= CURL_ERROR_SIZE || !can_quote(ptr, len)) {
        result = client_write(data, NULL, NULL, ptr, len, NULL);
        if(!result)
