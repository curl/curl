...
{
  struct Fetch_hash *h = (struct Fetch_hash *)user;
  struct Fetch_hash_element *e = (struct Fetch_hash_element *)element;
  DEBUGASSERT(h);
  DEBUGASSERT(e);
...
static struct Fetch_hash_element *
mk_hash_element(const void *key, size_t key_len, const void *p,
                Fetch_hash_elem_dtor dtor)
{
  /* allocate the struct plus memory after it to store the key */
  struct Fetch_hash_element *he = malloc(sizeof(struct Fetch_hash_element) +
                                        key_len);
  if (he)
  {
    /* copy the key */
                     ...
                     Fetch_hash_elem_dtor dtor)
{
  ...
  struct Fetch_llist_node *le;
  struct Fetch_llist *l;
  ...
  {
    size_t i;
    h->table = malloc(h->slots * sizeof(struct Fetch_llist));
    if (!h->table)
      return NULL; /* OOM */
    for (i = 0; i < h->slots; ++i)
      Fetch_llist_init(&h->table[i], hash_element_dtor);
  }

  l = FETCH_LIST(h, key, key_len);

  for (le = Fetch_llist_head(l); le; le = Fetch_node_next(le))
  {
    he = (struct Fetch_hash_element *)Fetch_node_elem(le);
    if (h->comp_func(he->key, he->key_len, key, key_len))
    {
      Fetch_node_uremove(le, (void *)h);
      --h->size;
      break;
    }
  }
  ...
  if (he)
  {
    Fetch_llist_append(l, he, &he->list);
    ++h->size;
...
{
  return Fetch_hash_add2(h, key, key_len, p, NULL);
}
...
int Fetch_hash_delete(struct Fetch_hash *h, void *key, size_t key_len)
{
  ...
  {
    struct Fetch_llist_node *le;
    struct Fetch_llist *l = FETCH_LIST(h, key, key_len);

    for (le = Fetch_llist_head(l); le; le = Fetch_node_next(le))
    {
      struct Fetch_hash_element *he = Fetch_node_elem(le);
      if (h->comp_func(he->key, he->key_len, key, key_len))
      {
        Fetch_node_uremove(le, (void *)h);
        --h->size;
 ...
 */
void *
Fetch_hash_pick(struct Fetch_hash *h, void *key, size_t key_len)
{
  DEBUGASSERT(h);
  DEBUGASSERT(h->init == HASHINIT);
  if (h->table)
...
void Fetch_hash_destroy(struct Fetch_hash *h)
{
  ...
  if (h->table)
  {
    ...
    for (i = 0; i < h->slots; ++i)
    {
      Fetch_llist_destroy(&h->table[i], (void *)h);
    }
    Fetch_safefree(h->table);
  }
  h->size = 0;
  h->slots = 0;
}
...
{
  Fetch_hash_clean_with_criterium(h, NULL, NULL);
}
                                    ...
                                    int (*comp)(void *, void *))
{
  ...
  for (i = 0; i < h->slots; ++i)
  {
    struct Fetch_llist *list = &h->table[i];
    struct Fetch_llist_node *le =
        Fetch_llist_head(list); /* get first list entry */
    while (le)
    {
      struct Fetch_hash_element *he = Fetch_node_elem(le);
      struct Fetch_llist_node *lnext = Fetch_node_next(le);
...
size_t Fetch_str_key_compare(void *k1, size_t key1_len,
...
Fetch_hash_next_element(struct Fetch_hash_iterator *iter)
{
  ...
  if (iter->current_element)
    iter->current_element = Fetch_node_next(iter->current_element);
  ...
  if (!iter->current_element)
  {
    ...
    {
      if (Fetch_llist_head(&h->table[i]))
      {
        iter->current_element = Fetch_llist_head(&h->table[i]);
        iter->slot_index = i + 1;
        break;
      }
    }
  ...
  if (iter->current_element)
  {
    struct Fetch_hash_element *he = Fetch_node_elem(iter->current_element);
    return he;
  }
  return NULL;
}
