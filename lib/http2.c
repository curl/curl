...
static void cf_h2_ctx_set_data(struct Curl_cfilter *cf,
                               struct Curl_easy *data)
{
  if(cf->ctx)
    ((struct cf_h2_ctx *)cf->ctx)->call_data = data;
}
                             ...
                             void *userp)
{
  ...
  struct cf_h2_ctx *ctx = cf->ctx;
  struct Curl_easy *data = ctx->call_data;
  ssize_t written;
                        ...
                        const nghttp2_push_promise *frame)
{
  ...
  DEBUGF(LOG_CF(data, cf, "PUSH_PROMISE received, stream %u",
                         ...
                         void *userp)
{
  ...
  case NGHTTP2_RST_STREAM:
    DEBUGF(LOG_CF(data_s, cf, "recv frame RST for stream %u", stream_id));
    stream->reset = TRUE;
    break;
  default:
                              ...
                              const uint8_t *mem, size_t len, void *userp)
{
  ...
  if(!data_s) {
    ...
    DEBUGF(LOG_CF(ctx->call_data, cf, "Data for unknown stream %u",
                  stream_id));
    return 0;
  }
  ...
  DEBUGF(LOG_CF(data_s, cf, "%zu data received for stream %u "
                "(%zu left in buffer %p, total %zu)",
                nread, stream_id,
                stream->len, (void *)stream->mem,
                stream->memlen));

  if(nread < len) {
    stream->pausedata = mem + nread;
    stream->pauselen = len - nread;
    DEBUGF(LOG_CF(data_s, cf, "NGHTTP2_ERR_PAUSE - %zu bytes out of buffer"
                  ", stream %u",
                  len - nread, stream_id));
                           ...
                           uint32_t error_code, void *userp)
{
  ...
  if(stream_id) {
    ...
    DEBUGF(LOG_CF(data_s, cf, "on_stream_close(), %s (err %d), stream %u",
    ...
    if(stream_id == ctx->pause_stream_id) {
      DEBUGF(LOG_CF(data_s, cf, "Stopped the pause stream"));
      ctx->pause_stream_id = 0;
    }
    DEBUGF(LOG_CF(data_s, cf, "Removed stream %u hash", stream_id));
                     ...
                     void *userp)
{
  ...
  if(stream->bodystarted) {
    /* This is a trailer */
    DEBUGF(LOG_CF(data_s, cf, "h2 trailer: %.*s: %.*s", (int)namelen, name,
                  (int)valuelen, value));
  ...
  if(namelen == sizeof(H2H3_PSEUDO_STATUS) - 1 &&
    ...
    DEBUGF(LOG_CF(data_s, cf, "h2 status: HTTP/2 %03d (easy %p)",
                  stream->status_code, (void *)data_s));
    return 0;
  }
  ...
  DEBUGF(LOG_CF(data_s, cf, "h2 header: %.*s: %.*s", (int)namelen, name,
                (int)valuelen, value));
                                         ...
                                         void *userp)
{
  ...
  DEBUGF(LOG_CF(data_s, cf, "data_source_read_callback: "
                            ...
                            struct Curl_easy *data, bool premature)
{
  ...
  if(stream->stream_id == ctx->pause_stream_id) {
    DEBUGF(LOG_CF(data, cf, "DONE the pause stream (%u)", stream->stream_id));
    ctx->pause_stream_id = 0;
  }

  if(premature || (!stream->closed && stream->stream_id)) {
    /* RST_STREAM */
    DEBUGF(LOG_CF(data, cf, "RST stream %u", stream->stream_id));
                                    ...
                                    CURLcode *err)
{
  ...
  if(nread == rv) {
    DEBUGF(LOG_CF(data, cf, "h2_process_pending_input: All data in "
                  "connection buffer processed"));
    ctx->inbuflen = 0;
    ctx->nread_inbuf = 0;
  }
  else {
                                         ...
                                         struct HTTP *stream, CURLcode *err)
{
  ...
  if(stream->error == NGHTTP2_REFUSED_STREAM) {
    DEBUGF(LOG_CF(data, cf, "REFUSED_STREAM (%u), try again on a new "
...
static int h2_session_send(struct Curl_cfilter *cf, struct Curl_easy *data)
{
  ...
  if((sweight_wanted(data) != sweight_in_effect(data)) ||
    ...
    DEBUGF(LOG_CF(data, cf, "Queuing PRIORITY on stream %u (easy %p)",
                  stream->stream_id, (void *)data));
    DEBUGASSERT(stream->stream_id != -1);
                          ...
                          char *buf, size_t len, CURLcode *err)
{
  ...
  if(stream->bodystarted &&
    ...
    DEBUGF(LOG_CF(data, cf, "http2_recv: Got %d bytes from header_recvbuf",
                  (int)ncopy));
    nread = ncopy;
    goto out;
  }

  DEBUGF(LOG_CF(data, cf, "http2_recv: easy %p (stream %u) win %u/%u",
                (void *)data, stream->stream_id,
  ...
  if((data->state.drain) && stream->memlen) {
    DEBUGF(LOG_CF(data, cf, "http2_recv: DRAIN %zu bytes stream %u (%p => %p)",
    ...
    if(stream->pauselen == 0) {
      DEBUGF(LOG_CF(data, cf, "Unpaused by stream %u", stream->stream_id));
      DEBUGASSERT(ctx->pause_stream_id == stream->stream_id);
      ctx->pause_stream_id = 0;
    ...
    DEBUGF(LOG_CF(data, cf, "http2_recv: returns unpaused %zd bytes on "
                  "stream %u", nread, stream->stream_id));
    goto out;
  }
    ...
    DEBUGF(LOG_CF(data, cf, "stream %u is paused, pause id: %u",
    ...
    *err = CURLE_AGAIN;
    ...
    if(ctx->inbuflen == 0) {
      ...
      if(nread == 0) {
        ...
        DEBUGF(LOG_CF(data, cf, "end of stream"));
        *err = CURLE_OK;
        nread = 0;
        goto out;
      }

      DEBUGF(LOG_CF(data, cf, "nread=%zd", nread));
  ...
  if(stream->memlen) {
    ...
    DEBUGF(LOG_CF(data, cf, "http2_recv: returns %zd for stream %u",
    ...
    if(ctx->pause_stream_id == stream->stream_id) {
      ...
      DEBUGF(LOG_CF(data, cf, "Data returned for PAUSED stream %u",
                    stream->stream_id));
    }
    else if(!stream->closed) {
      drained_transfer(cf, data);
  ...
  *err = CURLE_AGAIN;
  nread = -1;
  DEBUGF(LOG_CF(data, cf, "http2_recv returns AGAIN for stream %u",
                stream->stream_id));
out:
  cf_h2_ctx_set_data(cf, NULL);
  return nread;
}
                          ...
                          const void *buf, size_t len, CURLcode *err)
{
  ...
  DEBUGF(LOG_CF(data, cf, "http2_send len=%zu", len));
  cf_h2_ctx_set_data(cf, data);
  ...
  if(stream->stream_id != -1) {
    ...
    if(should_close_session(ctx)) {
      DEBUGF(LOG_CF(data, cf, "http2_send: nothing to do in this session"));
      *err = CURLE_HTTP2;
      len = -1;
      goto out;
    }
  ...
  DEBUGF(LOG_CF(data, cf, "http2_send request allowed %d (easy handle %p)",
  ...
  if(stream_id < 0) {
    DEBUGF(LOG_CF(data, cf, "http2_send() nghttp2_submit_request error (%s)%u",
  ...
  rv = h2_session_send(cf, data);
  if(rv) {
    DEBUGF(LOG_CF(data, cf, "http2_send() nghttp2_session_send error (%s)%d",
