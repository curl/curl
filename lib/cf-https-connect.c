                                 ...
                                 struct cf_hc_baller *winner)
{
  ...
  CURL_LOG_CF(data, cf, "connect+handshake %s: %dms, 1st data: %dms",
                              ...
                              struct curltime now)
{
  ...
  if(elapsed_ms >= ctx->hard_eyeballs_timeout_ms) {
    CURL_LOG_CF(data, cf, "hard timeout of %dms reached, starting h21",
  ...
  if(elapsed_ms >= ctx->soft_eyeballs_timeout_ms) {
    if(cf_hc_baller_reply_ms(&ctx->h3_baller, data) < 0) {
      CURL_LOG_CF(data, cf, "soft timeout of %dms reached, h3 has not "
                              ...
                              bool blocking, bool *done)
{
  ...
  *done = FALSE;
  ...
  case CF_HC_INIT:
    ...
    DEBUGASSERT(!cf->next);
    CURL_LOG_CF(data, cf, "connect, init");
    ctx->started = now;
  ...
  case CF_HC_CONNECT:
    ...
    if(cf_hc_baller_is_active(&ctx->h21_baller)) {
      CURL_LOG_CF(data, cf, "connect, check h21");
    ...
    if((!ctx->h3_baller.enabled || ctx->h3_baller.result) &&
      ...
      CURL_LOG_CF(data, cf, "connect, all failed");
...
out:
  CURL_LOG_CF(data, cf, "connect -> %d, done=%d", result, *done);
  return result;
}
                                  ...
                                  curl_socket_t *socks)
{
  ...
  for(i = s = 0; i < sizeof(ballers)/sizeof(ballers[0]); i++) {
    ...
    CURL_LOG_CF(data, cf, "get_selected_socks(%s) -> %x", b->name, brc);
    if(!brc)
      continue;
  ...
  CURL_LOG_CF(data, cf, "get_selected_socks -> %x", rc);
  return rc;
}
                               ...
                               const struct Curl_easy *data)
{
  ...
  CURL_LOG_CF((struct Curl_easy *)data, cf, "data_pending");
