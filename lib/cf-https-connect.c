                                 ...
                                 struct cf_hc_baller *winner)
{
  ...
  if(reply_ms >= 0)
    CURL_TRC_CF(data, cf, "connect+handshake %s: %dms, 1st data: %dms",
                winner->name,
                (int)curlx_timediff_ms(data->state.now,
                                       winner->started), reply_ms);
  else
    CURL_TRC_CF(data, cf, "deferred handshake %s: %dms",
                winner->name, (int)curlx_timediff_ms(data->state.now,
                                                     winner->started));
                              ...
                              bool *done)
{
  ...
  *done = FALSE;
  ...
  case CF_HC_CONNECT:
    ...
    if(time_to_start_next(cf, data, 1, data->state.now)) {
