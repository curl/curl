...
CURLcode Curl_output_aws_sigv4(struct Curl_easy *data)
{
  ...
  */
  ...
  /* Override timestamp if user provided X-Amz-Date */
  if(querystring_mode && data->state.up.query) {
    const char *date_param = strstr(data->state.up.query, "X-Amz-Date=");
    if(date_param) {
      const char *end;
      size_t len;
      date_param += 11; /* Skip "X-Amz-Date=" */
      end = strchr(date_param, '&');
      len = end ? (size_t)(end - date_param) : strlen(date_param);
      if(len == 16) { /* YYYYMMDDTHHMMSSZ */
        memcpy(timestamp, date_param, len);
        timestamp[len] = 0;
        user_provided_date = true;
        infof(data, "aws_sigv4: using x-amz-date override='%s'", timestamp);
      }
    }
  }
  else if(!querystring_mode) {
    char *explicit_date = Curl_checkheaders(data, STRCONST("X-Amz-Date"));
    if(explicit_date) {
  ...
  if(security_token && !querystring_mode) {
    /* Only add X-Amz-Security-Token if not already provided by user */
    if(!Curl_checkheaders(data, STRCONST("X-Amz-Security-Token"))) {
      char *security_token_hdr = curl_maprintf("X-Amz-Security-Token: %s",
                                             security_token);
  ...
  /* Add X-Amz-Date header for header mode before canonicalization */
  if(!querystring_mode) {
    /* Only add X-Amz-Date if not already provided by user */
    if(!user_provided_date) {
      char *date_hdr = curl_maprintf("X-Amz-Date: %s", timestamp);
      if(!date_hdr) {
        result = CURLE_OUT_OF_MEMORY;
        goto fail;
      }
    ...
    /* Add X-Amz-Region-Set header if region_set_or_region specified */
    if(curlx_strlen(&region_set_or_region) && sigv4a_mode) {
      char *region_set_hdr = curl_maprintf("X-Amz-Region-Set: %.*s",
                                           (int)curlx_strlen(
  ...
  /* If user provided explicit X-Amz-Date, use it for credential scope */
  if(querystring_mode && data->state.up.query) {
    const char *date_param = strstr(data->state.up.query, "X-Amz-Date=");
    if(date_param) {
      date_param += 11; /* Skip "X-Amz-Date=" */
    ...
    char *explicit_date = Curl_checkheaders(data, STRCONST("X-Amz-Date"));
    if(explicit_date) {
      /* Find the value part after "X-Amz-Date: " */
      char *date_value = strchr(explicit_date, ':');
      if(date_value) {
        date_value++; /* Skip ':' */
  ...
  if(querystring_mode) {
    ...
    /* Add signature parameters - let canon_query() sort them */
    {
      ...
      result = curlx_dyn_addf(&temp_query,
                              ...
                              algorithm);
      if(!result)
        result = curlx_dyn_addf(&temp_query, "&X-Amz-Credential=%s",
                                credential_string);
      if(!result)
        /* Only add X-Amz-Date if not already in query string */
        if(!has_query_param(data->state.up.query, "X-Amz-Date"))
          result = curlx_dyn_addf(&temp_query, "&X-Amz-Date=%s", timestamp);
      if(!result)
        result = curlx_dyn_addf(&temp_query, "&X-Amz-SignedHeaders=%s",
                                curlx_dyn_ptr(&signed_headers));

      if(!result && curlx_strlen(&region_set_or_region) && sigv4a_mode) {
        result = curlx_dyn_addf(&temp_query, "&X-Amz-Region-Set=%.*s",
                                (int)curlx_strlen(&region_set_or_region),
                                curlx_str(&region_set_or_region));
      }

      if(!result && security_token) {
        result = curlx_dyn_addf(&temp_query, "&X-Amz-Security-Token=%s",
                                security_token);
      }
  ...
  if(querystring_mode) {
    ...
    /* Build AWS signature parameters only */
    {
      ...
      if(encoded_region_set) {
        /* Check if X-Amz-Date already exists in query */
        bool has_date = has_query_param(data->state.up.query, "X-Amz-Date");
        if(has_date) {
    ...
    if(security_token) {
      ...
      if(encoded_security_token) {
        result = curlx_dyn_addf(&aws_params, "&X-Amz-Security-Token=%s",
