...
CURLcode Curl_output_aws_sigv4(struct Curl_easy *data)
{
  ...
  */
  ...
  infof(data, "aws_sigv4: Canonical request - (enclosed in [])");
  infof(data, "[ %s ]", canonical_request);
  ...
  infof(data, "aws_sigv4: String to sign - (enclosed in [])");
  infof(data, "[ %s ]", str_to_sign);
