...
dnl FETCH_CHECK_DEF (SYMBOL, [INCLUDES], [SILENT])
dnl -------------------------------------------------
...
AC_DEFUN([FETCH_CHECK_DEF], [
  AC_REQUIRE([FETCH_CPP_P])dnl
  OLDCPPFLAGS=$CPPFLAGS
  # CPPPFLAG comes from FETCH_CPP_P
  CPPFLAGS="$CPPFLAGS $CPPPFLAG"
  AS_VAR_PUSHDEF([ac_HaveDef], [fetch_cv_have_def_$1])dnl
  AS_VAR_PUSHDEF([ac_Def], [fetch_cv_def_$1])dnl
  if test -z "$SED"; then
...
ifelse($2,,,[$2])[[
    ...
    tmp_exp=`eval "$ac_cpp conftest.$ac_ext" 2>/dev/null | \
      "$GREP" FETCH_DEF_TOKEN 2>/dev/null | \
      "$SED" 's/.*FETCH_DEF_TOKEN[[ ]][[ ]]*//' 2>/dev/null | \
      "$SED" 's/[["]][[ ]]*[["]]//g' 2>/dev/null`
...
dnl FETCH_CHECK_DEF_CC (SYMBOL, [INCLUDES], [SILENT])
dnl -------------------------------------------------
...
dnl FETCH_CHECK_LIB_XNET
dnl -------------------------------------------------
dnl Verify if X/Open network library is required.

AC_DEFUN([FETCH_CHECK_LIB_XNET], [
...
#  undef _ALL_SOURCE
  ...
  AC_BEFORE([$0], [FETCH_CONFIGURE_REENTRANT])dnl
...
dnl FETCH_CHECK_NATIVE_WINDOWS
dnl -------------------------------------------------
dnl Check if building a native Windows target

AC_DEFUN([FETCH_CHECK_NATIVE_WINDOWS], [
  AC_CACHE_CHECK([whether build target is a native Windows one], [fetch_cv_native_windows], [
    AC_COMPILE_IFELSE([
      AC_LANG_PROGRAM([[
      ]],[[
        #ifdef _WIN32
      ...
      fetch_cv_native_windows="yes"
    ],[
      fetch_cv_native_windows="no"
    ])
  ])
  AM_CONDITIONAL(DOING_NATIVE_WINDOWS, test "x$fetch_cv_native_windows" = xyes)
])


dnl FETCH_CHECK_HEADER_LBER
dnl -------------------------------------------------
dnl Check for compilable and valid lber.h header,
dnl and check if it is needed even with ldap.h

AC_DEFUN([FETCH_CHECK_HEADER_LBER], [
  AC_REQUIRE([FETCH_CHECK_NATIVE_WINDOWS])dnl
  AC_CACHE_CHECK([for lber.h], [fetch_cv_header_lber_h], [
    AC_COMPILE_IFELSE([
      AC_LANG_PROGRAM([[
        #undef inline
        #ifdef _WIN32
      ...
      fetch_cv_header_lber_h="yes"
    ],[
      fetch_cv_header_lber_h="no"
    ])
  ])
  if test "$fetch_cv_header_lber_h" = "yes"; then
    ...
    AC_COMPILE_IFELSE([
      ...
      fetch_cv_need_header_lber_h="no"
    ],[
      fetch_cv_need_header_lber_h="yes"
    ])
    #
    case "$fetch_cv_need_header_lber_h" in
      yes)
...
dnl FETCH_CHECK_HEADER_LDAP
dnl -------------------------------------------------
dnl Check for compilable and valid ldap.h header

AC_DEFUN([FETCH_CHECK_HEADER_LDAP], [
  AC_REQUIRE([FETCH_CHECK_HEADER_LBER])dnl
  AC_CACHE_CHECK([for ldap.h], [fetch_cv_header_ldap_h], [
    AC_COMPILE_IFELSE([
      AC_LANG_PROGRAM([[
        #undef inline
        #ifdef _WIN32
      ...
      fetch_cv_header_ldap_h="yes"
    ],[
      fetch_cv_header_ldap_h="no"
    ])
  ])
  case "$fetch_cv_header_ldap_h" in
    yes)
...
dnl FETCH_CHECK_HEADER_LDAP_SSL
dnl -------------------------------------------------
dnl Check for compilable and valid ldap_ssl.h header

AC_DEFUN([FETCH_CHECK_HEADER_LDAP_SSL], [
  AC_REQUIRE([FETCH_CHECK_HEADER_LDAP])dnl
  AC_CACHE_CHECK([for ldap_ssl.h], [fetch_cv_header_ldap_ssl_h], [
    AC_COMPILE_IFELSE([
      AC_LANG_PROGRAM([[
        #undef inline
        #ifdef _WIN32
      ...
      fetch_cv_header_ldap_ssl_h="yes"
    ],[
      fetch_cv_header_ldap_ssl_h="no"
    ])
  ])
  case "$fetch_cv_header_ldap_ssl_h" in
    yes)
...
AC_DEFUN([FETCH_CHECK_LIBS_WINLDAP], [
  AC_REQUIRE([FETCH_CHECK_HEADER_WINBER])dnl
  #
  ...
  #
  fetch_cv_save_LIBS="$LIBS"
  fetch_cv_ldap_LIBS="unknown"
  #
  for x_nlibs in '' "$u_libs" \
    '-lwldap32' ; do
    if test "$fetch_cv_ldap_LIBS" = "unknown"; then
      if test -z "$x_nlibs"; then
        LIBS="$fetch_cv_save_LIBS"
      else
        LIBS="$x_nlibs $fetch_cv_save_LIBS"
      fi
      AC_LINK_IFELSE([
        AC_LANG_PROGRAM([[
          #undef inline
  ...
  case X-"$fetch_cv_ldap_LIBS" in
    ...
    X-)
      ...
      ;;
    *)
      if test -z "$fetch_cv_save_LIBS"; then
        LIBS="$fetch_cv_ldap_LIBS"
      else
        LIBS="$fetch_cv_ldap_LIBS $fetch_cv_save_LIBS"
      fi
      AC_MSG_RESULT([$fetch_cv_ldap_LIBS])
      ;;
  esac
  #
])
...
AC_DEFUN([FETCH_CHECK_LIBS_LDAP], [
  AC_REQUIRE([FETCH_CHECK_HEADER_LDAP])dnl
  #
  ...
  case X-"$fetch_cv_ldap_LIBS" in
    ...
    *)
      ...
      if false; then
        LIBFETCH_PC_REQUIRES_PRIVATE="ldap $LIBFETCH_PC_REQUIRES_PRIVATE"
      fi
      AC_MSG_RESULT([$fetch_cv_ldap_LIBS])
      ;;
  esac
...
dnl FETCH_CHECK_FUNC_RECV
dnl -------------------------------------------------
dnl Test if the socket recv() function is available,

AC_DEFUN([FETCH_CHECK_FUNC_RECV], [
  AC_REQUIRE([FETCH_CHECK_NATIVE_WINDOWS])dnl
  ...
  #
  if test "$fetch_cv_recv" = "yes"; then
    AC_DEFINE_UNQUOTED(HAVE_RECV, 1,
      [Define to 1 if you have the recv function.])
    fetch_cv_func_recv="yes"
  else
...
dnl FETCH_CHECK_FUNC_SEND
dnl -------------------------------------------------
dnl Test if the socket send() function is available,

AC_DEFUN([FETCH_CHECK_FUNC_SEND], [
  AC_REQUIRE([FETCH_CHECK_NATIVE_WINDOWS])dnl
  AC_REQUIRE([FETCH_INCLUDES_BSDSOCKET])dnl
  AC_CHECK_HEADERS(sys/types.h sys/socket.h)
  #
  ...
  #
  if test "$fetch_cv_send" = "yes"; then
    AC_DEFINE_UNQUOTED(HAVE_SEND, 1,
      [Define to 1 if you have the send function.])
    fetch_cv_func_send="yes"
  else
    AC_MSG_ERROR([Unable to link function send])
  fi
])

dnl FETCH_CHECK_MSG_NOSIGNAL
dnl -------------------------------------------------
dnl Check for MSG_NOSIGNAL

AC_DEFUN([FETCH_CHECK_MSG_NOSIGNAL], [
  AC_CHECK_HEADERS(sys/types.h sys/socket.h)
  AC_CACHE_CHECK([for MSG_NOSIGNAL], [fetch_cv_msg_nosignal], [
    AC_COMPILE_IFELSE([
      AC_LANG_PROGRAM([[
        #undef inline
        #ifdef _WIN32
      ...
      fetch_cv_msg_nosignal="yes"
    ],[
      fetch_cv_msg_nosignal="no"
    ])
  ])
  case "$fetch_cv_msg_nosignal" in
    yes)
...
dnl FETCH_CHECK_STRUCT_TIMEVAL
dnl -------------------------------------------------
dnl Check for timeval struct

AC_DEFUN([FETCH_CHECK_STRUCT_TIMEVAL], [
  AC_REQUIRE([FETCH_CHECK_NATIVE_WINDOWS])dnl
  AC_CHECK_HEADERS(sys/types.h sys/time.h sys/socket.h)
  AC_CACHE_CHECK([for struct timeval], [fetch_cv_struct_timeval], [
    AC_COMPILE_IFELSE([
      AC_LANG_PROGRAM([[
        #undef inline
        #ifdef _WIN32
      ...
      fetch_cv_struct_timeval="yes"
    ],[
      fetch_cv_struct_timeval="no"
    ])
  ])
  case "$fetch_cv_struct_timeval" in
    yes)
...
AC_DEFUN([TYPE_IN_ADDR_T], [
  AC_CHECK_TYPE([in_addr_t], ,[
    ...
    AC_CACHE_CHECK([for in_addr_t equivalent],
      [fetch_cv_in_addr_t_equiv], [
      fetch_cv_in_addr_t_equiv="unknown"
      for t in "unsigned long" int size_t unsigned long; do
        if test "$fetch_cv_in_addr_t_equiv" = "unknown"; then
          AC_LINK_IFELSE([
            AC_LANG_PROGRAM([[
              #undef inline
              #ifdef _WIN32
            ...
            fetch_cv_in_addr_t_equiv="$t"
          ])
        fi
      done
    ])
    case "$fetch_cv_in_addr_t_equiv" in
      unknown)
        AC_MSG_ERROR([Cannot find a type to use in place of in_addr_t])
        ;;
      *)
        AC_DEFINE_UNQUOTED(in_addr_t, $fetch_cv_in_addr_t_equiv,
...
dnl FETCH_CHECK_FUNC_CLOCK_GETTIME_MONOTONIC
dnl -------------------------------------------------
dnl Check if monotonic clock_gettime is available.

AC_DEFUN([FETCH_CHECK_FUNC_CLOCK_GETTIME_MONOTONIC], [
  ...
  if test "x$dontwant_rt" = "xno" ; then
    AC_COMPILE_IFELSE([
      ...
      AC_MSG_RESULT([yes])
      fetch_func_clock_gettime="yes"
    ],[
      AC_MSG_RESULT([no])
      fetch_func_clock_gettime="no"
    ])
  fi
...
dnl FETCH_CHECK_FUNC_CLOCK_GETTIME_MONOTONIC_RAW
dnl -------------------------------------------------
dnl Check if monotonic clock_gettime is available.

AC_DEFUN([FETCH_CHECK_FUNC_CLOCK_GETTIME_MONOTONIC_RAW], [
  AC_CHECK_HEADERS(sys/types.h sys/time.h)
...
dnl FETCH_CHECK_LIBS_CLOCK_GETTIME_MONOTONIC
dnl -------------------------------------------------
dnl If monotonic clock_gettime is available then,
dnl check and prepended to LIBS any needed libraries.

AC_DEFUN([FETCH_CHECK_LIBS_CLOCK_GETTIME_MONOTONIC], [
  AC_REQUIRE([FETCH_CHECK_FUNC_CLOCK_GETTIME_MONOTONIC])dnl
  #
  if test "$fetch_func_clock_gettime" = "yes"; then
    #
    AC_MSG_CHECKING([for clock_gettime in libraries])
    #
    fetch_cv_save_LIBS="$LIBS"
    fetch_cv_gclk_LIBS="unknown"
    #
    for x_xlibs in '' '-lrt' '-lposix4' ; do
      if test "$fetch_cv_gclk_LIBS" = "unknown"; then
        if test -z "$x_xlibs"; then
          LIBS="$fetch_cv_save_LIBS"
        else
          LIBS="$x_xlibs $fetch_cv_save_LIBS"
        fi
        AC_LINK_IFELSE([
          AC_LANG_PROGRAM([[
            #ifdef HAVE_SYS_TYPES_H
    ...
    case X-"$fetch_cv_gclk_LIBS" in
      ...
      *)
        if test -z "$fetch_cv_save_LIBS"; then
          LIBS="$fetch_cv_gclk_LIBS"
        else
          LIBS="$fetch_cv_gclk_LIBS $fetch_cv_save_LIBS"
        fi
        AC_MSG_RESULT([$fetch_cv_gclk_LIBS])
        fetch_func_clock_gettime="yes"
        ;;
    esac
    #
    ...
    if test "x$cross_compiling" != "xyes" &&
      test "$fetch_func_clock_gettime" = "yes"; then
    ...
    #
    case "$fetch_func_clock_gettime" in
      yes)
...
dnl FETCH_CHECK_FUNC_SELECT
dnl -------------------------------------------------
dnl Test if the socket select() function is available.

AC_DEFUN([FETCH_CHECK_FUNC_SELECT], [
  AC_REQUIRE([FETCH_CHECK_STRUCT_TIMEVAL])dnl
  AC_REQUIRE([FETCH_INCLUDES_BSDSOCKET])dnl
  ...
  #
  if test "$fetch_cv_select" = "yes"; then
    AC_DEFINE_UNQUOTED(HAVE_SELECT, 1,
      [Define to 1 if you have the select function.])
    fetch_cv_func_select="yes"
  fi
])


dnl FETCH_VERIFY_RUNTIMELIBS
dnl -------------------------------------------------
...
AC_DEFUN([FETCH_VERIFY_RUNTIMELIBS], [
...
AC_DEFUN([FETCH_CHECK_CA_BUNDLE], [
...
AS_HELP_STRING([--without-ca-path], [Don't use a default CA path]),
  ...
  if test "x$want_ca" != "xno" -a "x$want_ca" != "xunset" -a \
    ...
    if test "x$cross_compiling" != "xyes" -a \
            "x$fetch_cv_native_windows" != "xyes"; then
      dnl NOT cross-compiling and...
      dnl neither of the --with-ca-* options are provided
      if test "x$want_ca" = "xunset"; then
        dnl the path we previously would have installed the fetch CA bundle
        dnl to, and thus we now check for an already existing cert in that
        dnl place in case we find no other
        if test "x$prefix" != xNONE; then
          cac="${prefix}/share/fetch/fetch-ca-bundle.crt"
        else
          cac="$ac_default_prefix/share/fetch/fetch-ca-bundle.crt"
        fi
  ...
  if test "x$ca" != "xno"; then
    FETCH_CA_BUNDLE="$ca"
    AC_DEFINE_UNQUOTED(FETCH_CA_BUNDLE, "$ca", [Location of default ca bundle])
    AC_SUBST(FETCH_CA_BUNDLE)
    AC_MSG_RESULT([$ca])
  fi
  if test "x$capath" != "xno"; then
    FETCH_CA_PATH="\"$capath\""
    AC_DEFINE_UNQUOTED(FETCH_CA_PATH, "$capath", [Location of default ca path])
...
AS_HELP_STRING([--without-ca-fallback], [Don't use the built-in CA store of the SSL library]),
  ...
  if test "x$with_ca_fallback" = "xyes"; then
    ...
    AC_DEFINE_UNQUOTED(FETCH_CA_FALLBACK, 1, [define "1" to use built-in CA store of SSL library])
  fi
])


dnl FETCH_CHECK_CA_EMBED
dnl -------------------------------------------------
dnl Check if a ca-bundle should be embedded

AC_DEFUN([FETCH_CHECK_CA_EMBED], [

  AC_MSG_CHECKING([CA cert bundle path to embed in the fetch tool])

  AC_ARG_WITH(ca-embed,
AS_HELP_STRING([--with-ca-embed=FILE],
  [Absolute path to a file containing CA certificates to embed in the fetch tool (example: /etc/ca-bundle.crt)])
AS_HELP_STRING([--without-ca-embed], [Don't embed a default CA bundle in the fetch tool]),
  [
  ...
  if test "x$want_ca_embed" != "xno" -a "x$want_ca_embed" != "xunset" -a -f "$want_ca_embed"; then
    FETCH_CA_EMBED="$want_ca_embed"
    AC_SUBST(FETCH_CA_EMBED)
...
dnl FETCH_CHECK_WIN32_LARGEFILE
dnl -------------------------------------------------
dnl Check if fetch's Win32 large file will be used

AC_DEFUN([FETCH_CHECK_WIN32_LARGEFILE], [
  AC_REQUIRE([FETCH_CHECK_NATIVE_WINDOWS])dnl
  if test "$fetch_cv_native_windows" = 'yes'; then
    ...
    case $host_os in
      mingw32ce*|cegcc*)
        ...
        fetch_win32_has_largefile='no'
        ;;
      *)
        dnl All mingw-w64 versions support large files
        fetch_win32_has_largefile='yes'
        ;;
    esac
    case "$fetch_win32_has_largefile" in
      yes)
...
dnl FETCH_CHECK_WIN32_CRYPTO
dnl -------------------------------------------------
dnl Check if fetch's Win32 crypto lib can be used

AC_DEFUN([FETCH_CHECK_WIN32_CRYPTO], [
  AC_REQUIRE([FETCH_CHECK_NATIVE_WINDOWS])dnl
  AC_MSG_CHECKING([whether build target supports Win32 crypto API])
  fetch_win32_crypto_api="no"
  if test "$fetch_cv_native_windows" = "yes" -a "$fetch_cv_winuwp" != "yes"; then
    AC_COMPILE_IFELSE([
      AC_LANG_PROGRAM([[
        #undef inline
        #ifndef WIN32_LEAN_AND_MEAN
      ...
      fetch_win32_crypto_api="yes"
    ])
  fi
  case "$fetch_win32_crypto_api" in
    yes)
      AC_MSG_RESULT([yes])
...
dnl FETCH_CHECK_PKGCONFIG ($module, [$pcdir])
dnl ------------------------
...
AC_DEFUN([FETCH_CHECK_PKGCONFIG], [
  ...
  if test "x$PKGCONFIG" != "xno"; then
    ...
    itexists=`FETCH_EXPORT_PCDIR([$2]) dnl
...
dnl FETCH_PREPARE_CONFIGUREHELP_PM
dnl -------------------------------------------------
...
AC_DEFUN([FETCH_PREPARE_CONFIGUREHELP_PM], [
  AC_REQUIRE([AC_PROG_CPP])dnl
  tmp_cpp=`eval echo "$ac_cpp" 2>/dev/null`
  if test -z "$tmp_cpp"; then
    tmp_cpp='cpp'
  fi
  AC_SUBST(FETCH_CPP, $tmp_cpp)
])


dnl FETCH_PREPARE_BUILDINFO
dnl -------------------------------------------------
dnl Save build info for test runner to pick up and log

AC_DEFUN([FETCH_PREPARE_BUILDINFO], [
  fetch_pflags=""
  case $host in
    *-apple-*) fetch_pflags="${fetch_pflags} APPLE";;
  esac
  case $host in
    *-*-*bsd*|*-*-aix*|*-*-hpux*|*-*-interix*|*-*-irix*|*-*-linux*|*-*-solaris*|*-*-sunos*|*-apple-*|*-*-cygwin*|*-*-msys*)
      fetch_pflags="${fetch_pflags} UNIX";;
  esac
  case $host in
    *-*-*bsd*)
      fetch_pflags="${fetch_pflags} BSD";;
  esac
  case $host in
    *-*-android*)
      fetch_pflags="${fetch_pflags} ANDROID"
      ANDROID_PLATFORM_LEVEL=`echo "$host_os" | $SED -ne 's/.*android\(@<:@0-9@:>@*\).*/\1/p'`
      if test -n "${ANDROID_PLATFORM_LEVEL}"; then
        fetch_pflags="${fetch_pflags}-${ANDROID_PLATFORM_LEVEL}"
      fi
      ;;
  esac
  if test "$fetch_cv_native_windows" = 'yes'; then
    fetch_pflags="${fetch_pflags} WIN32"
  fi
  if test "$fetch_cv_winuwp" = 'yes'; then
    fetch_pflags="${fetch_pflags} UWP"
  fi
  if test "$fetch_cv_cygwin" = 'yes'; then
    fetch_pflags="${fetch_pflags} CYGWIN"
  fi
  case $host_os in
    msys*) fetch_pflags="${fetch_pflags} MSYS";;
    msdos*) fetch_pflags="${fetch_pflags} DOS";;
    amiga*) fetch_pflags="${fetch_pflags} AMIGA";;
  esac
  if test "x$compiler_id" = 'xGNU_C'; then
    fetch_pflags="${fetch_pflags} GCC"
  fi
  case $host_os in
    mingw*) fetch_pflags="${fetch_pflags} MINGW";;
  esac
  if test "x$cross_compiling" = 'xyes'; then
    fetch_pflags="${fetch_pflags} CROSS"
  fi
  squeeze fetch_pflags
  fetch_buildinfo="
...
buildinfo.target.flags: $fetch_pflags
...
dnl FETCH_SUPPORTS_BUILTIN_AVAILABLE
dnl
...
AC_DEFUN([FETCH_SUPPORTS_BUILTIN_AVAILABLE], [
