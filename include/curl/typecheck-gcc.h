  ...
  __extension__({                                                       \
    if(__builtin_constant_p(option)) {                                  \
      CURL_IGNORE_DEPRECATION(                                          \
        ...
        if((option) == CURLOPT_HEADEREXTFUNCTION)                       \
          if(!curlcheck_write_extended_cb(value))                       \
            Wcurl_easy_setopt_err_write_extended_callback();            \
...
CURLWARNING(Wcurl_easy_setopt_err_write_extended_callback,
            "curl_easy_setopt expects a curl_write_extended_callback argument")
...
/* evaluates to true if expr is of type curl_write_extended_callback or
   "similar" */
#define curlcheck_write_extended_cb(expr)                                     \
  (curlcheck_NULL(expr) ||                                                    \
   curlcheck_cb_compatible((expr), curl_write_extended_callback) ||           \
   curlcheck_cb_compatible((expr), Wcurl_write_extended_callback1) ||         \
   curlcheck_cb_compatible((expr), Wcurl_write_extended_callback2) ||         \
   curlcheck_cb_compatible((expr), Wcurl_write_extended_callback3) ||         \
   curlcheck_cb_compatible((expr), Wcurl_write_extended_callback4) ||         \
   curlcheck_cb_compatible((expr), Wcurl_write_extended_callback5) ||         \
   curlcheck_cb_compatible((expr), Wcurl_write_extended_callback6))
typedef size_t (*Wcurl_write_extended_callback1)(const char *, size_t,
                                                 unsigned int, void *);
typedef size_t (*Wcurl_write_extended_callback2)(const char *, size_t,
                                                 unsigned int, const void *);
typedef size_t (*Wcurl_write_extended_callback3)(const char *, size_t,
                                                 unsigned int, FILE *);
typedef size_t (*Wcurl_write_extended_callback4)(const void *, size_t,
                                                 unsigned int, void *);
typedef size_t (*Wcurl_write_extended_callback5)(const void *, size_t,
                                                 unsigned int, const void *);
typedef size_t (*Wcurl_write_extended_callback6)(const void *, size_t,
                                                 unsigned int, FILE *);
