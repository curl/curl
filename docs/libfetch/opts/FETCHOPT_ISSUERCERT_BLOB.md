...
Title: FETCHOPT_ISSUERCERT_BLOB
Section: 3
Source: libfetch
See-also:
  - FETCHOPT_CRLFILE (3)
...
FETCHOPT_ISSUERCERT_BLOB - issuer SSL certificate from memory blob
...
FETCHcode fetch_easy_setopt(FETCH *handle, FETCHOPT_ISSUERCERT_BLOB,
...
This option is an alternative to FETCHOPT_ISSUERCERT(3) which instead
...
int main(void)
{
  ...
  if(fetch) {
    ...
    fetch_easy_setopt(fetch, FETCHOPT_ISSUERCERT_BLOB, &blob);
