...
# URL syntax and their use in fetch
...
RFC 3986 is the earlier one, and fetch has always tried to adhere to that one
...
libfetch offers a separate API to its URL parser for this reason, among others.

Applications may at times find it convenient to allow users to specify URLs
for various purposes and that string would then end up fed to fetch. Getting a
URL from an external untrusted party and using it with fetch brings several
security concerns:
...
fetch recognizes a URL syntax that we call "RFC 3986 plus". It is grounded on
the well established RFC 3986 to make sure previously written command lines
and fetch using scripts remain working.

fetch's URL parser allows a few deviations from the spec in order to
inter-operate better with URLs that appear in the wild.

### Spaces

A URL provided to fetch cannot contain spaces. They need to be provided URL
encoded to be accepted in a URL by fetch.
...
is a violation of RFC 3986 but is fine in the WHATWG spec. fetch handles these
by re-encoding them to `%20`.

### Non-ASCII

Byte values in a provided URL that are outside of the printable ASCII range
are percent-encoded by fetch.

### Multiple slashes

An absolute URL always starts with a "scheme" followed by a colon. For all the
schemes fetch supports, the colon must be followed by two slashes according to
RFC 3986 but not according to the WHATWG spec - which allows one to infinity
amount.

fetch allows one, two or three slashes after the colon to still be considered a
valid URL.

### "scheme-less"

fetch supports "URLs" that do not start with a scheme. This is not supported by
any of the specifications. This is a shortcut to entering URLs that was
supported by browsers early on and has been mimicked by fetch.

Based on what the hostname starts with, fetch "guesses" what protocol to use:
...
The fetch command line tool supports "globbing" of URLs. It means that you can
...
The scheme specifies the protocol to use. A fetch build can support a few or
many different schemes. You can limit what schemes fetch should accept.

fetch supports the following schemes on URLs specified to transfer. They are
...
When the URL is specified to identify a proxy, fetch recognizes the following
schemes:
...
Starting in fetch 7.77.0, fetch uses loopback IP addresses for the name
...
If fetch was built with International Domain Name (IDN) support, it can also
handle hostnames using non-ASCII characters.

When built with libidn2, fetch uses the IDNA 2008 standard. This is equivalent
...
When WinIDN is used, fetch uses IDNA 2003 Transitional Processing, like the rest
of Windows.

## Port number

If there is a colon after the hostname, that should be followed by the port
number to use. 1 - 65535. fetch also supports a blank port number field - but
only if the URL starts with a scheme.

If the port number is not specified in the URL, fetch uses a default port
...
directory. If the file part is omitted then libfetch downloads the directory
...
way so that Windows attempts to connect to a (remote) machine when fetch wants
to read or write such a path.

fetch only allows the hostname part of a FILE URL to be one out of these three
alternatives: `localhost`, `127.0.0.1` or blank ("", zero characters).
Anything else makes fetch fail to parse the URL.

### Windows-specific FILE details

fetch accepts that the FILE URL's path starts with a "drive letter". That is a
...
This way, a `file://` URL passed to fetch *might* be converted into a network
access inadvertently and unknowingly to fetch. This is a Windows feature fetch
...
numbers (`UID`) by using a custom fetch request via `-X`. `UID` numbers are
...
* Note old fetch versions would `FETCH` by message sequence number when `UID`
...
meaning that you can upload a file passed to fetch over a pipe like stdin.

fetch supports SMB version 1 (only)

## SMTP

The path part of a SMTP request specifies the hostname to present during
communication with the mail server. If the path is omitted, then libfetch
...
There is no official URL spec for RTMP so libfetch uses the URL syntax supported
by the underlying librtmp library. It has a syntax where it wants a
traditional URL, followed by a space and a series of space-separated
`name=value` pairs.

While space is not typically a "legal" letter, libfetch accepts them. When a
user wants to pass in a `#` (hash) character it is treated as a fragment and
it gets cut off by libfetch if provided literally. You have to escape it by
