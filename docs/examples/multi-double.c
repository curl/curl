...
int main(void)
{
  ...
  http_handle2 = fetch_easy_init();

  /* set options */
  fetch_easy_setopt(http_handle, FETCHOPT_URL, "https://www.example.com/");

  /* set options */
  fetch_easy_setopt(http_handle2, FETCHOPT_URL, "http://localhost/");
  ...
  fetch_multi_remove_handle(multi_handle, http_handle2);

  fetch_multi_cleanup(multi_handle);

  fetch_easy_cleanup(http_handle);
  fetch_easy_cleanup(http_handle2);
