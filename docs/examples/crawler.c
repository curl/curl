 ...
 *
 * To compile:
 *   gcc crawler.c $(pkg-config --cflags --libs libxml-2.0 libfetch)
 *
 */
/* <DESC>
 * Web crawler based on fetch and libxml2 to stress-test fetch with
...
static FETCH *make_handle(const char *url)
{
  FETCH *handle = fetch_easy_init();
  memory *mem;

  /* Important: use HTTP2 over HTTPS */
  fetch_easy_setopt(handle, FETCHOPT_HTTP_VERSION, FETCH_HTTP_VERSION_2TLS);
  ...
  mem->size = 0;
  mem->buf = malloc(1);
  fetch_easy_setopt(handle, FETCHOPT_WRITEFUNCTION, grow_buffer);
  fetch_easy_setopt(handle, FETCHOPT_WRITEDATA, mem);
  fetch_easy_setopt(handle, FETCHOPT_PRIVATE, mem);

  /* For completeness */
  fetch_easy_setopt(handle, FETCHOPT_ACCEPT_ENCODING, "");
  fetch_easy_setopt(handle, FETCHOPT_TIMEOUT, 5L);
  fetch_easy_setopt(handle, FETCHOPT_FOLLOWLOCATION, 1L);
  /* only allow redirects to HTTP and HTTPS URLs */
  fetch_easy_setopt(handle, FETCHOPT_REDIR_PROTOCOLS_STR, "http,https");
  fetch_easy_setopt(handle, FETCHOPT_AUTOREFERER, 1L);
  fetch_easy_setopt(handle, FETCHOPT_MAXREDIRS, 10L);
  /* each transfer needs to be done within 20 seconds! */
  fetch_easy_setopt(handle, FETCHOPT_TIMEOUT_MS, 20000L);
  ...
  fetch_easy_setopt(handle, FETCHOPT_HTTPAUTH, FETCHAUTH_ANY);
  fetch_easy_setopt(handle, FETCHOPT_UNRESTRICTED_AUTH, 1L);
  fetch_easy_setopt(handle, FETCHOPT_PROXYAUTH, FETCHAUTH_ANY);
  fetch_easy_setopt(handle, FETCHOPT_EXPECT_100_TIMEOUT_MS, 0L);
  return handle;
}
...
int main(void)
{
  ...
  fetch_multi_setopt(multi_handle, FETCHMOPT_MAX_TOTAL_CONNECTIONS, max_con);
  fetch_multi_setopt(multi_handle, FETCHMOPT_MAX_HOST_CONNECTIONS, 6L);

  /* enables http/2 if available */
#ifdef FETCHPIPE_MULTIPLEX
  fetch_multi_setopt(multi_handle, FETCHMOPT_PIPELINING, FETCHPIPE_MULTIPLEX);
#endif

  /* sets html start page */
  fetch_multi_add_handle(multi_handle, make_handle(start_page));
  ...
  while (still_running && !pending_interrupt)
  {
    ...
    fetch_multi_wait(multi_handle, NULL, 0, 1000, &numfds);
