...
int main(void)
{
  ...
  if (fetch)
  {
    ...
    fetch_easy_setopt(fetch, FETCHOPT_URL, "smtp://mainserver.example.net:587");
     ...
     * of using FETCHUSESSL_TRY here, because if TLS upgrade fails, the
     * transfer continues anyway - see the security discussion in the libfetch
     ...
     * FETCHOPT_SSL_VERIFYPEER and FETCHOPT_SSL_VERIFYHOST options to 0 (false).
     ...
     * libfetch using FETCHOPT_CAINFO and/or FETCHOPT_CAPATH. See docs/SSLCERTS
     * for more information. */
    fetch_easy_setopt(fetch, FETCHOPT_CAINFO, "/path/to/certificate.pem");
