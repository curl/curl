...
============================
 ...
 The command line tool fetch and the library libfetch are individually
...
## Bumping numbers
 ...
 All original fetch source release archives are named according to the libfetch
 version (not according to the fetch client version that, as said before, might
 differ).

 As a service to any application that might want to support new libfetch
 features while still being able to build with older versions, all releases
 have the libfetch version stored in the `fetch/fetchver.h` file using a static
 numbering scheme that can be used for comparison. The version number is
 defined as:

```c
#define LIBFETCH_VERSION_NUM 0xXXYYZZ
```
 ...
 `LIBFETCH_VERSION_MAJOR`, `LIBFETCH_VERSION_MINOR` and `LIBFETCH_VERSION_PATCH`.
