...
relative or an absolute URL to follow. The long parameter *mode* decide
how libcurl should act on subsequent requests.
...
response code is return to signal a redirect, the method is changed from POST
to `GET`. For 307/308, the custom method remains set and used.

Note that only POST (or a custom post) is changed to GET on 301/302. Not PUT
etc - and not when libcurl issues a custom PUT. 303 makes it switch to GET
