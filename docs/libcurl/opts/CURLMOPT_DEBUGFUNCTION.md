...
Title: CURLMOPT_DEBUGFUNCTION
Section: 3
Source: libcurl
See-also:
  - CURLINFO_CONN_ID (3)
  - CURLINFO_XFER_ID (3)
  - CURLMOPT_DEBUGDATA (3)
  - CURLMOPT_VERBOSE (3)
  - curl_global_trace (3)
Protocol:
  - All
...
int debug_callback(CURLM *multi,
                   CURL *handle,
                   curl_infotype type,
                   char *data,
                   size_t size,
...
CURLMcode curl_multi_setopt(CURLM *handle, CURLMOPT_DEBUGFUNCTION,
                            debug_callback);
~~~
...
CURLMOPT_DEBUGFUNCTION(3) adds a debug function used when
CURLMOPT_VERBOSE(3) is in effect. This callback receives debug
...
The *clientp* argument is the pointer set with CURLMOPT_DEBUGDATA(3).
...
static
int my_multi_trace(CURLM *multi, CURL *handle, curl_infotype type,
                   char *data, size_t size,
                   void *clientp)
{
  const char *text;
  (void)multi; /* prevent compiler warning */
...
int main(void)
{
  ...
  curl_multi_setopt(multi, CURLMOPT_DEBUGFUNCTION, my_multi_trace);
  curl_multi_setopt(multi, CURLMOPT_DEBUGDATA, &mydata);
}
~~~
