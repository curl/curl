...
CURLOPT_HEADEREXTFUNCTION - callback that receives header data and origin
...
CURLcode curl_easy_setopt(CURL *handle, CURLOPT_HEADEREXTFUNCTION,
...
The options CURLOPT_HEADERFUNCTION(3) and CURLOPT_HEADEREXTFUNCTION(3) cannot
be used at the same time. Setting one of these options automatically clears
the other option. Headers are only passed to a single callback function.
