...
potentially causing system resources to be exhausted resulting in a potential
system, or application, crash. Setting the CURLOPT_MAXFILESIZE_LARGE(3) option
is not sufficient to guard against this. Instead, applications should monitor
the amount of data received within the write or progress callback and abort
