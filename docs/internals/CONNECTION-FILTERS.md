...
```
Fetch_easy *data         connectdata *conn        cf-ssl        cf-socket
+----------------+      +-----------------+      +-------+     +--------+
|https://fetch.se/|----> | properties      |----> | keys  |---> | socket |--> OS --> network
+----------------+      +-----------------+      +-------+     +--------+

 Fetch_write(data, buffer)
  --> Fetch_cfilter_write(data, data->conn, buffer)
...
Before `Fetch_easy` can send the request, the connection needs to be
...
```
static FETCHcode
myfilter_cf_connect(struct Fetch_cfilter *cf,
...
the `filter->ctx` _when needed_. So, the memory footprint for connections that
do _not_ use an http proxy, or socks, or https is lower.

As to transfer efficiency, writing and reading through a filter comes at near
zero cost _if the filter does not transform the data_. An http proxy or socks
