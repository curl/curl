...
and earl. This is a
[short WAV file](https://media.merriam-webster.com/soundc11/c/curl0001.wav)
to help you.
...
### When will you make curl do XXXX ?
...
See also: [curl.se/sponsors](https://curl.se/sponsors.html)
...
[curl.se/mail](https://curl.se/mail/)
...
submit all the details at
[hackerone.one/curl](https://hackerone.com/curl?type=team).
On there we keep the issue private while we investigate, confirm it, work and
validate a fix and agree on a time schedule for publication etc. That way we
produce a fix in a timely manner before the flaw is announced to the world,
reducing the impact the problem risks having on existing users.
...
[curl.se/support](https://curl.se/support.html)
...
for this purpose:
[curl.se/docs/caextract](https://curl.se/docs/caextract.html)
...
[apache.org/dev/crypto](https://www.apache.org/dev/crypto.html)
...
them (resp.) are in PDFs here:
<!-- NOTE - full URLs or condensed link text is used to stay under 81 char. -->
- www.bis.doc.gov/index.php/documents/regulations-docs/143-bis-eccn-pdf/file
- www.bis.gov/media/documents/table-changes-enc-wa2019-rule-final-version.pdf

An incomprehensible description is downloadable here
...
properly. For more details see:

- <https://github.com/curl/curl/blob/master/docs/CONTRIBUTE.md>
- <https://github.com/curl/curl/blob/master/docs/INTERNALS.md>
...
### Does curl work/build with other SSL libraries?
...
maintain a comparison of them here:
[curl.se/docs/ssl-compared](https://curl.se/docs/ssl-compared.html)
...
it with a newer version downloaded from
[curl.se/windows](https://curl.se/windows), then Windows Update will cease to
work on your system.
...
from [curl.se/windows](https://curl.se/windows) into a separate location.
...
depends on the HTTP operation you need curl to do and what the web server
that will receive your post expects.
...
See also item
[support for JS](#does-curl-support-javascript-or-pac-automated-proxy-config)
...
Not all redirects are HTTP ones, see item
[redirects not working in curl](#redirects-work-in-browser-but-not-with-curl).
...
[curl.se/libcurl](https://curl.se/libcurl/)
...
[Why "certificate verify failed"?](##why-do-i-get-certificate-verify-failed-)
and the SSLCERTS document
[curl.se/sslcerts](https://curl.se/docs/sslcerts.html). Server certificates
that are "self-signed" or otherwise signed by a CA that you do not have a CA
cert for, cannot be verified. If the verification during a connect fails, you
are refused access. You then need to explicitly disable the verification to
...
`curl http://example.com` it will use GET. If you use `-d` or `-F` curl will
use POST, `-I` will cause a HEAD and `-T` will make it a PUT.
...
or stdin to which you can pass the secret info. curl itself will also attempt
to "hide" the given password by blanking out the option - this does not work
on all platforms.
...
www.codeproject.com/articles/Beating-the-Daylight-Savings-Time-Bug-and-Getting

### Redirects work in browser but not with curl

curl supports HTTP redirects well. See item
[tell follow HTTP redirects?](#how-do-i-tell-curl-to-follow-http-redirects-).
Browsers generally support at least two other ways to perform redirects that
curl does not:
...
### Non-functional connect timeouts on Windows
...
https://support.microsoft.com/bg-bg/topic/hotfix-enables-the-configuration-of-the-tcp-maximum-syn-retransmission-amount-in-windows-7-or-windows-server-2008-r2-1b6f8352-2c5f-58bb-ead7-2cf021407c8e
...
you will find that even if `D:\blah.txt` exists, curl returns a
...
OS-specific mechanism, then signaling libcurl to abort. See item
[How do I stop an ongoing transfer?](#how-do-I-stop-an-ongoing-transfer-).
...
built. Please review the
[guidelines for thread safety](https://curl.se/libcurl/c/threadsafe.html) to
learn more.
...
### Link errors when building libcurl
...
`libcurl.dll` and import lib, you do not need any extra `CFLAGS`, but use one
of the import libraries below. These are the libraries produced by the various
`lib/Makefile.*` files:
...
list parser is available at
[cr.yp.to/ftpparse](https://cr.yp.to/ftpparse.html). Versions of libcurl since
7.21.0 also provide the ability to specify a wildcard to download multiple
files from one FTP directory.
...
As can be seen here: [curl.se/companies](https://curl.se/docs/companies.html)
and elsewhere, more and more companies are discovering the power of libcurl
and take advantage of it even in commercial environments.
...
In one go: no. Little by little over time? Maybe. Over the years, new
languages and clever operating environments come and go. Every now and
then the urge apparently arises to request that we rewrite curl in another
language.
