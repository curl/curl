...
Added: 7.59.0
---
Before SSL handshake, if the specified file contains the available session,
then use this session. When the SSL handshake is complete, this SSL session is
...
When the second time the curl command is called with the "--ssl-session-file"
option (no "--ssl-no-ticket"), the session ID and session ticket will be
carried in ClientHello during the SSL handshake, if the server can decrypt the
session ticket, it will enter the handshake process of session reuse.
