...
When the Curl command is executed for the second time and the --ssl-session-file
option is taken, the session ID is taken in ClientHello, and if the server caches
this session, it will enter the session reuse handshake process.
