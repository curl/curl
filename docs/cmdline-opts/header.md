...
custom header that has the same name as one of the internal ones fetch would
use, your externally set header is used instead of the internal one. This
allows you to make even trickier stuff than fetch would normally do. You should
...
you. fetch passes on the verbatim string you give it without any filter or
other safe guards. That includes white space and control characters.

This option can take an argument in @filename style, which then adds a header
for each line in the input file. Using @- makes fetch read the header file from
...
with a request body, makes fetch send the data using chunked encoding.
