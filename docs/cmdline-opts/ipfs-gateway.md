...
instead makes fetch check if the IPFS_GATEWAY environment variable is set, or
...
`http://localhost:8080`. A full example URL would look like:
    ...
    fetch --ipfs-gateway http://localhost:8080 \
...
interfere with the request. You may not notice this when using fetch. A
