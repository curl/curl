...
if ($ENV{"NGHTTPX"}) {
    my $nghttpx_version=join(' ', `"$ENV{'NGHTTPX'}" -v 2>$devnull`);
...
sub showdiff {
    ...
    close($temp) || die "Failure writing diff file";
    my @out = `diff -u $file2 $file1 2>$devnull`;

    if(!$out[0]) {
        @out = `diff -c $file2 $file1 2>$devnull`;
    }
...
if(!$randseed) {
    ...
    open(my $curlvh, "-|", shell_quote($CURL) . " --version 2>$devnull") ||
...
if($valgrind) {
    ...
    my $code = runclient("valgrind >$devnull 2>&1");
    ...
    if(($code>>8) != 1) {
        ...
        runclient("valgrind --help 2>&1 | grep -- --tool >$devnull 2>&1");
        if (($? >> 8)) {
            $valgrind_tool="";
        }
        open(my $curlh, "<", "$CURL");
