                                 ...
                                 struct fetch_sockaddr *address)
{
  ...
  (void)address;
  (void)clientp;
  fprintf(stderr, "opensocket() returns FETCH_SOCKET_BAD\n");
...
FETCHcode test(char *URL)
{
  ...
  test_setopt(fetch, FETCHOPT_URL, "http://99.99.99.99:9999");
  test_setopt(fetch, FETCHOPT_VERBOSE, 1L);
  test_setopt(fetch, FETCHOPT_OPENSOCKETFUNCTION, opensocket);
