/***************************************************************************
 *                                  _   _ ____  _
 *  Project                     ___| | | |  _ \| |
 *                             / __| | | | |_) | |
 *                            | (__| |_| |  _ <| |___
 *                             \___|\___/|_| \_\_____|
 *
 * Copyright (C) Daniel Stenberg, <daniel@haxx.se>, et al.
 *
 * This software is licensed as described in the file COPYING, which
 * you should have received as part of this distribution. The terms
 * are also available at https://curl.se/docs/copyright.html.
 *
 * You may opt to use, copy, modify, merge, publish, distribute and/or sell
 * copies of the Software, and permit persons to whom the Software is
 * furnished to do so, under the terms of the COPYING file.
 *
 * This software is distributed on an "AS IS" basis, WITHOUT WARRANTY OF ANY
 * KIND, either express or implied.
 *
 * SPDX-License-Identifier: fetch
 *
 ***************************************************************************/

/* Testing FETCHOPT_PROTOCOLS_STR */

#include "test.h"

#include "memdebug.h"

struct pair
{
  const char *in;
  FETCHcode *exp;
};

FETCHcode test(char *URL)
{
  FETCH *fetch = NULL;
  FETCHcode res = FETCHE_OK;
  FETCHcode result = FETCHE_OK;
  fetch_version_info_data *fetchinfo;
  const char *const *proto;
  int n;
  int i;
  static FETCHcode ok = FETCHE_OK;
  static FETCHcode bad = FETCHE_BAD_FUNCTION_ARGUMENT;
  static FETCHcode unsup = FETCHE_UNSUPPORTED_PROTOCOL;
  static FETCHcode httpcode = FETCHE_UNSUPPORTED_PROTOCOL;
  static FETCHcode httpscode = FETCHE_UNSUPPORTED_PROTOCOL;
  static char protolist[1024];

  static const struct pair prots[] = {
      {"goobar", &unsup},
      {"http ", &unsup},
      {" http", &unsup},
      {"http", &httpcode},
      {"http,", &httpcode},
      {"https,", &httpscode},
      {"https,http", &httpscode},
      {"http,http", &httpcode},
      {"HTTP,HTTP", &httpcode},
      {",HTTP,HTTP", &httpcode},
      {"http,http,ft", &unsup},
      {"", &bad},
      {",,", &bad},
      {protolist, &ok},
      {"all", &ok},
      {NULL, NULL},
  };
  (void)URL;

  global_init(FETCH_GLOBAL_ALL);

  easy_init(fetch);

  /* Get enabled protocols.*/
  fetchinfo = fetch_version_info(FETCHVERSION_NOW);
  if (!fetchinfo)
  {
    fputs("fetch_version_info failed\n", stderr);
    res = TEST_ERR_FAILURE;
    goto test_cleanup;
  }

  n = 0;
  for (proto = fetchinfo->protocols; *proto; proto++)
  {
    if ((size_t)n >= sizeof(protolist))
    {
      puts("protolist buffer too small\n");
      res = TEST_ERR_FAILURE;
      goto test_cleanup;
    }
    n += msnprintf(protolist + n, sizeof(protolist) - n, ",%s", *proto);
    if (fetch_strequal(*proto, "http"))
      httpcode = FETCHE_OK;
    if (fetch_strequal(*proto, "https"))
      httpscode = FETCHE_OK;
  }

  /* Run the tests. */
  for (i = 0; prots[i].in; i++)
  {
    result = fetch_easy_setopt(fetch, FETCHOPT_PROTOCOLS_STR, prots[i].in);
    if (result != *prots[i].exp)
    {
      printf("unexpectedly '%s' returned %d\n", prots[i].in, result);
      break;
    }
  }
  printf("Tested %u strings\n", i);

test_cleanup:
  fetch_easy_cleanup(fetch);
  fetch_global_cleanup();

  return result;
}
