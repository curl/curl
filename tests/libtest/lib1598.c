...
FETCHcode test(char *URL)
{
  ...
  struct fetch_slist *hhl = NULL, *list;
  ...
  if (hhl)
  {
    list = fetch_slist_append(hhl, "Transfer-Encoding: chunked");
    if (!list)
      goto test_cleanup;
    hhl = list;
  }
  ...
  test_setopt(fetch, FETCHOPT_POSTFIELDSIZE, (long)strlen(post_data));
  test_setopt(fetch, FETCHOPT_POSTFIELDS, post_data);
