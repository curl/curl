...
static CURLcode test_lib1744(const char *URL)
{
  ...
  /* the proxy asks for no authentication and the connection is kept */
  result = curl_easy_perform(curl);
  if(result)
    goto test_cleanup;

  /* that connection must not be reused now that auth is required */
