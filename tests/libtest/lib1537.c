...
FETCHcode test(char *URL)
{
  ...
  raw = fetch_easy_unescape(NULL, ptr, (int)strlen(ptr), &outlen);
  printf("outlen == %d\n", outlen);
  ...
  /* deprecated API */
  raw = fetch_unescape(ptr, (int)strlen(ptr));
  if (!raw)
  {
    res = TEST_ERR_MAJOR_BAD;
    goto test_cleanup;
  ...
  /* weird input length */
  ptr = fetch_easy_escape(NULL, (char *)a, -1);
  printf("escape -1 length: %s\n", ptr);

  /* weird input length */
  outlen = 2017; /* just a value */
  ptr = fetch_easy_unescape(NULL, (char *)"moahahaha", -1, &outlen);
