...
static CURLcode init1630(CURL *curl, const char *url, const char *userpwd)
{
...
static CURLcode run1630(CURL *curl, const char *url, const char *userpwd)
{
  CURLcode result = CURLE_OK;

  result = init1630(curl, url, userpwd);
  if(result)
    return result;
...
static CURLcode test_lib1630(const char *URL)
{
  CURLcode result = CURLE_OK;
  CURL *curl = NULL;
  ...
  result = run1630(curl, URL, libtest_arg3);
  if(result)
    goto test_cleanup;

  curl_mprintf("--- Then get '%s'\n", libtest_arg2);
  result = run1630(curl, libtest_arg2, libtest_arg4);
