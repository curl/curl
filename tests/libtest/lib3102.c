...
static bool is_chain_in_order(struct curl_certinfo *cert_info)
{
  ...
  for(cert = 0; cert < cert_info->num_of_certs; cert++) {
    ...
    for(; slist && (!issuer || !subject); slist = slist->next) {
      ...
      if(!strncmp(slist->data, issuer_prefix, CSTRLEN(issuer_prefix))) {
        issuer = slist->data + CSTRLEN(issuer_prefix);
      }
      if(!strncmp(slist->data, subject_prefix, CSTRLEN(subject_prefix))) {
        subject = slist->data + CSTRLEN(subject_prefix);
      }
    }
