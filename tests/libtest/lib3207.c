...
static CURLcode test_actual(const char *URL, const char *cafile)
{
  int res = 0;
  int i;
  CURLSH* share;
...
CURLcode test(char *URL)
{
  ...
  if(!strcmp("check", URL)) {
    CURLSH *e;
    CURLSHcode w = CURLSHE_OK;
    struct curl_blob blob = {0};
    e = curl_share_init();
    if(e) {
      w = curl_share_setopt(e, CURLSHOPT_CAINFO_BLOB, &blob);
      if(w == CURLSHE_NOT_BUILT_IN)
        printf("CURLSHOPT_CAINFO_BLOB is not supported\n");
      curl_share_cleanup(e);
    }
    res = w == CURLSHE_NOT_BUILT_IN ? CURLE_NOT_BUILT_IN : CURLE_OK;
  }
  else {
    res = test_actual(URL, CAINFO);
  }
