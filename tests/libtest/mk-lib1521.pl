...
static curl_write_callback_ex headerfunction_extendedcb;
...
MOO
        ...
        if($type eq "CURLOPTTYPE_STRINGPOINT") {
            ...
            if($name eq "CURLOPT_HEADERFUNCTION_EXTENDED") {
                # Special case for extended header callback
                print $fh "${fpref}\n$i${w}headerfunction_extendedcb);\n$fcheck";
            }
            elsif($name =~ /([^ ]*)FUNCTION/) {
                my $l=lc($1);
                $l =~ s/^curlopt_//;
