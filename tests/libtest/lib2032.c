...
static size_t callback(char *ptr, size_t size, size_t nmemb, void *data)
{
  ...
  code = curl_easy_getinfo(ntlm_curl[idx], CURLINFO_LASTSOCKET, &longdata);
...
static CURLcode test_lib2032(const char *URL)  /* libntlmconnect */
{
  ...
  for(;;) {
    ...
    if(state == ReadyForNewHandle) {
      easy_init(ntlm_curl[num_handles]);

      if(num_handles % 3 == 2) {
        curl_msnprintf(full_url, urllen, "%s0200", URL);
        easy_setopt(ntlm_curl[num_handles], CURLOPT_HTTPAUTH, CURLAUTH_NTLM);
      }
      else {
        curl_msnprintf(full_url, urllen, "%s0100", URL);
        easy_setopt(ntlm_curl[num_handles], CURLOPT_HTTPAUTH, CURLAUTH_BASIC);
      }
      easy_setopt(ntlm_curl[num_handles], CURLOPT_FRESH_CONNECT, 1L);
      easy_setopt(ntlm_curl[num_handles], CURLOPT_URL, full_url);
      easy_setopt(ntlm_curl[num_handles], CURLOPT_VERBOSE, 1L);
      easy_setopt(ntlm_curl[num_handles], CURLOPT_HTTPGET, 1L);
      easy_setopt(ntlm_curl[num_handles], CURLOPT_USERPWD,
                  "testuser:testpass");
      easy_setopt(ntlm_curl[num_handles], CURLOPT_WRITEFUNCTION, callback);
      easy_setopt(ntlm_curl[num_handles], CURLOPT_WRITEDATA,
                  (void *)(ntlm_curl + num_handles));
      easy_setopt(ntlm_curl[num_handles], CURLOPT_HEADER, 1L);

      multi_add_handle(multi, ntlm_curl[num_handles]);
      num_handles += 1;
...
test_cleanup:
  ...
  for(i = 0; i < MAX_EASY_HANDLES; i++) {
    ...
    curl_multi_remove_handle(multi, ntlm_curl[i]);
    curl_easy_cleanup(ntlm_curl[i]);
  }
