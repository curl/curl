...
/* snprintf.c times a format that uses widths and a '*' argument. This test
   ...
   messages curl builds with infof() and similar all over the library; it is
   modeled on one in lib/imap.c rather than copied from it, so it measures the
   common case rather than the elaborate one. The sink is a caller's buffer,
   as in snprintf.c. */
