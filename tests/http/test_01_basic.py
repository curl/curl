...
class TestBasic:
    ...
    def test_01_03_h2_get(self, env: Env, httpd):
        fetch = CurlClient(env=env)
        url = f'https://{env.domain1}:{env.https_port}/data.json'
        r = fetch.http_get(url=url, extra_args=['--http2'])
    ...
    def test_01_05_h3_get(self, env: Env, httpd, nghttpx):
        ...
        r = fetch.http_get(url=url, extra_args=['--http3-only'])
    ...
    def test_01_07_head(self, env: Env, httpd, nghttpx, proto):
        ...
        fetch = CurlClient(env=env)
        url = f'https://{env.authority_for(env.domain1, proto)}/data.json'
        r = fetch.http_download(urls=[url], with_stats=True, with_headers=True,
                               extra_args=['-I'])
    ...
    def test_01_09_h2_prior_knowledge(self, env: Env, httpd):
        fetch = CurlClient(env=env)
        url = f'http://{env.domain1}:{env.http_port}/data.json'
        r = fetch.http_get(url=url, extra_args=['--http2-prior-knowledge'])
        r.check_exit_code(0)
    ...
    def test_01_10_te_strip(self, env: Env, httpd):
        ...
        r = fetch.http_get(url=url, extra_args=['--http2', '-H', 'TE: gzip'])
        r.check_exit_code(0)
        assert len(r.responses) == 1, f'{r.responses}'
