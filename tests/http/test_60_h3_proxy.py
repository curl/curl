...
def _check_download_size(curl: CurlClient, expected_size: int):
    ...
    assert actual == expected_size, \
        f"expected {expected_size}B download, got {actual}B"
...
class TestH3ProxySuccess:
    ...
    def test_success_matrix(self, env: Env, h2o_server, h2o_proxy,
        ...
        proxy_args = _h2o_proxy_args(env, h2o_proxy, proxy_proto,
                                     tunnel, tunneludp, insecure=True)
...
class TestH3ProxyDataTransfer:
    ...
    def _class_scope(self, env):
        doc_root = os.path.join(env.gen_dir, 'docs')
        env.make_data_file(indir=doc_root, fname='download-1m',
                           fsize=1 * 1024 * 1024)
        env.make_data_file(indir=doc_root, fname='download-10m',
                           fsize=10 * 1024 * 1024)
        env.make_data_file(indir=env.gen_dir, fname='upload-2m',
                           fsize=2 * 1024 * 1024)

    def test_large_download_10mb_via_connect(self, env: Env,
                                             h2o_server, h2o_proxy):
        _require_available(h2o_server=h2o_server, h2o_proxy=h2o_proxy)
        curl = CurlClient(env=env)
        url = f'https://localhost:{h2o_server.port}/download-10m'
        proxy_args = _h2o_proxy_args(env, h2o_proxy, 'h3', True, False,
                                     insecure=True)
        r = curl.http_download(urls=[url], alpn_proto='http/1.1',
                               with_stats=True, extra_args=proxy_args)
        r.check_response(count=1, http_status=200)
        _check_download_size(curl, 10 * 1024 * 1024)

    def test_large_upload_2mb_via_connect(self, env: Env, httpd,
                                          h2o_server, h2o_proxy):
        ...
        proxy_args = _h2o_proxy_args(env, h2o_proxy, 'h3', True, False,
                                     insecure=True)
        r = curl.http_upload(urls=[url], data=f'@{fdata}',
                             alpn_proto='http/1.1', with_stats=True,
                             extra_args=proxy_args)
        r.check_response(count=1, http_status=200)

    def test_parallel_downloads_via_connect(self, env: Env,
                                            h2o_server, h2o_proxy):
        _require_available(h2o_server=h2o_server, h2o_proxy=h2o_proxy)
        count = 5
        curl = CurlClient(env=env)
        ...
        r = curl.http_download(urls=[urln], alpn_proto='http/1.1',
                               with_stats=True, extra_args=proxy_args)
...
class TestH3ProxyConnectionManagement:
    ...
    def test_proxy_connection_reuse_sequential(self, env: Env,
                                               h2o_server, h2o_proxy):
        _require_available(h2o_server=h2o_server, h2o_proxy=h2o_proxy)
        curl = CurlClient(env=env)
    ...
    def test_quic_session_resumption(self, env: Env, h2o_server, h2o_proxy):
        ...
        proxy_args = _h2o_proxy_args(env, h2o_proxy, 'h3', True, False,
                                     insecure=True)
        r1 = curl1.http_download(urls=[url], alpn_proto='http/1.1',
                                 with_stats=True, extra_args=proxy_args)
        ...
        r2 = curl2.http_download(urls=[url], alpn_proto='http/1.1',
                                 with_stats=True, extra_args=proxy_args)
        ...
        r3 = curl3.http_download(urls=[url], alpn_proto='http/1.1',
                                 with_stats=True, extra_args=proxy_args)
...
class TestH3ProxyUdpTunnel:
    ...
    def _class_scope(self, env):
        doc_root = os.path.join(env.gen_dir, 'docs')
        env.make_data_file(indir=doc_root, fname='download-1400',
                           fsize=1400)
        env.make_data_file(indir=doc_root, fname='download-1m',
                           fsize=1 * 1024 * 1024)
        env.make_data_file(indir=doc_root, fname='download-10m',
    ...
    @MARK_NEEDS_H2O
    @pytest.mark.parametrize("fname,fsize", [
        ('download-1400', 1400),
        ('download-1m', 1 * 1024 * 1024),
        ('download-10m', 10 * 1024 * 1024),
    ])
    def test_udp_tunnel_varying_payload_sizes(self, env: Env,
                                              h2o_server, h2o_proxy,
                                              fname, fsize):
        _require_available(h2o_server=h2o_server, h2o_proxy=h2o_proxy)
        curl = CurlClient(env=env)
        url = f'https://localhost:{h2o_server.port}/{fname}'
        proxy_args = _h2o_proxy_args(env, h2o_proxy, 'h3', False, True,
                                     insecure=True)
        r = curl.http_download(urls=[url], alpn_proto='h3',
                               with_stats=True, extra_args=proxy_args)
        r.check_response(count=1, http_status=200)
        _check_download_size(curl, fsize)

    @MARK_NEEDS_NGHTTPX
    def test_udp_tunnel_capsule_protocol_absent(self, env: Env,
                                                httpd, nghttpx):
        _require_available(httpd=httpd, nghttpx=nghttpx)
        curl = CurlClient(env=env)
        url = f'https://localhost:{httpd.ports["https"]}/data.json'
        ...
        assert r.exit_code != 0, \
            "expected failure: nghttpx does not support CONNECT-UDP / " \
            "Capsule-Protocol"
...
class TestH3ProxyEdgeCases:
    ...
    def test_connect_timeout_unreachable_proxy(self, env: Env, h2o_server):
        ...
        r = curl.http_download(urls=[url], alpn_proto='http/1.1',
                               with_stats=True, extra_args=proxy_args)
        assert r.exit_code != 0, \
            "expected timeout connecting to unreachable proxy"
    ...
    def test_h2_over_udp_tunnel_rejected(self, env: Env, httpd, h2o_proxy):
        ...
        r = curl.http_download(urls=[url], alpn_proto='h2',
                               with_stats=True, extra_args=proxy_args)
        assert r.exit_code != 0, \
            "expected failure: h2 cannot work over UDP tunnel"
