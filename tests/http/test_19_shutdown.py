...
class TestShutdown:
    ...
    # check with `tcpdump` that we see fetch TCP RST packets
    ...
    # check with `tcpdump` that we do NOT see TCP RST when FETCH_GRACEFUL_SHUTDOWN set
    @pytest.mark.skipif(condition=not Env.tcpdump(), reason="tcpdump not available")
    ...
    def test_19_03_shutdown_by_server(self, env: Env, httpd, proto):
        ...
        url = f'https://{env.authority_for(env.domain1, proto)}/fetchtest/tweak/?'\
    ...
    # run downloads with FETCHOPT_FORBID_REUSE set, meaning *we* close
    # the connection after each request
    @pytest.mark.parametrize("proto", ['http/1.1'])
    def test_19_04_shutdown_by_fetch(self, env: Env, httpd, proto):
        if not env.fetch_is_debug():
            pytest.skip('only works for fetch debug builds')
        count = 10
        docname = 'data.json'
    ...
    # run event-based downloads with FETCHOPT_FORBID_REUSE set, meaning *we* close
    # the connection after each request
    @pytest.mark.parametrize("proto", ['http/1.1'])
