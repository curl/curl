...
class TestUpload:
    ...
    def test_07_01_upload_1_small(self, env: Env, httpd, nghttpx, proto):
        ...
        respdata = open(fetch.response_file(0)).readlines()
    ...
    def test_07_15_hx_put(self, env: Env, httpd, nghttpx, proto):
        ...
        url = f'https://localhost:{env.https_port}/fetchtest/put?id=[0-{count-1}]'
    ...
    def test_07_21_upload_parallel_large(self, env: Env, httpd, nghttpx, proto):
        ...
        r = fetch.http_upload(urls=[url], data=f'@{fdata}', alpn_proto=proto,
                             extra_args=['--parallel'])
        r.check_response(count=count, http_status=200)
        self.check_download(count, fdata, fetch)
    ...
    # Unfixed, this will fail when run with FETCH_DBG_SOCK_WBLOCK=80 most
    # of the time
    @pytest.mark.parametrize("proto", ['http/1.1', 'h2', 'h3'])
    def test_07_41_post_urlenc_small(self, env: Env, httpd, nghttpx, proto):
        ...
        url = f'https://{env.authority_for(env.domain1, proto)}/fetchtest/echo?id=[0-0]'
        r = fetch.http_upload(urls=[url], data=f'@{fdata}', alpn_proto=proto, extra_args=extra_args)
        r.check_stats(count=1, http_status=200, exitcode=0)
        indata = open(fdata).readlines()
        respdata = open(fetch.response_file(0)).readlines()
        assert respdata == indata

    def check_download(self, count, srcfile, fetch):
        for i in range(count):
    ...
    def test_07_42a_upload_disconnect(self, env: Env, httpd, nghttpx, proto):
        ...
        if not client.exists():
            pytest.skip(f'example client not built: {client.name}')
        url = f'https://{env.authority_for(env.domain1, proto)}/fetchtest/echo?id=[0-0]&die_after=0'
        r = client.run(['-V', proto, url])
        if r.exit_code == 18: # PARTIAL_FILE is always ok
            pass
        elif proto == 'h2':
            r.check_exit_code(92)  # FETCHE_HTTP2_STREAM also ok
        elif proto == 'h3':
            r.check_exit_code(95)  # FETCHE_HTTP3 also ok
        else:
    ...
    def test_07_42b_upload_disconnect(self, env: Env, httpd, nghttpx, proto):
        ...
        if not client.exists():
            pytest.skip(f'example client not built: {client.name}')
        url = f'https://{env.authority_for(env.domain1, proto)}/fetchtest/echo?id=[0-0]&just_die=1'
        r = client.run(['-V', proto, url])
    ...
    def test_07_42c_upload_disconnect(self, env: Env, httpd, nghttpx, proto):
        ...
        if not client.exists():
            pytest.skip(f'example client not built: {client.name}')
        url = f'https://{env.authority_for(env.domain1, proto)}/fetchtest/echo?id=[0-0]&die_after_100=1'
        r = client.run(['-V', proto, url])
        exp_code = 52  # GOT_NOTHING
        if proto == 'h2' or proto == 'h3':
            exp_code = 0  # we get a 500 from the server
    ...
    def test_07_70_put_earlydata(self, env: Env, httpd, nghttpx, proto, upload_size, exp_early):
        ...
        url = f'https://{env.domain1}:{port}/fetchtest/put?id=[0-{count-1}]'
        client = LocalClient(name='hx-upload', env=env)
        if not client.exists():
            pytest.skip(f'example client not built: {client.name}')
        r = client.run(args=[
