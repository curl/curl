...
class TestProxy:
    ...
    def test_10_09_reuse_ser(self, env: Env, httpd, nghttpx_fwd, tunnel):
        ...
        xargs = fetch.get_proxy_args(tunnel=True, proto=tunnel)
        r = fetch.http_download(urls=[url1, url2], alpn_proto='http/1.1', with_stats=True,
                               extra_args=xargs)
    ...
    def test_10_10_reuse_proxy(self, env: Env, httpd, nghttpx_fwd, tunnel):
        ...
        r2 = fetch.http_download(urls=[url], alpn_proto='http/1.1', with_stats=True,
                               extra_args=x2_args)
    ...
    @pytest.mark.skipif(condition=not Env.fetch_has_feature('HTTPS-proxy'),
                        reason='fetch lacks HTTPS-proxy support')
    @pytest.mark.skipif(condition=Env.fetch_uses_lib('bearssl'), reason="ip address cert verification not supported")
    @pytest.mark.parametrize("proto", ['http/1.1', 'h2'])
    def test_10_14_proxys_ip_addr(self, env: Env, httpd, proto):
        ...
        if env.fetch_uses_lib('mbedtls') and \
                not env.fetch_lib_version_at_least('mbedtls', '3.5.0'):
            r.check_exit_code(60) # FETCHE_PEER_FAILED_VERIFICATION
        else:
            r.check_response(count=1, http_status=200,
                             protocol='HTTP/2' if proto == 'h2' else 'HTTP/1.1')
