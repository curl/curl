...
static void my_elem_dtor(const char *key, size_t key_len, void *p)
{
  (void)p; /* unused */
  (void)key; /* unused */
  (void)key_len; /* unused */
...
UNITTEST_START
{
  ...
  nodep = Curl_hash_add(&hash_static, key1, strlen(key1), (void *)key1);
  ...
  nodep = Curl_hash_add(&hash_static, key2, strlen(key2), (void *)key2);
  fail_unless(nodep, "insertion into hash failed");
  nodep = Curl_hash_pick(&hash_static, key2, strlen(key2));
  fail_unless(nodep == key2, "hash retrieval failed");

  nodep = Curl_hash_add(&hash_static, key3, strlen(key3), (void *)key3);
  fail_unless(nodep, "insertion into hash failed");
  ...
  nodep = Curl_hash_add(&hash_static, key4, strlen(key4), (void *)key4);
  fail_unless(nodep, "insertion into hash failed");
  ...
  nodep = Curl_hash_add(&hash_static, key1, strlen(key1), (void *)notakey);
  fail_unless(nodep, "insertion into hash failed");
  nodep = Curl_hash_pick(&hash_static, key1, strlen(key1));
  ...
  nodep = Curl_hash_add2(&hash_static, key1, strlen(key1), (void *)key1,
                         my_elem_dtor);
