...
{
  Fetch_hash_init(&hash_static, slots, Fetch_hash_str,
...
if (Fetch_hash_str(key1, strlen(key1), slots) != 1 ||
    Fetch_hash_str(key2, strlen(key2), slots) != 0 ||
    Fetch_hash_str(key3, strlen(key3), slots) != 2 ||
    Fetch_hash_str(key4, strlen(key4), slots) != 1)
  fprintf(stderr, "Warning: hashes are not computed as expected on this "
                  "architecture; test coverage will be less comprehensive\n");

nodep = Fetch_hash_add(&hash_static, &key1, strlen(key1), &key1);
fail_unless(nodep, "insertion into hash failed");
nodep = Fetch_hash_pick(&hash_static, &key1, strlen(key1));
fail_unless(nodep == key1, "hash retrieval failed");

nodep = Fetch_hash_add(&hash_static, &key2, strlen(key2), &key2);
fail_unless(nodep, "insertion into hash failed");
nodep = Fetch_hash_pick(&hash_static, &key2, strlen(key2));
fail_unless(nodep == key2, "hash retrieval failed");

nodep = Fetch_hash_add(&hash_static, &key3, strlen(key3), &key3);
fail_unless(nodep, "insertion into hash failed");
nodep = Fetch_hash_pick(&hash_static, &key3, strlen(key3));
fail_unless(nodep == key3, "hash retrieval failed");

/* The fourth element exceeds the number of slots & collides */
nodep = Fetch_hash_add(&hash_static, &key4, strlen(key4), &key4);
fail_unless(nodep, "insertion into hash failed");
nodep = Fetch_hash_pick(&hash_static, &key4, strlen(key4));
...
rc = Fetch_hash_delete(&hash_static, &key4, strlen(key4));
...
rc = Fetch_hash_delete(&hash_static, &key1, strlen(key1));
fail_unless(rc == 0, "hash delete failed");
nodep = Fetch_hash_pick(&hash_static, &key1, strlen(key1));
fail_unless(!nodep, "hash retrieval should have failed");
nodep = Fetch_hash_pick(&hash_static, &key4, strlen(key4));
...
nodep = Fetch_hash_add(&hash_static, &key1, strlen(key1), &notakey);
fail_unless(nodep, "insertion into hash failed");
nodep = Fetch_hash_pick(&hash_static, &key1, strlen(key1));
...
nodep = Fetch_hash_add2(&hash_static, &key1, strlen(key1), &key1,
                       my_elem_dtor);
