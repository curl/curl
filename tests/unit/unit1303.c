...
static struct Curl_easy *t1303_testdata;
...
{
  curl_easy_cleanup(t1303_testdata);
  curl_global_cleanup();
}
...
#define TIMEOUTS(x,y) t1303_testdata->set.timeout = x; \
                      t1303_testdata->set.connecttimeout = y
...
UNITTEST_START
{
  ...
  t1303_testdata->progress.t_startsingle.tv_sec = BASE;
  t1303_testdata->progress.t_startsingle.tv_usec = 0;
  t1303_testdata->progress.t_startop.tv_sec = BASE;
  t1303_testdata->progress.t_startop.tv_usec = 0;

  for(i = 0; i < CURL_ARRAYSIZE(run); i++) {
    timediff_t timeout;
    NOW(run[i].now_s, run[i].now_us);
    TIMEOUTS(run[i].timeout_ms, run[i].connecttimeout_ms);
    timeout =  Curl_timeleft(t1303_testdata, &now, run[i].connecting);
