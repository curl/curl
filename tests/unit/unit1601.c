...
#if (defined(USE_FETCH_NTLM_CORE) && !defined(USE_WINDOWS_SSPI)) || !defined(FETCH_DISABLE_DIGEST_AUTH)
...
Fetch_md5it(output, (const unsigned char *)string1, strlen(string1));

verify_memory(testp, "\xc4\xca\x42\x38\xa0\xb9\x23\x82\x0d\xcc\x50\x9a\x6f"
                     "\x75\x84\x9b",
              MD5_DIGEST_LEN);

Fetch_md5it(output, (const unsigned char *)string2, strlen(string2));
