...
struct test_spec {
  void (*setbuf)(struct test_spec *spec, struct dynbuf *buf);
  size_t n;
  CURLcode exp_result;
};

static void make_nested(struct test_spec *spec, struct dynbuf *buf)
{
  size_t i;
...
static bool do_test(struct test_spec *spec, size_t i, struct dynbuf *buf)
{
  CURLcode result;
