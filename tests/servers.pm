...
sub clearlocks {
    ...
    if(os_is_win()) {
        ...
        if(checkcmd($handle)) {
            ...
            print "clearlocks: $^O: $runnerid: Executing: '$cmd'\n";
            my @handles = `$cmd`;
            for my $tryhandle (@handles) {
                ...
                if($tryhandle =~ /^(\S+)\s+pid:\s+(\d+)\s+type:\s+(\w+)\s+([0-9A-F]+):\s+(.+)\r\r/) {
                    print "clearlocks: $^O: $runnerid: Found $3 lock of '$5' ($4) by $1 ($2)\n";
                    # Ignore stunnel since we cannot do anything about its locks
                    if("$3" eq "File" && "$1" ne "tstunnel.exe") {
                        print "clearlocks: $^O: $runnerid: Killing IMAGENAME eq $1 and PID eq $2\n";
