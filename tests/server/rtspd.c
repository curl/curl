...
static int rtspd_ProcessRequest(struct rtspd_httprequest *req)
{
        ...
        if(cmdsize) {
          ...
          do {
            ...
            if(!strncmp(CMD_AUTH_REQUIRED, ptr,
                        CSTRLEN(CMD_AUTH_REQUIRED))) {
              logmsg("instructed to require authorization header");
              req->auth_req = TRUE;
            }
            else if(!strncmp(CMD_IDLE, ptr, CSTRLEN(CMD_IDLE))) {
              logmsg("instructed to idle");
              req->rcmd = RCMD_IDLE;
              req->open = TRUE;
            }
            else if(!strncmp(CMD_STREAM, ptr, CSTRLEN(CMD_STREAM))) {
  ...
  if(req->pipe)
    ...
    req->checkindex += (end - line) + CSTRLEN(END_OF_HEADERS);
  ...
  if(!req->pipe &&
     req->open &&
     req->prot_version >= 11 &&
     req->reqbuf + req->offset > end + CSTRLEN(END_OF_HEADERS) &&
     (!strncmp(req->reqbuf, "GET", CSTRLEN("GET")) ||
      !strncmp(req->reqbuf, "HEAD", CSTRLEN("HEAD")))) {
    /* If we have a persistent connection, HTTP version >= 1.1
       and GET/HEAD request, enable pipelining. */
    req->checkindex = (end - req->reqbuf) + CSTRLEN(END_OF_HEADERS);
    req->pipelining = TRUE;
  }
