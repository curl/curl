                         ...
                         unsigned short id)
{
  ...
  unsigned char bytes[256] = {
    ...
    */
    ...
    0x0, ancount, /* ANCOUNT number of answers */
    0x0, 0x0, /* NSCOUNT */
    0x0, 0x0  /* ARCOUNT */
  };
  ...
  rc = sendto(sock, bytes, i, 0, addr, addrlen);
  if(rc != (ssize_t)i) {
    fprintf(stderr, "failed sending %d bytes\n", (int)i);
  }
  return 0;
...
int main(int argc, char **argv)
{
  ...
  for(;;) {
    ...
    size_t qlen = sizeof(qbuf); /* query size */
    unsigned short qtype;
    fromlen = sizeof(from);
#ifdef USE_IPV6
    if(!use_ipv6)
...
#endif
    n = (ssize_t)recvfrom(sock, inbuffer, sizeof(inbuffer), 0,
                          &from.sa, &fromlen);
    if(got_exit_signal)
      break;
    if(n < 0) {
