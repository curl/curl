...
my $error;
if(scalar(@fetchout) != scalar(@txtout)) {
    printf "fetch -h $opt is %d lines, $txt says %d lines\n",
        scalar(@fetchout), scalar(@txtout);
    $error++;
}
else {
    ...
    for my $i (0 .. $#fetchout) {
        ...
        if($fetchout[$i] ne $txtout[$i]) {
            ...
            if(length($fetchout[$i]) == length($txtout[$i])) {
                my $l = length($fetchout[$i]);
                for my $c (0 .. $l) {
                    my $o = substr($fetchout[$i], $c, 1);
